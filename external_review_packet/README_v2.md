...
|---|---|---|
| `revised_sustainability_manuscript.md` | The monograph working preprint v1.0 (the architectural kernel, public and citable) | **Working preprint** — v1.1 (`revised_sustainability_manuscript_v1.1.md`, batch-5: the hybrid-trajectory formula repair, the completed §27 indicator rationale, one citation) supersedes it as the citable record |
...
- **`BATCH5_JOINT_AUDIT_EVALUATION.md`** — the batch-5 joint evaluation: every finding from the four auditors, adjudicated with verification evidence; the rejected findings argued from the committed artifacts; the implementation map to the second editions.
...
| Batch-5 editions | `python3 reaudit/verify_batch5_editions.py` | PASS (all second-edition hashes and content checks) |
