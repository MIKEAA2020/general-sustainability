...
*Second edition (batch-5 corrections, 2026-08-29): the family enumeration of the Abstract is completed to the twelve families actually carried; four restored remarks (5.9, 6.16, 8.15, 13.4) are moved to their numbered positions so section numbering is monotone in order of appearance; the one-step proof of Theorem 6.4 is supplied, discharging its registered obligation; the defining data of Conditional Theorem 10.1 and the CES specification of Theorem 5.2 are stated in the manuscript (restored preambles); the quantifier phrasing of Theorem 3.4 and the identifier split of the hypothesis objects of §12 are corrected; the ledger renderings are standardised. The adjudication record is `BATCH5_JOINT_AUDIT_EVALUATION.md`; no claim status, CC-identifier set, or row count changes.*
...
**Remark A.3 (one balance per moiety)** [CC-A002-009 · remark, bounded appendix]: the conservation theorem gives one balance per declared moiety — the no-summing scope remark is stated in Appendix A.3.
...
**Theorem 5.2 (Capital–resource substitution thresholds) [CC-A001-049 · theorem].** For fixed $R > 0$, let $c_{\max}(R) = \sup_{A \geq 0} [F(A, R) - \delta_A A]$. Then: (1) if $\sigma < 1$, $F(A,R)$ converges to a finite ceiling and $c_{\max}(R) < \infty$; (2) if $\sigma = 1$, $F(A,R)/A \to 0$ and $c_{\max}(R) < \infty$ for $\delta_A > 0$; (3) if $\sigma > 1$ and $\mu_A < \delta_A$, $c_{\max}(R) < \infty$; (4) if $\sigma > 1$ and $\mu_A > \delta_A$, $c_{\max}(R) = +\infty$; (5) if $\sigma > 1$ and $\mu_A = \delta_A$, the boundary case also gives $c_{\max}(R) = +\infty$. The threshold is
...
*Scope:* the multipliers are a separation certificate, not universal exchange rates; the feasibility caveat for nonlinear, nonconvex, path-dependent, spatial, or irreversible technologies is Remark A.4.
...
**Conditional Theorem 10.1 (Local-horizon bracket) [CC-A002-040 · conditional theorem, local diagnostic].** Let $A(0) > A_{\min}$ and define $y(t) = A(t) - A_{\min}$; let the current net depletion rate be $v_0 = -\dot A(0) > 0$ and the local ratio $H_{\rm loc} = y(0)/v_0$. Suppose $0\le\epsilon<1$ and $y$ is continuously differentiable on $[0,H_*]$ for some $H_*\ge H_{\rm loc}/(1-\epsilon)$, and suppose throughout that interval that $(1-\epsilon)v_0\le-\dot A(t)\le(1+\epsilon)v_0$. Then the first hitting time $T_A$ of $\{A \le A_{\min}\}$ (the first $t$ with $y(t) = 0$) exists in $[0,H_*]$ and satisfies
...
Every retained row of this manuscript, with claim status, evidence status (post-closure), and primary destination. `P` = provenance concordance row in `research_program/canonical_concordance_A001_A025.csv`. Review state: all 89 rows `row_verified` (A001/A002 scientific closure passes, 2026-08-27; A003/A005/A006/A007/A010/A013/A018 closure passes, 2026-08-28 — every source read end to end; kind, module, mapping type, and evidence status verified per row) — content-level acceptance; no theorem status promoted; the cross-module interface contract remains open per row. Rows 1–70 are the budgeted selection (63 main + 7 bounded-appendix); rows 71–89 are the closure-campaign seam routings — rows the destination passes and seam closures of the 2026-08-28 campaign assign to this atlas from the seven further closed sources (rows 71–88) together with one further A002 row (row 89, CC-A002-050), stated here at exactly their verified statuses.
...
| 24 | CC-A001-026 | Instantaneous common-action obstruction | Thm 6.4 | theorem | proof omitted in source; one-step proof obligation registered — supplied in this manuscript | Paper 2 |
...
| 64 | CC-A001-084 | Nested-constraint impossibility | Thm 13.1 | theorem (limit) | proof verified present | Paper 1 (conditional on the registered prerequisite result) / Paper 2 |
...
| 85 | CC-A003-003 | H3: inertia/capture/state-dependent response | Hypothesis object 12.3a | defined source object | defined source object | Paper 2 (seam: Paper 4 §8.1) |
| 86 | CC-A005-006 | Competing-institutional-hypothesis ladder (groundwater restatement) | Hypothesis object 12.3b | defined source object (registered template obligation) | defined source object | Paper 2 |
