# Conserved Material Ledgers and Componentwise Depletion Diagnostics: Typed Stock–Flow Accounting, the Closed Finite-Donor Ledger, and Depletion-Horizon Semantics

**Paper 3 of the programme's five-paper publication architecture (general-sustainability, A001–A025).** Every concordance-sourced statement is stated at its row-verified status; proofs are reproduced in full where short and summarized with verified-present status otherwise; application records carry their declared evidentiary level; conditional results stay conditional; no status is promoted.

---

## Abstract

This article develops the material-accounting layer of a typed theory of sustainability: how stocks, services, products, waste, detritus, active and geological pools, and depletion diagnostics are represented without compensatory aggregation and without double counting. The representation is a typed primitive-flux ledger: compartments carry a moiety, a boundary, and a unit; non-negative primitive fluxes connect them through a signed incidence matrix; conversions between types appear only as explicit stoichiometric coefficients; and every one-way transfer is donor-limited. On this scaffold the paper states and proves the closed finite-donor ledger's complete theorem set — a system in which the geological donor is a state and no derived target appears: the natural-block mass identity (mass leaves the natural block exactly at the extraction and mining rates), forward invariance of the nonnegative orthant, the absence of any interior rest point at positive effort, the extinction–geochemical rest set as the only rest structure with vanishing extraction, and integrability of extraction on the finite donor budget. The canonical conservation family (typed hybrid conservation, the closed positive-moiety bound, and the sufficiency of donor limitation for the tangency condition) is stated with the atlas as proof owner. A service layer is added as a typed readout, never as conserved mass: the state-dependent feasible balance domain, the support-provenance partition with its directional support gap, and the componentwise deficit, whose single-resource specialization yields the exact identity $qEN-R=-\dot N$. Depletion time is then separated into three non-interchangeable quantities — gross turnover/dependency intensity, a frozen-rate ratio, and a scenario-conditioned hitting time — and the application classifications are stated verbatim: the G3P anomaly-persistence index is a statistical index, not a physical stock ratio; the phosphate reserve-life ratio is arithmetic, not an exhaustion forecast; and the fisheries removals-only time is outside the three-quantity hierarchy. First-passage semantics are supplied by two proved theorems on declared stochastic surrogates (inverse-Gaussian groundwater passage with median below mean; geometric-Brownian fisheries passage via Itô calculus) together with the record-relative-barrier discipline and seven explicit non-claims. The phosphorus and groundwater domain templates enter as registered identification ladders with falsification protocols and displacement disciplines; no constitutive content is claimed for them. The boundary to the companion nonlinear-dynamics paper is fixed by an interface contract: the exact shared object is the single-resource deficit identity, and no exact dynamic reduction from the closed primitive ledger to the working retarded field exists — a rejection stated with its five reasons. Every claim carries its concordance provenance at its closed status.
...
All 52 of this paper's concordance rows are row-closed at content level (`row_verified`, dated scientific passes over full source reads: A001 and A002 on 2026-08-27; A003, A004, A005, A006, A010, A012, A013, A018, A019, and A024 on 2026-08-28); they are stated below at exactly those statuses, with no promotion. Content-level acceptance means the row's existence, kind, proof presence, module, and mapping type were verified against the source; it is not a theorem-status promotion, and the cross-module interface contract remains an open obligation recorded per row. Of the 409-row concordance, 354 rows are row-verified; the 27 rows that remain open are exactly the three gated conditional-paper sources (A021, A022, A023) — none of them behind this paper.

### 1.5 Relationship to the programme

This is Paper 3 of five assured papers. Paper 1 owns the typed canonical architecture (the type system, the diagnostic types and their no-transfer rule, the model-map taxonomy); this paper carries a Minimal Working Realization of the ledger objects it needs and never transfers a status across modules. Paper 2 owns the theorem atlas, including the canonical conservation and invariance family whose ledger-side readings are stated here — where a full proof lives in the atlas, it is cited there, not reproduced beyond what local use requires. Paper 4 owns the named open/frozen-donor retarded systems and their bifurcation results; the boundary between that paper and this one is fixed by the ledger-to-dynamics interface contract, stated in §8, whose exact shared object is the single-resource deficit identity. Paper 5 owns the sampled-governance family, the empirical identification programme, and the worked fisheries case. The monograph reintegrates at full length after external scrutiny; two scored-forecast papers exist in the programme and are outside this paper's scope. No paper depends on another for a locally load-bearing definition: the ledger equations, closures, and diagnostics used here are stated in full.

---

## 2 The primitive typed ledger (Minimal Working Realization)

### 2.1 Typed stocks, primitive fluxes, and the incidence discipline [CC-A010-003 · identity]
...
### 2.4 The four-stock resource–sink–nutrient–product system [CC-A001-042 · theorem, exact specialization]

A second exact specialization closes a resource–sink system with a nutrient stock $N$ and product stock $P$: state $(S,K,N,P)\in\mathbb R^4_+$ with
...
The reading is exact: the closed natural block loses mass at precisely the extraction and mining rates, and nothing else. Conservation is a property of the routing, not of the constitutive laws' magnitudes.

### 3.5 Conservation of the six-compartment ledger [CC-A013-005 · theorem, proof reproduced]

**Theorem 3.7 (Mass conservation) [CC-A013-005 · theorem].** For the six-compartment system of §2.3, the total tracked material $M=X+U+A+G+P+W$ is constant along every classical solution.
...
### 3.6 Orthant invariance [CC-A019-003 · theorem, proof reproduced; CC-A013-006 · theorem; CC-A018-003 · theorem]

**Theorem 3.8 (Orthant invariance of the closed ledger) [CC-A019-003 · theorem].** The nonnegative orthant in $(N,A^{\mathrm{act}},A^{\mathrm{geo}},U,Z,E)$ is forward invariant for the closed natural block.
...
### 3.8 The extinction–geochemical rest set [CC-A019-005 · theorem]

**Theorem 3.12 (Extinction–geochemical rest set) [CC-A019-005 · theorem].** The set $N=0$, $U=0$, $E$ arbitrary, $A^{\mathrm{act}}=A^{\mathrm{eq,intrinsic}}\sigma$ consists of rest points of the natural block. If $A_{g0}=0$ and $\sigma\equiv1$ for $A^{\mathrm{geo}}>0$, this is the ray $A^{\mathrm{act}}=A^{\mathrm{eq,intrinsic}}$, $A^{\mathrm{geo}}\ge0$. *Proof (verified present; summary):* with $N=U=0$ the remaining equations are $\dot A^{\mathrm{act}}=\omega_AA^{\mathrm{eq,intrinsic}}\sigma-\omega_AA^{\mathrm{act}}$ and its negative in $A^{\mathrm{geo}}$; both vanish iff $A^{\mathrm{act}}=A^{\mathrm{eq,intrinsic}}\sigma$. $\blacksquare$ The institutional memory yields $E\to E^*$ at $N=0$ with extraction vanishing identically — consistent with the rest set and not an interior rest. The scope statement is exact: the only rest points with vanishing extraction are extinction plus geochemical equilibrium.

### 3.9 Extraction integrability [CC-A019-007 · theorem, proof reproduced]

**Theorem 3.13 (Integrable extraction) [CC-A019-007 · theorem].** Let $M=N+A^{\mathrm{act}}+A^{\mathrm{geo}}+U$. Then $M(t)=M(0)-\int_0^tqE(s)N(s)\,ds\ge0$, so
...
### 3.10 The six-state material cancellation and its limit [CC-A010-008 · identity]
...
### 4.2 The state-dependent feasible balance domain [CC-A013-002 · definition]

**Definition 4.1 (Feasible balance domain) [CC-A013-002 · definition].** For an admissible operating set $\mathcal U(x,t)$ and a declared demand set $\mathcal D(t)$,
...
### 4.3 Support provenance and the directional support gap [CC-A002-014 · definition; CC-A013-003 · definition]

Current service adequacy and regenerative feasibility are different claims. Let $\Gamma_{\rm all}(x,t)\subseteq\mathbb R^n_+$ contain the service vectors feasible through all pathways admitted by an application, and $\Gamma_{\rm reg}(x,t)\subseteq\Gamma_{\rm all}(x,t)$ the feasible set after imposing the declared regenerative-flow, system-boundary, material-quality, and exergy or capacity restrictions. These correspondences are application inputs obtained from a typed pathway or technology model; the stock ledger alone does not construct them.

**Definition 4.2 (Directional regenerative-support fraction and gap) [CC-A013-003 · definition; CC-A002-014 · definition].** Assume $0\in\Gamma_{\rm reg}(x,t)$ and choose a nonzero service direction $\bar s\ge0$. Define

$$
\alpha_{\rm reg}(\bar s;x,t)=\sup\{\alpha\in[0,1]:\ \alpha\bar s\in\Gamma_{\rm reg}(x,t)\}.
$$

The vector $(1-\alpha_{\rm reg})\bar s$ is the *directional support gap*, measured in the same service units as $\bar s$. A realized service $s\in\Gamma_{\rm all}\setminus\Gamma_{\rm reg}$ is support-dependent under that declaration even when $s\ge d$.

The inventory's own status line is part of the row: attainment requires closedness — if $\Gamma_{\rm reg}$ is not closed the supremum may not be attained, and the gap is relative to a supremal fraction, not necessarily an achievable boundary service. The non-interpretation discipline is equally part of it: the statement neither subtracts raw material from service nor proves that a physical stock is declining; net depletion still requires a negative stock balance or a trajectory argument. The provenance partition behind $\Gamma_{\rm reg}$ (renewable flow, recovered or recycled material, imports, non-renewable drawdown) never adds unlike physical units.

### 4.4 The componentwise deficit and the specialization identity [CC-A018-004 · lemma; CC-A012-005 · identity]
...
**Lemma 4.3 (Exact specialization deficit identity) [CC-A018-004 · lemma].** On every trajectory of the specialized system, and of every reduced core whose stock equation is $\dot N=R(N,A)-qEN$,
...
### 5.1 Gross turnover and dependency [CC-A013-007 · definition + false-implication record]

**Definition 5.1 (Gross turnover intensity and support coverage) [CC-A013-007 · definition].** With assimilation $g(X,A)>0$, define the gross turnover intensity $J_A^{\rm gross}=g(X,A)/A$ and the gross support-coverage ratio $H_A^{\rm gross}=(A-A_{\min})/g(X,A)$.
...
### 5.2 The frozen-rate ratio [CC-A013-008 · definition]

**Definition 5.2 (Local net-depletion ratio) [CC-A013-008 · definition].**

$$
H_A^{\rm loc}(t)=\frac{A(t)-A_{\min}}{\bigl[-\dot A(t)\bigr]_+},
$$

with the extended-real convention $H_A^{\rm loc}=+\infty$ when $\dot A\ge0$ — correctly reporting no current net decline at a stationary or replenishing state.
...
### 5.3 The scenario-conditioned hitting time [CC-A013-009 · definition]

**Definition 5.3 (Scenario-conditioned hitting time) [CC-A013-009 · definition].** For a fully specified dynamical model, policy or scenario $\pi$, disturbance history $d$, and initial state $x_0$,
...
### 5.4 The local threshold-horizon bound [CC-A010-002 · theorem]

**Theorem 5.4 (Local threshold-horizon bracket) [CC-A010-002 · theorem].** Let $A:[0,T]\to\mathbb R$ be absolutely continuous with $A(0)>A_{\min}$, let $v_0>0$ and $0<\varepsilon<1$, set $H_0=(A(0)-A_{\min})/v_0$, and suppose $T\ge H_0/(1-\varepsilon)$ and
...
*Proof (verified present; summary):* if no crossing occurs before $t_*=H_0/(1-\varepsilon)$, absolute continuity gives $A(t_*)\le A(0)-(1-\varepsilon)v_0t_*=A_{\min}$, a contradiction; integrating both rate bounds over $[0,H]$ and using $A(0)-A(H)=v_0H_0$ yields the two-sided bracket. $\blacksquare$ The scope note is part of the row: this is a local diagnostic only — it fails when depletion reverses, the rate approaches zero, or feedback moves the trajectory outside the declared rate bounds. The atlas carries the canonical form of the same bracket [Paper 2, Cond. Thm 10.1; CC-A002-040]; this row is its predecessor statement, recorded here with canonical credit and used locally to bracket the frozen-rate ratio's error under declared rate bounds.

### 5.5 Application classifications at their exact status

#### 5.5.1 Groundwater anomaly-persistence indices [CC-A013-010 · application record]

The G3P v1.12 product provides monthly groundwater-storage anomalies relative to a reference period rather than absolute aquifer volumes. For a basin-mean anomaly series over the reported April 2002–September 2023 window, the Linear-Trend Anomaly Persistence Index is

$$
L_{\rm hist}^{\rm anom}=\frac{a_{\rm latest}-a_{\rm hist,min}}{\bigl[-\widehat{\dot a}\bigr]_+},
$$

the fitted distance to the series' own historical minimum divided by the fitted decline rate. The four-basin record: Indo-Gangetic $-49.7$ cm/yr with index $\approx2.7$ yr; North China Plain $-18.6$ with $\approx7.9$; Central Valley $-16.1$ with $\approx9.5$; La Mancha $-3.2$ with $\approx21.4$. Classification, stated at the source's own status: a statistical anomaly index with units of time — not the physical stock ratio $H_A^{\rm loc}$ and not a forecast of aquifer exhaustion. Its value depends on the product window, basin mask, anomaly reference, and linear-trend convention; a physical $H_A^{\rm loc}$ requires an absolute stock estimate and a net stock derivative (aquifer geometry or saturated thickness together with storage parameters), not an anomaly series alone. The values are accepted at attested source status; the submission-stage supplement (processing files, source extracts, shared references) is pending.

#### 5.5.2 The applied depletion-horizon tables [CC-A018-017 · application record]

The unified applied source reports component-resolved depletion horizons on the same three public domains, computed without fitting any dynamical parameter of the reduced core:
...
The ledger's own first-passage object is the model hitting time of Definition 5.3 — a quantity on trajectories of the mass-conserved ledger or of a named reduced system. The public-data quantities of §5.5 are constructed proxies on observed series. The distinction is the entry discipline of this section: the surrogates below do not compute the ledger's hitting time, do not complete the ledger stochastically, and do not identify physical failure thresholds; the scoping statement is shared with Paper 5, which owns the model-side object and its empirical identification [CC-A024-001].

### 6.2 The observed-drift Brownian surrogate [CC-A024-002 · definition]

**Definition 6.1 (Observed-drift Brownian surrogate) [CC-A024-002 · definition].** Let $A_0$ be the latest observed anomaly and $\mu=\widehat\mu<0$ the fitted drawdown rate. On the scale of the tabulated series define
...
### 6.3 The inverse-Gaussian groundwater first passage [CC-A024-003 · theorem]

**Theorem 6.2 (Inverse-Gaussian first passage) [CC-A024-003 · theorem].** Let $T_{\mathrm{GW}}=\inf\{t>0:\ A(t)\le A_{\min}^{\mathrm{win}}\}$ for the process of Definition 6.1 and $d=A_0-A_{\min}^{\mathrm{win}}>0$. Conditional on treating $\mu$ and the barrier as fixed,
...
*Proof (verified present; summary).* The first-passage time of a Brownian motion with constant negative drift to a lower barrier is inverse Gaussian; substituting the initial gap $d$, drift magnitude $|\mu|$, and diffusion scale $\sigma$ gives the law, and the standard inverse-Gaussian moments give the displayed mean and variance. $\blacksquare$
...
### 6.4 Moments, zero-noise limit, and median [CC-A024-004 · corollary]

**Corollary 6.3 (Zero-noise limit and median) [CC-A024-004 · corollary].** As $\sigma\to0^+$, $T_{\mathrm{GW}}\to H^{\mathrm{win}}_{\mathrm{GW}}$ in probability, and at $\sigma=0$ the deterministic trajectory reaches the barrier exactly there. For every finite $\sigma>0$ the inverse-Gaussian median $m$ satisfies
...
### 6.5 The record-relative barrier discipline [CC-A024-005 · counterexample/limit]
...
### 6.6 The geometric-Brownian fisheries first passage [CC-A024-006 · theorem]

**Theorem 6.4 (Geometric-Brownian correction) [CC-A024-006 · theorem].** Let $dB_t=-hB_t\,dt+\sigma B_t\,dW_t$ under the Itô convention with $h>0$ and $0<B_{\min}<B_0$, and $T_{\mathrm{fish}}=\inf\{t>0:\ B_t\le B_{\min}\}$. Then
...
*Proof (verified present; summary):* Itô's lemma gives $d\log B_t=-(h+\sigma^2/2)\,dt+\sigma\,dW_t$, so the logarithmic threshold is a Brownian first-passage problem with initial distance $\log(B_0/B_{\min})$ and downward drift $h+\sigma^2/2$; the inverse-Gaussian result of Theorem 6.2 applies. $\blacksquare$

For fixed arithmetic drift and the Itô parameterization, the finite-noise mean is strictly shorter than the deterministic horizon. This is a property of the chosen surrogate parameterization; it is not a universal claim that environmental variability accelerates physical biomass loss. The construction joins the removals-only classification of §5.5.4 — the same pure-decay process, now under a declared stochastic surrogate — and the worked fisheries case at the boundary with Paper 5.

### 6.7 The constant-production phosphate passage time [CC-A024-007 · application record]
...
### 6.8 The explicit non-claims [CC-A024-009 · counterexample/limit]
...
holds for every trajectory of either the specialized ledger or the named core. *Proof:* substitute the stock equation (Lemma 4.3). $\blacksquare$ The mapping type is exact specialization for the identity once the local stock equation and $\mathcal S_{1R}$ are imposed; the three-state core's constitutive replacement $R(N,A)\to rN(1-N/K)$ is separately an approximation and carries its own finite-time scope. Paper 4 restates the same identity locally in one line; the identity is the one object both papers may prove without substantive duplication.

**The hand-off projection [CC-A018-005 · theorem, projectable reduction].** Under the institutional-failure specialization, the macroeconomic block, prices, and demand do not appear in $(\dot N,\dot A,\dot U,\dot Z,\dot E)$; the ecological–institutional subsystem is an exact closed projection for every $\varepsilon>0$, with no singular limit required. *Proof (verified present; summary):* under the specialization, each of the five right-hand sides depends only on the block's own variables and the delayed memory; none contains the macroeconomic states, prices, or demand. $\blacksquare$ The projection is classified as a projectable reduction in the atlas's projectability-criterion family [Paper 2; CC-A002-036]; this paper states the hand-off, and Paper 4 states the named retarded equations it receives.

**The non-reduction boundary [MS-Native-2 · boundary statement, restated from the contract].** There is no exact dynamic reduction from the closed primitive finite-donor ledger (`LEDGER-PRIM-CLOSED-v1`) to the working four-state field (`DYN-C4-WORKING`) — not as a projectable reduction and not as a regular perturbation. The reasons are mathematical:
...
The certification boundary is an algebraic fact. For every positive weight vector $w$ and every component $k$, the construction $b_k=-L$, $b_j=(w_kL+1)/w_j$ (all other components zero) gives $w^{\top}b=1>0$ despite $b_k=-L<0$, for any severity $L$; the atlas carries the two witness rows of this construction from the accounting and applied sources [Paper 2; CC-A013-001, CC-A018-001]. Consequently an unrestricted compensatory certificate does not exist: a positive weighted sum cannot certify the conjunction of componentwise nonnegativity. On a restricted feasible domain such as $\mathcal B(x,t)$ (Definition 4.1), a scalar certificate requires the separately proved implication $b\in\mathcal B(x,t)$, $w^{\top}b\ge0\Rightarrow b\ge0$ — provable only from the physical restrictions defining the domain, and vacuous where they exclude nothing. Non-compensatory scalar encodings exist when reference scales and zero conventions are declared — $\min_ib_i$, $\|[-b]_+\|$, and $\max_i[-b_i]_+/s_i^{\rm ref}$ preserve the conjunction $b_i\ge0$ — but they discard the identity and cause of the limiting component, so the vector accompanies them. Scalar summaries may rank or communicate; certification requires the full vector, a logically equivalent non-compensatory encoding, component thresholds, or a proved feasible-domain implication.
...
Three rows of this paper are counterexample-or-limit rows and are stated as such: the fisheries removals-only time (§5.5.4) exists to block a promotion; the record-relative barrier discipline (§6.5) exists to qualify every groundwater number in §5.5; the seven non-claims (§6.8) exist to fence the first-passage theorems off from the physical ledger. Together with the false-implication record of §5.1, the conditional status of §3.2, and the rejected dynamic mapping of §8, they carry the paper's boundary content: the accounting layer certifies balances, positivity, and classifications — and it declares, on the line, everything it does not certify. A diagnostic identifies where a floor is violated or how fast a stock currently declines; it does not establish a mechanism, and under the architecture's no-transfer rule a diagnostic claim never inherits the status of a control or causal claim [Paper 1; CC-A002-005].

---

## 10 Status ledger

Every retained row of this manuscript, with claim status, evidence status (post-closure), and destination note. Review state: all 52 rows `row_verified` (dated scientific closure passes over full source reads, 2026-08-27/28) — content-level acceptance; no theorem status promoted; the cross-module interface contract remains open per row.

| ID | statement | status | evidence | destination-note |
|---|---|---|---|---|
| CC-A001-042 | Four-stock mass balance (Proposition 6.1) | theorem (exact specialization) | proof verified in source §6.6; row-verified 2026-08-27 | Paper 3 §2.4 (monograph restates) |
| CC-A002-008 | Typed hybrid conservation ($L^{\top}S=0$, $L^{\top}S^{J}=0$) | theorem | proof verified in source §3; row-verified 2026-08-27 | Paper 3 §3.1 (Paper 2 proof owner) |
| CC-A002-010 | Closed positive-moiety component bound | theorem (corollary) | proof verified in source §3; row-verified 2026-08-27 | Paper 3 §3.1 (Paper 2 proof owner) |
| CC-A002-012 | Donor-limited outflows and boundary flows suffice for tangency | theorem (corollary) | proof verified in source §3; row-verified 2026-08-27 | Paper 3 §3.1 (Paper 2 proof owner; module formal_foundations) |
| CC-A002-014 | Support-provenance partition and directional support gap | definition | defined source object; row-verified 2026-08-27 | Paper 3 §4.3 (Paper 2 atlas entry) |
| CC-A002-038 | Support-saturated logistic stock limit ($O(\kappa)$ Grönwall bound) | theorem (approximation; partial reduction) | proof verified in source §9; row-verified 2026-08-27 | Paper 3 §2.6 |
| CC-A003-004 | Standing-stock culling mechanism type (routing never set by diagnostic labels) | definition | defined source object; row-verified 2026-08-28 | Paper 3 §2.5 |
| CC-A004-001 | H0 aggregate regional phosphorus balance (null comparator) | definition (registered template obligation) | defined source object; row-verified 2026-08-28 | Paper 3 §7.1 (identification with Paper 5) |
| CC-A004-002 | H1 multi-compartment regional/catchment model (displacement discipline) | definition (registered template obligation) | defined source object; row-verified 2026-08-28 | Paper 3 §7.1 |
| CC-A004-003 | H2 spatial trade-network/catchment model (falsification protocol) | definition (registered template obligation) | defined source object; row-verified 2026-08-28 | Paper 3 §7.1 |
| CC-A005-001 | H0 one-pool storage (null comparator) | definition (registered template obligation) | defined source object; row-verified 2026-08-28 | Paper 3 §7.2 |
| CC-A005-002 | H1 fast/slow two-pool storage with bidirectional leakage | definition (registered template obligation; falsifiable hypothesis, not an ontology) | defined source object; row-verified 2026-08-28 | Paper 3 §7.2 (two-pool gap: §7.3) |
| CC-A005-003 | H2 distributed or higher-dimensional groundwater model | definition (registered template obligation; one identification object with -001/-002) | defined source object; row-verified 2026-08-28 | Paper 3 §7.2 |
| CC-A005-004 | Scarcity-amplifying extraction (institutional hypothesis) | definition (registered template obligation; A003 H1 restatement) | defined source object; row-verified 2026-08-28 | Paper 3 §7.2 (named instantiation in Paper 4) |
| CC-A005-005 | Protective restraint/restoration (institutional hypothesis) | definition (registered template obligation; A003 H2 restatement) | defined source object; row-verified 2026-08-28 | Paper 3 §7.2 (named instantiation in Paper 4) |
| CC-A006-001 | Conditional hybrid moiety balance (jump interpretation + yield-routing obligation) | conditional theorem | proof verified in source §2; row-verified 2026-08-28 | Paper 3 §3.2 |
| CC-A010-002 | Local threshold-horizon bound (two-sided bracket) | theorem | proof verified in source §8; row-verified 2026-08-28 | Paper 3 §5.4 (canonical form Paper 2, CC-A002-040; predecessor statement) |
| CC-A010-003 | Typed conservation under the left-kernel condition | identity | verified in source §3.1; row-verified 2026-08-28 | Paper 3 §2.1 (CC-A002-008 family precedent) |
| CC-A010-008 | Six-state material cancellation (ghost-sink check; cancellation only) | identity | verified on the line in source §9.2; row-verified 2026-08-28 | Paper 3 §3.10 |
| CC-A012-005 | Decline pressure equals the positive stock-decline rate | identity | verified in source §2.1 Eq. (4); row-verified 2026-08-28 | Paper 3 §4.4 (dynamics family owned by Paper 4) |
| CC-A012-008 | MPF support-saturated logistic stock limit (+ companion simplex invariance) | theorem (approximation; interior-pointwise, not boundary-uniform) | proof verified in source §5.5; companion simplex theorem in the committed article of record; row-verified 2026-08-28 | Paper 3 §2.6 (Paper 4 owns the registered family) |
| CC-A013-002 | State-dependent feasible balance domain | definition | defined source object; row-verified 2026-08-28 | Paper 3 §4.2 |
| CC-A013-003 | Directional regenerative-support fraction and gap (attainment requires closedness) | definition | defined source object; row-verified 2026-08-28 | Paper 3 §4.3 |
| CC-A013-004 | Six-compartment incidence matrix, zero column sums ($\mathbf 1^{\top}S=0$) | identity | verified in source §3.2; row-verified 2026-08-28 | Paper 3 §2.3 |
| CC-A013-005 | Total represented material conserved | theorem | proof verified in source §3.3; row-verified 2026-08-28 | Paper 3 §3.5 |
| CC-A013-006 | Nonnegative cone forward invariant (donor boundary assumptions) | theorem | proof verified in source §3.3; row-verified 2026-08-28 | Paper 3 §3.6 |
| CC-A013-007 | Gross turnover/dependency intensity (false-implication record) | definition | defined source object; row-verified 2026-08-28 | Paper 3 §5.1 |
| CC-A013-008 | Local net-depletion ratio (frozen-rate; +$\infty$ convention) | definition | defined source object; row-verified 2026-08-28 | Paper 3 §5.2 |
| CC-A013-009 | Scenario-conditioned hitting time | definition | defined source object; row-verified 2026-08-28 | Paper 3 §5.3 |
| CC-A013-010 | G3P anomaly-persistence indices (statistical index, not $H^{\rm loc}$) | application record | values accepted at attested source status; submission supplement pending; row-verified 2026-08-28 | Paper 3 §5.5.1 (cited with CC-A018-017) |
| CC-A013-011 | Phosphate reserve-life ratio (arithmetic, not a forecast) | application record | source-specific empirical status check required; row-verified 2026-08-28 | Paper 3 §5.5.3 |
| CC-A013-012 | Fisheries removals-only threshold time $\Theta_F$ (outside the J/H/T hierarchy) | counterexample/limit | defined source object; row-verified 2026-08-28 | Paper 3 §5.5.4 (worked case in Paper 5) |
| CC-A018-002 | Donor-limited vector material ledger and conservation theorem | theorem (exact conservation identity) | proof verified in source §3.3; row-verified 2026-08-28 | Paper 3 §3.3 (approximation content separated to Paper 4, CC-A018-007) |
| CC-A018-003 | Nonnegative-orthant theorem (mass orthant, gated-law scope) | theorem (exact specialization) | proof verified in source §6; row-verified 2026-08-28 | Paper 3 §3.6 |
| CC-A018-004 | Exact specialization deficit identity | lemma (exact specialization) | proof verified in source §6; row-verified 2026-08-28 | Paper 3 §4.4 (the seam's shared object; MS-Native-1 restates) |
| CC-A018-005 | Exact triangular projection (the seam hand-off) | theorem (projectable reduction) | proof verified in source §6; row-verified 2026-08-28 | Paper 3 §8 (A002-036 family) |
| CC-A018-017 | Groundwater/phosphate/fisheries applied depletion-horizon tables | application record (descriptive diagnostics, not dynamical predictions) | source-specific empirical status check required; row-verified 2026-08-28 | Paper 3 §5.5.2 (cited with CC-A013-010) |
| CC-A019-001 | Primitive donor-limited exchange and mining laws (closed natural block) | definition (exact specialization) | defined source object; row-verified 2026-08-28 | Paper 3 §2.2 |
| CC-A019-002 | Natural-block mass identity (full-ledger conservation restored) | theorem | proof verified in source §3; row-verified 2026-08-28 | Paper 3 §3.4 |
| CC-A019-003 | Orthant invariance of the closed ledger | theorem | proof verified in source §3; row-verified 2026-08-28 | Paper 3 §3.6 |
| CC-A019-004 | No interior rest at positive effort (the source's title result) | theorem | proof verified in source §4; row-verified 2026-08-28 | Paper 3 §3.7 (routed here by the interface contract) |
| CC-A019-005 | Extinction–geochemical rest set | theorem | proof verified in source §4; row-verified 2026-08-28 | Paper 3 §3.8 |
| CC-A019-007 | Extraction integrability (finite donor budget) | theorem | proof verified in source §6; row-verified 2026-08-28 | Paper 3 §3.9 |
| CC-A019-008 | Frozen-donor limit distinction (intrinsic target retained) | completion distinction (approximation scope) | verified in source §7; row-verified 2026-08-28 | Paper 3 §8 |
| CC-A019-009 | Long-time finite-budget interpretation (order/budget bound; open slow-passage problem) | interpretation (approximation; conditional/open) | conditional/open; row-verified 2026-08-28 | Paper 3 §8 |
| CC-A024-002 | Brownian observed-drift surrogate (non-completion non-claim) | definition (exact specialization) | defined source object; row-verified 2026-08-28 | Paper 3 §6.2 |
| CC-A024-003 | Inverse-Gaussian groundwater first passage | theorem | proof verified in source §2; row-verified 2026-08-28 | Paper 3 §6.3 |
| CC-A024-004 | Mean, variance, zero-noise limit, and median results | corollary | proofs verified in source §2; row-verified 2026-08-28 | Paper 3 §6.4 |
| CC-A024-005 | Record-relative barrier and boundary-degenerate case (zero-cells discipline) | counterexample/limit | verified in source §3; row-verified 2026-08-28 | Paper 3 §6.5 |
| CC-A024-006 | Geometric-Brownian fisheries first passage (Itô drift) | theorem | proof verified in source §5; row-verified 2026-08-28 | Paper 3 §6.6 |
| CC-A024-007 | Constant-production phosphate passage time | application record | source-specific empirical status check required; row-verified 2026-08-28 | Paper 3 §6.7 (consistent with CC-A013-011) |
| CC-A024-009 | Seven explicit non-claims relative to the physical ledger | counterexample/limit | defined source object; row-verified 2026-08-28 | Paper 3 §6.8 |
| MS-Native-1 | The seam specialization identity ($\mathcal S_{1R}$: $D=qEN-R=-\dot N$, $\Lambda=[D]_+$) | identity | restatement of the ledger-to-dynamics interface contract; one-line proof here | Paper 3 §8 |
| MS-Native-2 | The non-reduction boundary (no exact dynamic reduction of `LEDGER-PRIM-CLOSED-v1` to `DYN-C4-WORKING`) | boundary statement (rejected mapping) | restatement of the contract's five reasons | Paper 3 §8 |
| MS-Native-3 | Two-pool identification requirements (geological geometry — aquitard depth and extent; multi-depth heads; pumping tests; tracer/isotope/water-age evidence; recharge estimates; prior ranges for $C_i$ and $\kappa_{fs}$; leakage may not absorb unexplained residuals) | registered identification requirements (template status) | source-declared template content (A005, §7); no constitutive content behind it; empirical adjudication with Paper 5 | Paper 3 §7.3 |
| MS-Native-4 | Sink obstructions independent of the stock (no assimilation $\delta\equiv0$; weak assimilation $\delta(K_{\max})<w(H_{\min})$) and the closed-unrecycled-ledger corollary (positive output floor forces an empty viability kernel) | remark (construction and corollary at source-declared status) | source remark (A001, Remark 6.1); mechanisms argued inline in the source | Paper 3 §2.4 |
| MS-Native-5 | $\psi$-assignment mass-routing discipline (evidence per channel; soil-zinc $0.85/0.25$ and pollinator $0.70/0.20$ illustrative pairs; $\approx1.5\times$ trough-depth factor; pre-recruit harvest vs prevented inflow; capital damage not a $\psi$ channel) | illustrative calibrated examples (logistic two-channel proxy) | source's illustrative $\psi$-mechanism table (A018, §5); no constitutive claim for the named domains | Paper 3 §2.5 |
| MS-Native-6 | Ten-state calibration underidentification (SSB and $F$ alone insufficient; juvenile abundance through governance timing required; minimum module set) with the four application prerequisites | registered application prerequisites | source-declared prerequisites and calibration remark (A010, §11) | Paper 3 §3.10 |
| MS-Native-7 | Variance-unclosed and $Q$-undefined negative witnesses of the ten-state admissibility audit | negative witnesses (audited status) | source admissibility stress test (A010); no unique autonomous DDE at the displayed scope | Paper 3 §3.10 |
| MS-Native-8 | Clark under-extraction remark (open access $S_{\rm OA}=c/(pq)$ infeasible under a floor $S_{\min}>S_{\rm OA}$; modified golden rule $g'(S_\rho)=\rho$; harvest tax shifts $S_{\rm OA}(t)=c/((p-t)q)$, not the floor) | remark (policy-instrument scope) | source remark (A001, Remark 12.1); extractor-side counterpart of the commons-obstruction family | Paper 3 §7.4 |

No status is promoted anywhere in this ledger; the manuscript-native rows are restatements of the interface contract (MS-Native-1, MS-Native-2) and source-declared remarks, requirements, examples, and witnesses carried at their declared statuses without concordance rows (MS-Native-3 through MS-Native-8), not new results, and the paper asserts nothing beyond the statuses above.

---

## 11 Provenance, reproducibility, and limits

**Sources.** The retained set derives from twelve sources of the programme corpus, each read in full by the scientific closure passes whose per-row findings govern the statuses above: A001 (`uploads/topdown.txt`; 1 row), A002 (`uploads/general_theory.txt`; 5), A003 (`uploads/Paper_V_Institutional_Feedback_and_Nonlinear_Transitions.txt`; 1), A004 (`uploads/Paper_IV_Phosphorus_Agriculture_Module.txt`; 3), A005 (`uploads/Paper_III_Groundwater_Module.txt`; 5), A006 (`uploads/Paper_II_Robust_Epistemic_Viability_V2.txt`; 1), A010 (`uploads/paper4_perspective.txt`; 3), A012 (`uploads/paper2_dynamics.txt`; 2), A013 (`uploads/paper1_accounting.txt`; 11), A018 (`uploads/manuscript.txt`; 5), A019 (`uploads/paper_II_closed_ledger.txt`; 8), A024 (`uploads/paper_VII_first_passage.txt`; 7). The committed articles of record for five of these sources are read alongside the uploads and carry the governing mathematics where they differ: `revised_articles/A019_closed_ledger_corrected.tex`, `A012_delay_dynamics_corrected.tex`, `A018_capital_liquidation_corrected.tex`, `A013_component_accounting_corrected.tex`, and `A024_first_passage_corrected.tex`. Row-level evidence: `research_program/canonical_concordance_A001_A025.csv` (409 rows; the 52 rows of §10 row-verified). Closure record: `research_program/concordance_row_closure_twenty_sources.md`. Seam: `research_program/A018_ledger_to_dynamics_interface_contract.md`. Architecture: `research_program/revised_optimal_publication_architecture_A001_A025.md`. Six manuscript-native entries (MS-Native-3 through MS-Native-8 of §10) carry source-declared content at its own statuses, with no concordance row: the A005 two-pool identification requirements at registered template status (§7.3); the A001 sink-obstruction construction and closed-unrecycled-ledger corollary at remark status (§2.4); the A018 illustrative $\psi$-assignment calibrated examples (§2.5); the A010 registered application prerequisites and the audited negative witnesses of the ten-state admissibility audit (§3.10); and the A001 Clark under-extraction remark at policy-instrument scope (§7.4).

**Proof handling.** Proofs marked *reproduced* are printed in full from the source. Proofs marked *verified present; summary* exist in full in the source (the closure passes verified proof presence and read them); the summaries are faithful to the source arguments, and camera-ready reproduces each proof verbatim. The canonical conservation and invariance family (§3.1) is stated with Paper 2 as proof owner; this paper reproduces no atlas proof beyond what its local statements require.

**Reproducibility.** The paper is analytical; it cites no computational artifact as evidence. The application records of §5.5 and §6.7 carry their source-stated evidentiary levels: the G3P values are accepted at attested source status with the submission-stage supplement pending (processing files, source extracts, and shared references not yet distributed); the phosphate reserve and production quantities carry source-specific empirical status with the source vintage supplied at submission; the fisheries proxy values are descriptive statistics of the cited assessment database. The unreproduced data pipelines remain on the programme's computational docket. The manuscript self-check (`papers/paper3_material_ledgers/verify_retained_rows.py`, stdlib-only, idempotent) verifies that every cited concordance identifier resolves, that the cited set equals the retained set plus the declared cross-references, and that the status ledger contains every retained row exactly once.

**Limits.** (i) The two-pool exact specialization of the groundwater template remains open; the admitted object is the one-pool affine approximation, and no two-pool model is claimed as established (§7.3). (ii) The phosphorus and groundwater rows are registered template obligations: no constitutive content exists behind their identification ladders, and their empirical adjudication belongs to Paper 5. (iii) The domain templates' competing-model ladders are one identification object per source and are stated as observation-governance content, not as ledger theorems. (iv) The non-reduction boundary of §8 is permanent mathematics, not a gap: the closed primitive ledger and the working retarded field are different completions, and no result of this paper or Paper 4 may be read as reducing one to the other. (v) The first-passage theorems of §6 concern declared stochastic surrogates, not the ledger: they do not compute the ledger's hitting time, do not conserve its mass, and carry the record-relative-barrier and non-claim disciplines verbatim. (vi) The G3P, phosphate, and fisheries application records are classified diagnostics at their stated evidentiary levels — statistical index, arithmetic ratio, removals-only pressure scale — and none is a calibrated early-warning system or a forecast. (vii) The frozen-donor transient-duration statement is an order/budget bound, and the slow-passage persistence question is open; the conditional theorem of §3.2 stays conditional, with its jump-interpretation and yield-routing obligations open per application. (viii) All concordance rows behind this paper (§10) are row-verified; no statement depends on a pending verification, and the 27 open rows of the concordance (the gated conditional-paper sources A021, A022, A023) are behind no claim of this paper.

---

## References (to be completed at camera-ready)

Physical-economy and material-flow-analysis literature for the accounting scaffold (stock–flow consistent material accounting; economy-wide material flow analysis; substance flow analysis). Stoichiometric and conservation-law bookkeeping of reaction-network theory as used by the sources. Compensability in composite indicators and non-compensatory MCDA methods, cited as motivation for the certification boundary. Critical-natural-capital and weak/strong sustainability debates as background to the support-provenance distinction. Reserve-life and resource-depletion metrics, and the USGS mineral commodity summaries for the phosphate quantities. The GRACE/G3P groundwater-storage product literature for the anomaly series. RAM Legacy stock-assessment database for the fisheries series. The inverse-Gaussian distribution and Brownian first-passage literature; geometric-Brownian hitting times and Itô calculus. Nagumo's tangency criterion and tangent-cone invariance; viability-theoretic formulations of constraint invariance. Programme-internal provenance documents as listed in §11 (Sources).

Full bibliographic data per the programme's shared bibliography register at camera-ready.
