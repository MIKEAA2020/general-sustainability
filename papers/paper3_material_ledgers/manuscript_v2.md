...
Every retained row of this manuscript, with claim status, evidence status (post-closure), and destination note. Review state: all 52 rows `row_verified` (dated scientific closure passes over full source reads, 2026-08-27/28) — content-level acceptance; no theorem status promoted; the cross-module interface contract remains open per row. The table's identifier column shows 54 codes because two of them (CC-A002-040, canonical form owned by Paper 2; CC-A018-007, approximation content owned by Paper 4) appear only as cross-citation pointers inside destination cells of other rows — they are cross-references, not retained rows, and the retained count is 52.
...
| CC-A010-002 | Local threshold-horizon bound (two-sided bracket) | theorem | proof verified in source §8; row-verified 2026-08-28 | Paper 3 §5.4 (canonical form Paper 2, CC-A002-040 — cross-reference, not a retained row; predecessor statement) |
...
| CC-A018-002 | Donor-limited vector material ledger and conservation theorem | theorem (exact conservation identity) | proof verified in source §3.3; row-verified 2026-08-28 | Paper 3 §3.3 (approximation content separated to Paper 4, CC-A018-007 — cross-reference, not a retained row) |
