...
*Second edition (batch-5 corrections, 2026-08-29): the lower-boundary passage of §6.1 restores the source's reconciling sentence and states the branch-resolved evidence (the collocated orbit persists through $\tau\ge5.5815$ with multiplier $0.964<1$; the $+1$-crossing signature is exhibited by the small branch); the loop-gain maximising frequency of §5.2 is corrected to $\omega\approx0.0589$; the branch mapping of Proposition 5.2's shifted delays is made explicit; the $\tau_+$ comparison of §6.5 names its variants; the memory gain of §7.1 is renamed $\gamma_m$ to free $g$ for the assimilation flux; the unverifiable numerical value of the tether threshold $\alpha$ is withdrawn in favour of the source's ratio form; the ledger's two cross-reference identifiers are marked; in-text citation hooks are attached at the method- and lineage-bearing points. The adjudication record is `BATCH5_JOINT_AUDIT_EVALUATION.md`; no theorem, registered number (other than as listed), claim status, or row count changes.*
...
**Proposition 5.3 (Protective sample-and-hold monodromy) [CC-A020-008 · proposition + sampled numerics, source-stated status].** Replacing the protective delayed feedback by sample-and-hold of period $T_r$ with one explicit Euler review step (the sample-and-hold review operator of computer-controlled systems: Åström and Wittenmark 1997), the monodromy is
...
All results in this section are numerical results at their declared status: computed outputs tied to registered equations, parameter sets, history classes, methods, tolerances, and finite domains, with basins restricted to the histories actually tested. The fold-status discipline governs every global statement: the events below are numerical continuation, multiplier, basin, and turning-region results; no Moore–Spence, Krawczyk, or nondegeneracy certificate and no continuous-delay fold proof is claimed (Appendix A.3; rigorous saddle-node-of-periodic-orbit results for DDEs exist for specific classes — Beretka and Vas 2020 — but none is claimed for the events below).
...
| MS-Native-1 | Single-resource deficit identity $qEN-R=-\dot N$ (seam restatement) | identity | one-line proof; restates the interface contract's shared object (ledger row is CC-A018-004, Paper 3 — cross-reference, not a retained row of this paper) | §9 |
| MS-Native-2 | Non-reduction boundary: no exact dynamic reduction of the closed ledger to the working C4 field | rejected-mapping statement | restates the interface contract's five mathematical reasons (no-rest theorem is CC-A019-004, Paper 3 — cross-reference, not a retained row of this paper) | §9 |
