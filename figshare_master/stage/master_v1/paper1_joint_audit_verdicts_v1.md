...
> **Naming note (2026-09-20).** This report was written while the
> SafeTransition manuscript carried the working label "paper 3"; it is in
> fact paper 1 of the series in extended, venue-agnostic form. All files
> have been renamed accordingly (`paper1_safetransition_ems_vN.*`,
> `paper1_supplementary_vN.md`); mentions of "paper 3" below are
> historical references to the audited artifacts under their former
> names. The venue-agnostic master implementing this report's verified
> fixes is `paper1_safetransition_master_v1.tex`.

**Scope.** One joint verification round over the three external-audit
streams, conducted against **paper 3 v6** (now `paper1_safetransition_ems_v6.tex`)
...
(`paper1_safetransition_ems_v7.tex`/`.pdf`, now 16 pp. with the
certificate support matrix), supplementary
**v4** (`paper1_supplementary_v4.md`, S1–S11), highlights **v7**;
venue-agnostic master `paper1_safetransition_master_v1` added in the
reorganization round (§10).
...
| 8 | §3.4 empty (S-A #20) | Scaling prose + Table 3 moved under the heading |
...
| 11 | Belief scaling row contradicts sweep (m=10 vs bound) (S-A §23, S-C 7.2) | Table 3: largest completed m = 12 (4,096 beliefs); overflow test m = 13 raises, no partial results |
...
| 14 | Performance reporting lacks reproducibility detail (S-A §26, S-C 7.6) | Table 3 caption: single run, no warm-up, wall-clock, Xeon 2.60 GHz, Linux x86-64, CPython 3.13, checker times incl. subprocess startup, tracemalloc overhead caveat; availability table carries the same platform row |
...
| 21 | Recourse example unspecified (S-A §39, S-C §12) | Table 5: full three-state system (states, observations, actions, successors) matching the package demo exactly |
...
| 24 | Module count "nine" vs ten rows (S-A §34, §35) | §3.1/Table 2: nine modules + CLI entry point + standalone checker; `benchmarks/` annotated "(directory, not a module)"; line counts updated with method (`wc -l`: 1,698/966/375/386) |
...
| 13.2 verdict-semantics table | **Done** — Table 1 in v7 §2.1 |
...
## 9. Rescan round (post-release systematic audit, same day)

A full re-read of the shipped artifacts (paper, supplementary, deposit)
surfaced and fixed the following; all re-verified in the rebuilt files.

**Recovered silent edit losses** (a failed multi-edit patch run had been
partially applied in an earlier round; post-write verification is now
mandatory):

- Abstract: the scoped v7 abstract (tube statuses, canonical JSON,
  six families, 323-bit denominators) had been silently reverted to the
  v6 text ("321-bit denominators", "five parameterized families",
  "sharing no code"); re-applied (149 words).
- Intro: scoped soundness/completeness sentence re-applied, extended
  with the audit's trusted-boundary sentence ("exact audit layer for
  declared models, not a substitute for environmental modelling,
  statistical validation, or ecological judgement").

**Remnants found by the systematic scan and fixed:**

| Remnant | Fix |
| --- | --- |
| QA artifacts table still said "46/46 pass", "under 4 s (laptop CPU)", "1,633/621 (+250, +291)" | 59/59, under 5 s (Xeon 2.60 GHz), 1,698/966 (+375, +386) |
| §2.1 datum sentence called tubes "an exact tube" before statuses were defined | status-neutral "a tube"; disturbance set stated finite |
| Table row shorthand "fibre criterion" | "safety-classification criterion" (consistent with the renamed criterion) |
| §5 said "test suite executed on continuous integration" while §6 disclaims CI | "a regression suite any user can re-run" |
| §5 duplicated the "Modular environmental simulators" frame for Pywr/airGR after the Raven/FloPy rewrite | rewritten as "Environmental simulators with executable regression suites" |
| Query count: §5 claimed twelve queries (q1–q12) but the SafeTransition record set is six (q1–q6); the twelve are the *companion paper's* searches elsewhere in the deposit | paper now says six and points to `novelty_searches/`; inherited v6/v3 inconsistency resolved against the actual records |
| Supplementary S8 cited "EMS submission, v3" | v7 |
| Supplementary/paper hard-coded table numbers after the new tables shifted numbering | realigned (recourse = Table 5; QA = Table 4; verdict semantics = Table 1; scaling = Table 3) |
| §6 supplementary range (S1–S8) | S1–S11 |

**Newly implemented (remaining audit items judged worth doing):**

- §5: exact-polyhedral computation family added with DOI-verified
  references (Avis 2000, reverse-search vertex enumeration,
  10.1007/978-3-0348-8438-9_9; Bruns & Ichim 2010, Normaliz,
  10.1016/j.jalgebra.2010.01.031) — closes S-A §42's family request.
- §6: SHA-256 release-manifest row; "executed on CPython 3.13, the only
  interpreter exercised" honesty note.
- Supplementary S7: notation rows for `Tube(a,d)` + statuses, `S_typ`/
  `S_phys`, `y⁻`/`z⁺ = R(y⁻)`.

**Still declined, with reasons (unchanged):** feasibility witnesses
(decided-not-witnessed is the contract; documented in §2.3/§3.3);
13.4 external-solver cross-check (stdlib-only trust boundary; the
in-package exact vertex-enumeration oracle serves the cross-check role);
six-column comparison table (families covered in prose; functional
table kept readable).

---

## 10. Reorganization round (user-directed, same day)

1. **Renaming.** The manuscript is paper 1 in extended, venue-agnostic
   form, not a third paper. All workspace and repository files renamed
   `paper3_*` → `paper1_*` (83 files in the latex tree, package
   `figs_p3/` → `figs_p1/` with all scripts and tests updated; the
   repository's unrelated `papers/paper1_general_theory` series made a
   collision-prone bare "paper1" label, so the manuscript files carry
   the disambiguated `paper1_safetransition_` scheme). Content-level
   historical mentions retain the former names, flagged by the naming
   note above.
2. **Venue-agnostic master manuscript created:**
   `paper1_safetransition_master_v1.tex`/`.pdf` — full v7 content
   (16 pp., all eight tables, certificate support matrix, lrslib/Normaliz
   positioning), converted to venue-neutral formatting (article class,
   numbered sections, standard natbib). Scope correction (user): the
   venue-agnostic *deposit* (`figshare_deposit/pkg/`, "Master Deposit"
   README) and the paper-1 v44-v46 addendum/readership-audit cycle
   already existed and were venue-agnostic; the missing piece was
   precisely the venue-agnostic master *manuscript* of the software
   description, all of whose prior versions (v1-v7) were
   EMS-formatted. The master is now archived in the deposit itself at
   `pkg/manuscripts/` (with the supplementary material and this
   report), so the figshare new version carries it; a copy remains
   staged at `figshare_deposit/stage/master_v1/`.
3. **Newly implemented from the audit's previously unread sections
   (§3.2, §4.1, §4.2, §10.1):** certificate-classes enumeration
   (§2.3: four serialized classes; two findings deliberately
   unserialized — re-derivable, no dedicated schema); certificate
   support matrix (Table 6, stated honestly per class incl. the
   witness-free feasibility verdict); `lrslib`/`Normaliz` named as the
   implementations of the exact-polyhedral family; supplementary S9
   documents the design difference from the audit's reference belief
   certificate (flat object + checker re-derivation instead of nested
   failure certificates — same machine-checkability, different
   packaging).
4. **Declined after reading (with reasons):** renaming library modules
   (`benchmark`→`witness_benchmark`, §6.1 option 3) — a breaking API
   change across the audited 1.3.0 tree for a cosmetic clarity gain;
   the docstring/table already disambiguate (`benchmarks/` is
   annotated "directory, not a module"). Nested per-action failure
   certificates inside the belief-failure object (§4.1 item 5's
   recursive form) — the checker's independent re-derivation achieves
   the same verifiability with a flat schema; documented in S9.
   Splitting the deposit into two DOIs (§11.2) — deferred to the user;
   the single-DOI master with explicit role separation is consistent
   until then.
