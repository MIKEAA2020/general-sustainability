# Sentence-level normalised diff scan — paperE3_edwards_forecast_ladder (final v13)

Universe: body sentences/segments of the final v13 vs the five preceding versions (v8 … v12). A segment counts as dropped only if absent from the *whole* final file. Table rows and display-math lines are atomic. The per-version `*Version log*` paragraph is excluded from the universe (meta) and quoted per transition.

Final v13 segment counts: para 326, listitem 9, heading 19, display 1, tablerow 71
...
3. **[para @v8 L23 · ## 1. Introduction]** The three hydrological objects are worth separating, since they are read off the same number.
   - paired replacement (r=0.93): Three hydrological objects are worth separating, because they are read off the same number.
4. **[para @v8 L23 · ## 1. Introduction]** The head record is observed at an access point, the index well; that head indexes a store of water in the aquifer, which is the resource; and the store is replenished and drawn by fluxes, which are the flow.
   - paired replacement (r=0.52): That head indexes a store of water in the aquifer, which is the resource.
5. **[para @v8 L23 · ## 1. Introduction]** The one-pool map closes this loop approximately.
