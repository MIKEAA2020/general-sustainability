#!/usr/bin/env python3
"""Wave-17 / Task 90, part 2: build paper4_delay_dynamics_v34.tex/.pdf from
paper4_delay_dynamics_v34.md (authored by wave17/make_v34.py).

The wave-16 pipeline with every fail-loud check inherited (pandoc; title/
abstract/figure processing; Unicode-to-macros; pure-ASCII body; emphasis
mis-parse symptom; figure counts; numeric-token multiset EXACTLY equal
between markdown and LaTeX body; no markdown word lost; declaration
...
idempotent rebuild), extended with the Task-89 plan's v34 gates:

(a) md-level math-subset vs v33: every math span of v34 is byte-identical
    to a v33 span (EMPTY whitelist --- v34 adds no mathematics);
(b) the v34 content needles = the full wave-16 needle battery (v33 style
    markers + frozen v32 scientific claims, all of which must survive)
    PLUS the device sentinels of the humanizing revision PLUS the
    dropped-caveat regression gates (mesh-range caveat, not-a-calibration
    clause, scheme-dependence caveat, H1-H5 scaffolding, provisional
    saddle-node qualifier, reversed-gain loop-gain attribution);
(c) the machine-checkable rejection list (Task-89 joint assessment Part
    III.D): zero hits for both gemini reference fabrications, the
    archetype vocabulary, the invented table cells, the M3-LC channel
    inversion notation, the title-level overclaims, and americanised
    spellings of the paper's register words --- scanned on the markdown
    AND on the final LaTeX (en/em dashes in their pandoc '--'/'---'
    forms);
(d) 'if and only if' occurs exactly 5 times (grok's four fixes plus
    Theorem 4.1's original long form);
(e) tex-level numeric discipline vs the v33 tex: every frozen token of
    the v33 LaTeX body survives in v34 (superset) AND v34 introduces no
    new token VALUE (longtable column-spec digits stripped from both
    bodies before comparison --- they are pandoc formatting artifacts,
    not content).

Outputs: arena agent 1/paper rewrites/latex/paper4_delay_dynamics_v34.tex
and .pdf.  v33 tex/pdf remain untouched (version discipline).  Run three
...
PAPERS = ["paper4_delay_dynamics_v34"]
...
V33_STYLE_NEEDLES = [
    ...
    "the main finding is that a delay of intermediate length stabilises the equilibrium",
    "Two Hopf crossings",
...
V32_FROZEN_NEEDLES = [
    ...
    "the northern cod record runs from annual assessment with incremental response",
...
RENUMBER_TOKENS: set[str] = set()  # no renumbering in v34: same skeleton
...
def header_comment(paper: str) -> str:
    return (
        ...
        f"wave17/build_latex_v17.py\n"
        f"% (batch 7 (audits of agent arena 1 paper rewrites)/wave17; md "
        f"authored by\n"
        f"% wave17/make_v34.py from paper4_delay_dynamics_v33.md via 37 "
        f"verbatim-anchored\n"
        f"% surgical edits).\n"
        f"% Task 90, owner-directed: the implemented humanizing revision of "
        f"v33 per the\n"
        f"% Task-89 joint assessment/plan (humanizing audits/JOINT_"
        f"HUMANIZING_ASSESSMENT_AND_PLAN.md):\n"
        f"% v33's content spine frozen (all theorems, hypotheses, numbers, "
        f"intervals,\n"
        f"% tables, references and certification-tier statements "
        f"machine-verified as\n"
        f"% surviving verbatim), restyled with gemini's device kit (named "
        f"concepts,\n"
        f"% bulleted/numbered contrasts, the five-regime summary table, "
        f"warning\n"
        f"% call-outs, the three-tier certification list, the extended "
        f"translation\n"
        f"% table, What-this-says readings) and grok's micro-fixes (the four\n"
        f"% iff expansions, plain section openers, protective-channel "
        f"artefact\n"
        f"% attribution), with a\n"
        f"% machine-checked rejection list for every verified audit "
        f"fabrication.\n"
...
def build_paper(paper: str) -> dict:
    ...
    # 1. fail-loud math check at md level (every span verbatim in v33;
    #    empty whitelist: v34 adds no mathematics of its own)
    t33 = (PR / "paper4_delay_dynamics_v33.md").read_text(encoding="utf-8")
    spans = re.findall(r"\$\$.*?\$\$|\$[^\$\n]+\$", md, flags=re.S)
    missing = [s for s in spans if s not in t33]
    assert not missing, f"{paper}: math spans missing from v33: {missing[:5]}"
    ...
    for needle in V34_NEEDLES[paper]:
        assert needle in final_flat, (
            f"{paper}: v34 content needle missing: {needle!r}"
        )
    ...
    if texfile.exists():
        ...
        if "by wave17/build_latex_v17.py" in old:
    ...
    # 6. tex-level numeric discipline vs the v33 tex: every frozen token
    #    survives (superset) and no new token VALUE appears.  Longtable
    #    column-spec digits are pandoc formatting artifacts --- stripped
    #    from both bodies before comparison.
    v33tex = (LATEX / "paper4_delay_dynamics_v33.tex").read_text(encoding="utf-8")
    v33body = v33tex.split(MAKETITLE_SEP, 1)[1]
    v34body = final.split(MAKETITLE_SEP, 1)[1]
    v33body = LONGTABLE_SPEC.sub(" ", v33body)
    v34body = LONGTABLE_SPEC.sub(" ", v34body)
    t33n = tokens_numeric(strip_cmds(v33body))
    t34n = tokens_numeric(strip_cmds(v34body))
    lost_t = {t: c for t, c in t33n.items() if t34n[t] < c and t not in RENUMBER_TOKENS}
    assert not lost_t, f"{paper}: frozen v33 tex numeric tokens lost/reduced: {lost_t}"
    new_t = {t for t in t34n if t not in t33n}
