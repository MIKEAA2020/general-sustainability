#!/usr/bin/env python3
"""Wave-14 / Task 86, part 2: build paper4_delay_dynamics_v31.tex/.pdf from
paper4_delay_dynamics_v31.md (created by wave14/make_v31.py).

The wave-13 pipeline with every fail-loud check inherited (pure-ASCII body,
...
idempotent rebuild).  The wave-13 P4 wave-11 body-identity check is dropped
(v31 legitimately edits the md body); in its place:

  - v31 content needles (the new abstract lead, breadth keywords, the
    intro generality paragraph, the 5.1 and Section 8 glosses, the new
    Discussion 11.3 with the management-translation table, the conclusion
    breadth sentences, the renumbered open-problem cross-reference);
  - tex-level numeric discipline vs the v30 tex: every frozen scientific
    token of the v30 LaTeX survives verbatim in v31 (section renumber
    11.3->11.7 exempted, exactly as verified at md level).

Outputs: arena agent 1/paper rewrites/latex/paper4_delay_dynamics_v31.tex
and .pdf.  v30 tex/pdf remain untouched (version discipline).  Run twice
...
PAPERS = ["paper4_delay_dynamics_v31"]

# --- front matter (identical to wave-13) --------------------------------------
AUTHOR = (
...
# --- back matter (identical to wave-13) ---------------------------------------
AI_TEXT = (
...
# --- v31 content needles (the journal-fit joint edits) -------------------------
V31_NEEDLES = {
    "paper4_delay_dynamics_v31": [
        "Delayed feedback is a classic source of instability in "
        "renewable-resource systems",                     # abstract lead
        "renewable resource management",                   # keywords
        "The institutional loop is not fisheries-specific",  # intro para
        "Terminology for the broad reader",                # 5.1 gloss
        "one-period linear return map of the reviewed loop",  # Sec 8 gloss
        "11.3 Generality: what carries beyond the analysed class",  # new 11.3
        "Management reading",                              # translation table
        "Beyond the declared class, the coordinates themselves generalise",  # concl
        "the generality of the institutional-loop coordinates",  # organization
        "(Section 11.4's first stated open task",          # re-pointed xref
        "stock-agnostic",                                  # breadth wording
        "coupled human--natural system",                   # pandoc en-dash form
    ],
}
...
def header_comment(paper: str) -> str:
    return (
        f"% LaTeX source generated from {paper}.md by wave14/build_latex_v14.py "
        f"(batch 7 (audits of agent arena 1 paper rewrites)/wave14).\n"
        f"% Wave-13 pipeline with the wave-14 journal-fit edits: broad-lead\n"
        f"% abstract, breadth keywords, intro generality paragraph, first-use\n"
        f"% glosses, Discussion 11.3 'Generality' with the management-language\n"
        f"% translation table, conclusion breadth sentences.\n"
...
def build_paper(paper: str) -> dict:
    ...
    for needle in V31_NEEDLES[paper]:
        assert needle in final_flat, (
            f"{paper}: v31 content needle missing: {needle!r}"
        )
    ...
    if texfile.exists():
        ...
        if "by wave14/build_latex_v14.py" in old:
    ...
    # 6. tex-level numeric discipline vs the v30 tex (frozen values survive);
    #    run after the write so failures are debuggable on disk. The v30
    #    header comment's own version/wave tokens (30/13/9/11 in the header
    #    lines) are excluded with the v30 header stripped first.
    v30tex = (LATEX / "paper4_delay_dynamics_v30.tex").read_text(encoding="utf-8")
    v30body = v30tex.split(MAKETITLE_SEP, 1)[1]
    v31body = final.split(MAKETITLE_SEP, 1)[1]
    t30 = tokens_numeric(strip_cmds(v30body))
    t31 = tokens_numeric(strip_cmds(v31body))
    lost_t = {t: c for t, c in t30.items() if t31[t] < c and t not in RENUMBER_TOKENS}
