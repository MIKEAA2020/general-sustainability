#!/usr/bin/env python3
"""Wave-21 / Task 97, part 3: structural PDF verification of
paper4_delay_dynamics_v38.pdf (PyMuPDF), plus page renders for VLM
verification.

Checks (all fail-loud): the wave-20 battery inherited and retargeted,
plus the wave-21 gates:
...
11. Renders pages (1, the Section 1.1 lineage page, the Section 7
    ...
    supplementary-pointer page, the final declarations) to wave21/logs/
    for VLM verification.
"""
...
PDF = LATEX / "paper4_delay_dynamics_v38.pdf"
...
def main() -> int:
    ...
    assert "paper4_supplementary_v7.md" in nospace, (
        "aligned supplementary pointer (v7) missing from rendered text"
    )
    assert "paper4_supplementary_v6" not in nospace, (
        "stale supplementary pointer (v6) present in rendered text"
    )
    nospace_b = re.sub(r"\s+", "", full_b)
    assert "paper4_supplementary_v6" not in nospace_b, (
        "stale supplementary pointer (v6) present in rendered text (b)"
    )
    ...
    print(
        ...
        f"the v7 supplementary pointer rendered with the stale v6 "
