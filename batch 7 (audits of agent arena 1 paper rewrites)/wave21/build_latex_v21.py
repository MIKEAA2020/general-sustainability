#!/usr/bin/env python3
"""Wave-21 / Task 97, part 2: build paper4_delay_dynamics_v38.tex/.pdf from
paper4_delay_dynamics_v38.md (authored by wave21/make_v38.py).

The wave-19/20 pipeline with every fail-loud check inherited, retargeted
to v38 (the reference round + artifact-class removal round): Adamson &
Hilker (2020) and Hocherman, Trop & Ghermandi (2025) added (references
36 -> 38; the Section 1.1 lineage link, the Section 11.2 echo, and the
three governance-lag citations); the diary/meta/self-referential
artifacts removed; the aligned supplement published as
paper4_supplementary_v7.md and the paper's pointer updated.

(a) md level: math-span multiset EQUALITY vs v37 (nothing moved); the
cross-reference resolver (every Section-construct number resolves to an
existing heading); the section-reference multiset == v37 minus the one
removed 'Sections 8.3 and 10.4' construct; the label counts unchanged;
content numerics == v37 + the declared reference/citation tokens + the
filename version digit (+7, -6);
...
sentinels PLUS the dropped-caveat regression gates PLUS the new
reference/citation needles PLUS the aligned supplementary pointer
(tex-escaped form) with the stale pointer absent;
(c) the rejection scanner (44 banned strings: the 35 inherited + 9
artifact-class regression gates) --- zero hits on markdown AND final
LaTeX;
(d) 'if and only if' still exactly 5 times;
(e) tex-level numeric discipline vs the v37 tex by the same stripped
comparison (delta == the declared reference/citation tokens + the
filename version digit), the tex-level resolver, and the section-order
gate (7 < 8 < 9).

Outputs: arena agent 1/paper rewrites/latex/paper4_delay_dynamics_v38.tex
and .pdf.  v37 tex/pdf remain untouched (version discipline).  Run three
...
PAPERS = ["paper4_delay_dynamics_v38"]
PARENT_MD = "paper4_delay_dynamics_v37"
PARENT_TEX = "paper4_delay_dynamics_v37"
...
def header_comment(paper: str) -> str:
    return (
        ...
        f"wave21/build_latex_v21.py\n"
        f"% (batch 7 (audits of agent arena 1 paper rewrites)/wave21; md "
        f"authored by\n"
        f"% wave21/make_v38.py from paper4_delay_dynamics_v37.md).\n"
        f"% Task 97, owner-directed: the reference round + the "
        f"process-language removal.\n"
        f"% References 36 -> 38: Adamson & Hilker (2020, Theoretical "
        f"Ecology 13,\n"
        f"% 425-434, doi 10.1007/s12080-020-00462-x) cited at the "
        f"Section 1.1\n"
        f"% lineage paragraph and echoed at Section 11.2; Hocherman, Trop "
        f"& Ghermandi\n"
        f"% (2025, Ambio 54(12), 2042-2059, doi 10.1007/s13280-025-02211-y) "
        f"cited at\n"
        f"% the three governance-lag sites (9.3/11.4/11.7), resolving the "
        f"supplement's\n"
        f"% dangling S4 citation.  The diary/meta/self-referential "
        f"process\n"
        f"% language removed (the Section 7 opening announcement "
        f"sentence,\n"
        f"% the editing-history relocation clauses, the change-of-state "
        f"'now's); the aligned\n"
        f"% supplement is published as paper4_supplementary_v7.md and "
        f"the paper's\n"
        f"% pointer updated.  Everything else is frozen and "
        f"machine-verified (all\n"
        f"% 1,473 math spans byte-identical; the 258-word abstract, "
        f"title,\n"
        f"% declarations, Data availability and figure byte-identical).\n"
...
def build_paper(paper: str) -> dict:
    ...
    # this counter's regex counts only 'Section(s) N' prefixed numbers,
    # so the removed 'Sections 8.3 and 10.4' construct contributes its
    # '8.3' member alone here (the review script's construct-aware
    # counter additionally sees the '10.4' list member)
    assert r36 == r35 - Counter({"8.3": 1}), (
    ...
    assert "paper4\\_supplementary\\_v7.md" in final_flat, (
        f"{paper}: supplementary pointer missing in tex"
    )
    assert "paper4\\_supplementary\\_v6.md" not in final_flat, (
    ...
    if texfile.exists():
        ...
        if "by wave21/build_latex_v21.py" in old:
    ...
    # 6. tex-level numeric discipline vs the v37 tex: the declared
    #    reference/citation tokens + the filename version digit
