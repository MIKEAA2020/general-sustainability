#!/usr/bin/env python3
"""Wave-19 / Task 93, part 2: build paper4_delay_dynamics_v36.tex/.pdf from
paper4_delay_dynamics_v36.md (authored by wave19/make_v36.py).

The wave-18 pipeline with every fail-loud check inherited, retargeted to
v36 (owner-directed restructuring round: the convergent batch-8 spine ---
review interval promoted to Section 7, global numerics to Section 8, the
maturation-delay analogue to Section 9 --- plus the E1-E4 consolidations
of the Task-92 plan):

(a) md level: math-span multiset EQUALITY vs v35 (empty whitelist; the
    rotation moves blocks, adds and removes nothing); the cross-reference
    resolver (every Section-construct number resolves to an existing
    heading); the section-reference multiset transform (bijective map
    7->9, 8->7, 9->8, 7.k->9.k, 9.k->8.k plus the declared E-deltas);
    the label renumbers (Theorem/Proposition/Remark 8.1 -> 7.1);
    content numerics == v35 + {1992, 2024};
(b) the full inherited needle battery (v33 style markers + frozen v32
    scientific claims + v34/v35 device sentinels with the label-renumber
    updates) PLUS the rotation and E1-E4 sentinels PLUS the
    dropped-caveat regression gates;
(c) the rejection scanner extended with the batch-8 banned coinages
    (Sign Separation Theorem, viability bleed, phase-stabilisation trap,
    administrative hunting, basin lock-in, the eigenvalue maxims) ---
    zero hits on markdown AND final LaTeX;
(d) 'if and only if' still exactly 5 times;
(e) tex-level numeric discipline vs the v35 tex by the same stripped
    comparison (section refs, headings and theorem labels stripped from
    both bodies; delta == {1992: +1, 2024: +1}), the tex-level resolver,
    and the section-order gate (7 < 8 < 9).

Outputs: arena agent 1/paper rewrites/latex/paper4_delay_dynamics_v36.tex
and .pdf.  v35 tex/pdf remain untouched (version discipline).  Run three
...
PAPERS = ["paper4_delay_dynamics_v36"]
PARENT_MD = "paper4_delay_dynamics_v35"
PARENT_TEX = "paper4_delay_dynamics_v35"
...
def header_comment(paper: str) -> str:
    return (
        ...
        f"wave19/build_latex_v19.py\n"
        f"% (batch 7 (audits of agent arena 1 paper rewrites)/wave19; md "
        f"authored by\n"
        f"% wave19/make_v36.py from paper4_delay_dynamics_v35.md via the "
        f"owner-commissioned\n"
        f"% restructuring of the batch-8 joint assessment).\n"
        f"% Task 93, owner-directed: (1) the convergent restructuring "
        f"spine --- the\n"
        f"% Review Interval promoted to Section 7, Global Numerics to "
        f"Section 8, the\n"
        f"% Delayed-Recruitment (Maturation-Delay) System to Section 9 "
        f"--- with every\n"
        f"% cross-reference renumbered by a bijective map and the "
        f"promoted section's\n"
        f"% theorem, proposition and remark labels following their host\n"
        f"% (from 8.1 to 7.1); the Organization\n"
        f"% paragraph rewritten for the new argument arc; (2) the E1 "
        f"cross-reference\n"
        f"% repairs (4.2->5.2, 4.4->5.4, 6.2->6.2-and-6.4) plus the "
        f"permanent\n"
        f"% cross-reference resolver; (3) the E2/E3/E4 consolidations "
        f"(delay stages,\n"
        f"% parameter-box open task, identification layer); (4) the "
        f"content spine\n"
        f"% otherwise frozen and machine-verified (all 1,473 math spans "
        f"byte-identical;\n"
        f"% content numerics == v35 + 1992 + 2024 only; title, abstract, "
        f"references,\n"
        f"% declarations, figure byte-identical).\n"
...
def build_paper(paper: str) -> dict:
    ...
    # 1. md-level math-span multiset EQUALITY vs v35 (empty whitelist)
    spans = re.findall(r"\$\$.*?\$\$|\$[^\$\n]+\$", md, flags=re.S)
    spans35 = re.findall(r"\$\$.*?\$\$|\$[^\$\n]+\$", t35, flags=re.S)
    assert Counter(spans) == Counter(spans35), (
        f"{paper}: math-span multiset differs from v35"
    )
    ...
    assert (n36s - n35s) == Counter({"1992": 1, "2024": 1}), (
        f"{paper}: md content-numeric delta != 1992+2024: "
        f"{dict(n36s - n35s)} / lost {dict(n35s - n36s)}"
    )
    ...
    for k, d in (("4.2", -1), ("4.4", -1), ("5.2", 1), ("5.4", 1),
        ...
        expected[k] += d
    assert r36 == expected, (
        f"{paper}: md section-ref multiset mismatch "
    ...
    if texfile.exists():
        ...
        if "by wave19/build_latex_v19.py" in old:
    ...
    # 6. tex-level numeric discipline vs the v35 tex: identical stripped
    #    content numerics + {1992, 2024}
    ...
    assert (t36n - t35n) == Counter({"1992": 1, "2024": 1}) and not (
        t35n - t36n
    ), (
