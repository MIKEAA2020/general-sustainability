#!/usr/bin/env python3
"""Wave-16 / Task 88, part 2: build paper4_delay_dynamics_v33.tex/.pdf from
paper4_delay_dynamics_v33.md (authored by wave16/make_v33.py).

The wave-15 pipeline with every fail-loud check inherited (pure-ASCII body,
...
idempotent rebuild).  The v32 content needles are replaced by the v33
content needles (the reader-oriented rewrite markers plus the frozen
scientific claims), and the tex-level numeric discipline is taken against
the v32 tex: every frozen scientific token of the v32 LaTeX body survives
verbatim in v33 (no renumber exemptions --- the section skeleton is
unchanged).  A new fail-loud check verifies at markdown level that every
math span of v33 is byte-identical to a v32 math span (single documented
whitelist: the corrected '$\\rho(M_{\\mathrm{ZOH}})$' span; v32 carried the
double-backslash typo in prose).

Outputs: arena agent 1/paper rewrites/latex/paper4_delay_dynamics_v33.tex
and .pdf.  v32 tex/pdf remain untouched (version discipline).  Run twice
...
PAPERS = ["paper4_delay_dynamics_v33"]

# --- front matter (identical to wave-13/14/15) ---------------------------------
AUTHOR = (
...
# --- back matter (identical to wave-13/14/15) ----------------------------------
AI_TEXT = (
...
# --- v33 content needles -------------------------------------------------------
# (a) rewrite markers: sentences that exist only in the reader-oriented v33
V33_STYLE_NEEDLES = [
    ...
    "Two Hopf crossings",  # (with) parameter values at which a pair of eigenvalues crosses
...
V33_NEEDLES = {"paper4_delay_dynamics_v33": V33_STYLE_NEEDLES + V32_FROZEN_NEEDLES}

RENUMBER_TOKENS: set[str] = set()  # no renumbering in v33: same skeleton

# the one deliberate math correction inherited from make_v33.py
...
def header_comment(paper: str) -> str:
    return (
        ...
        f"wave16/build_latex_v16.py\n"
        f"% (batch 7 (audits of agent arena 1 paper rewrites)/wave16; md "
        f"authored by\n"
        f"% wave16/make_v33.py from parts_v33 + verbatim v32 blocks).\n"
        f"% Task 88, owner-directed: the reader-oriented formal rewrite of "
        f"v32 ---\n"
        f"% landmark-paper writing conventions (Fridman 2010; Richard 2003;\n"
        f"% Hetel et al. 2017; May 1973; Hutchings and Myers 1994): short\n"
        f"% declarative sentences, terms defined in plain words at first "
        f"use,\n"
        f"% results before machinery, explicit signposting; all equations,\n"
        f"% tables, numbers, hypotheses and certification-tier statements\n"
        f"% carried over unchanged (every math span byte-identical to v32;\n"
        f"% every frozen v32 numeric token survives; the "
        + "'\\rho(M_{\\mathrm{ZOH}})'\n"
        + f"% double-backslash typo of v32's Section 8 prose corrected).\n"
...
def build_paper(paper: str) -> dict:
    ...
    # 1. fail-loud math check at md level (every span verbatim in v32)
    t32 = (PR / "paper4_delay_dynamics_v32.md").read_text(encoding="utf-8")
    spans = re.findall(r"\$\$.*?\$\$|\$[^\$\n]+\$", md, flags=re.S)
    missing = [s for s in spans if s not in t32 and s not in MATH_WHITELIST]
    assert not missing, f"{paper}: math spans missing from v32: {missing[:5]}"
    ...
    for needle in V33_NEEDLES[paper]:
        assert needle in final_flat, (
            f"{paper}: v33 content needle missing: {needle!r}"
        )
    ...
    if texfile.exists():
        ...
        if "by wave16/build_latex_v16.py" in old:
    ...
    # 6. tex-level numeric discipline vs the v32 tex (frozen values survive);
    #    run after the write so failures are debuggable on disk. The v32
    #    header comment's own version/wave tokens are excluded with the v32
    #    header stripped first.
    v32tex = (LATEX / "paper4_delay_dynamics_v32.tex").read_text(encoding="utf-8")
    v32body = v32tex.split(MAKETITLE_SEP, 1)[1]
    v33body = final.split(MAKETITLE_SEP, 1)[1]
    t32n = tokens_numeric(strip_cmds(v32body))
    t33n = tokens_numeric(strip_cmds(v33body))
    lost_t = {t: c for t, c in t32n.items() if t33n[t] < c and t not in RENUMBER_TOKENS}
    assert not lost_t, f"{paper}: frozen v32 tex numeric tokens lost/reduced: {lost_t}"
