#!/usr/bin/env python3
"""Wave-22 / Task 98, part 3: structural PDF verification of
paper4_delay_dynamics_v39.pdf (PyMuPDF), plus page renders for VLM
verification.

Checks (all fail-loud): the wave-20/21 battery inherited and retargeted,
plus the wave-22 retitle gates:
1. Page 1: the NEW TITLE (rendered verbatim), byline (name / affiliation
   / clickable ORCID / clickable email), pinned date, the abstract's
   device sentences, keywords; the old title's fragments ABSENT from the
   whole rendered text (retitle regression gates).
...
7. The wave-21 reference-round needles: the Section 1.1 lineage link
   and the Section 11.2 echo (Adamson and Hilker), the three
   governance-lag citations (Hocherman, Trop, and Ghermandi), both new
   reference entries with their DOIs.
8. The wave-21 supplementary-pointer gate: the v7 filename renders and
   the stale v6 pointer is absent from the rendered text.
9. Declarations structure: Data availability + Declaration of competing
...
10. Figure 1 present exactly once; rejection scan on the rendered text
    (the wave-20 list + the artifact-class regression gates: the
    removed meta-announcement and editing-history phrases must be
    absent from the rendered text).
11. Renders pages (1, the Section 1.1 lineage page, the Section 7
    opening, the Section 9 opening, the ecological reading, the regime
    table, the references page with the two new entries, the
    supplementary-pointer page, the final declarations) to wave22/logs/
    for VLM verification.  This round's VLM focus: page 1 (the new title
    block, byline, abstract opening) and the final declarations page.
"""
...
PDF = LATEX / "paper4_delay_dynamics_v39.pdf"
...
def main() -> int:
    ...
    print(
        ...
        f"retitle gates), AI declaration last, the NEW TITLE rendered "
        f"verbatim on page 1 with the old title absent everywhere, "
        f"the v8 supplementary pointer rendered with the stale v7 "
        f"pointer absent; the A&H lineage on pdf page "
        f"{lineage_page + 1}, Section 7 opens on pdf page {sec7_page + 1}, "
        f"Section 9 on pdf page {sec9_page + 1}, regime table on pdf page "
        f"{regime_page + 1}, ecological reading on pdf page "
        f"{eco_page + 1}, the Adamson reference entry on pdf page "
