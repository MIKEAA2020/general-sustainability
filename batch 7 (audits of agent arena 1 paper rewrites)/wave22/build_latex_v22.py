#!/usr/bin/env python3
"""Wave-22 / Task 98, part 2: build paper4_delay_dynamics_v39.tex/.pdf from
paper4_delay_dynamics_v39.md (authored by wave22/make_v39.py).

The wave-19/20/21 pipeline with every fail-loud check inherited,
retargeted to v39 (the retitle round): the owner-delegated title
...
replaces the v38 title; the accompanying file, which carries the title
in three places, is republished as paper4_supplementary_v8.md and the
paper's pointer updated (v7 -> v8).  NOTHING else changes.
...
Outputs: arena agent 1/paper rewrites/latex/paper4_delay_dynamics_v39.tex
and .pdf.  v38 tex/pdf remain untouched (version discipline).  Run three
...
PAPERS = ["paper4_delay_dynamics_v39"]
PARENT_MD = "paper4_delay_dynamics_v38"
PARENT_TEX = "paper4_delay_dynamics_v38"
...
NUM_ADD = Counter({"8": 1})   # the supplementary pointer's version digit
NUM_LOST = Counter({"7": 1})
...
def header_comment(paper: str) -> str:
    return (
        ...
        f"wave22/build_latex_v22.py\n"
        f"% (batch 7 (audits of agent arena 1 paper rewrites)/wave22; md "
        f"authored by\n"
        f"% wave22/make_v39.py from paper4_delay_dynamics_v38.md).\n"
        f"% Task 98, owner-directed: the retitle round.  The "
        f"owner-delegated title\n"
        f"% choice for Theoretical Ecology:\n"
        f"%   Governance delay and the stability of harvested stocks: "
        f"mobilising and\n"
        f"%   protective feedback rules, and the review interval as a "
        f"design parameter\n"
        f"% replaces the v38 title (its regime-change headline "
        f"% dropped: the large-amplitude attractor is unverified, the "
        f"% dropped: the large-amplitude attractor is unverified, the "
        f"folds are\n"
        f"% discrete-collocation-certified - no overclaim in the title); "
        f"the\n"
        f"% accompanying file carries the title in three places and is "
        f"republished\n"
        f"% as paper4_supplementary_v8.md with the paper's pointer "
        f"updated.\n"
...
def build_paper(paper: str) -> dict:
    ...
    assert Counter(spans) == Counter(spans35), (
        f"{paper}: math-span multiset differs from v38"
    )
    ...
    if texfile.exists():
        ...
        if "by wave22/build_latex_v22.py" in old:
