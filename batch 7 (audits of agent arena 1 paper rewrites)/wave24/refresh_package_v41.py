#!/usr/bin/env python3
"""Wave-24 / Task 102, part 6: refresh the supplementary submission package
in place --- paper4_supplementary_v8.zip --- so its README names the
manuscript it now accompanies (v41, the DOI + abstract-honesty round).

Why in place: the supplement DOCUMENT is unchanged this round (no
supplement file was touched by the v41 round), so no supplement version
...
v40 -> v41.  The built-asset precedent (waves 11/12, Tasks 98/99)
applies: git history preserves the previous zip bytes, and this script
records BOTH sha256 values (old and new) fail-loud.

What changes inside the package:
  1. README.md --- the Manuscript line (v40 -> v41 filename + md5s +
     the one-line round description); the "Package built" paragraph
     gains the v41 refresh sentence.
  2. verification/packaging_refresh_2026-09-19_v41.md --- NEW: the
...
What does NOT change (verified byte-for-byte, file by file):
  ...
  supplementary document itself, the prior verification notes (the
  v7/v8/v40 refresh notes, the re-execution record + logs).
"""
...
OLD_V40_MD5 = "931bf92e8f6eeed77a08fcffc5c78daa"
OLD_V40_TEX_MD5 = "0ce11af0262a80b15a2a24bf1695254f"
OLD_V40_PDF_MD5 = "3d7f3c04680286581fba97c7d4bf1d28"
NEW_V41_MD5 = "10cafbbdbce794954bdab4449d89e3d3"
NEW_V41_TEX_MD5 = "9690145302dd5ffe790a5cee799de0b7"
NEW_V41_PDF_MD5 = "9a63b3d62eb6be1eb3f217a362941b2f"
...
REFRESH_NOTE = """# Packaging refresh — 2026-09-19 (v41; Task 102)
...
def verify_post_state() -> None:
    with zipfile.ZipFile(ZIP) as zf:
        nn = zf.namelist()
        assert len(nn) == 92, f"entry count {len(nn)}"
        assert ("paper4_supplementary_v8/verification/"
                "packaging_refresh_2026-09-19_v41.md") in nn
        rtxt = zf.read(
            "paper4_supplementary_v8/README.md").decode("utf-8")
        assert NEW_V41_MD5 in rtxt and OLD_V40_MD5 not in rtxt
        assert NEW_V41_TEX_MD5 in rtxt and OLD_V40_TEX_MD5 not in rtxt


def main() -> int:
    current = sha256(ZIP)
    if current != OLD_SHA256:
        # already refreshed (or unknown state): verify the post-state
        verify_post_state()
        print(f"  the package is already at the v41 refresh "
              f"(on-disk sha256 {current}); post-state verified")
        print("\nrefresh_package_v41: ALL CHECKS PASS (idempotent)")
        return 0
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        with zipfile.ZipFile(ZIP) as zf:
            names = zf.namelist()
            assert len(names) == 91, f"unexpected entry count {len(names)}"
            zf.extractall(tdp)
        ...
        assert n_payload == 91, f"unexpected file count {n_payload}"

        # 1. the README Manuscript line: v40 -> v41
        readme = tree / "README.md"
        old_line = (
            f"**Manuscript:** *{TITLE}* — "
            f"`paper4_delay_dynamics_v40.md` (md5 `{OLD_V40_MD5}`), LaTeX "
            f"`paper4_delay_dynamics_v40.tex` (md5 "
            f"`{OLD_V40_TEX_MD5}`; three consecutive byte-identical "
            f"tectonic builds), rendered PDF "
            f"`paper4_delay_dynamics_v40.pdf` (md5 `{OLD_V40_PDF_MD5}`; "
        ...
        new_line = (
            f"**Manuscript:** *{TITLE}* — "
            f"`paper4_delay_dynamics_v41.md` (md5 `{NEW_V41_MD5}`), LaTeX "
            f"`paper4_delay_dynamics_v41.tex` (md5 "
            f"`{NEW_V41_TEX_MD5}`; three consecutive byte-identical "
            f"tectonic builds), rendered PDF "
            f"`paper4_delay_dynamics_v41.pdf` (md5 `{NEW_V41_PDF_MD5}`; "
            f"45 pages). The v41 round: the 16 Crossref-verified "
            f"reference DOIs inserted (23 doi: suffixes total) and the "
            f"abstract's honesty upgrade with a one-word compensating "
            f"trim (259 journal words); every other block byte-identical "
            f"to v40's."
        )
        rt = readme.read_text(encoding="utf-8")
        assert rt.count(old_line) == 1, "the v40 Manuscript line not found"
        ...
        new_built = (
            ...
            "verified file-by-file; see "
            "`verification/packaging_refresh_2026-09-19_v40.md`); "
            "refreshed in place again 2026-09-19 (Task 102, the v41 "
            "DOI + abstract-honesty round — the Manuscript line advanced "
            "to v41; the payload byte-identical, verified file-by-file; "
            "see `verification/packaging_refresh_2026-09-19_v41.md`)."
        )
        ...
        note = tree / "verification/packaging_refresh_2026-09-19_v41.md"
        ...
        assert n_manifest == 91, f"manifest entry count {n_manifest}"
        ...
        #    and payload samples byte-identical, the new note present
        with zipfile.ZipFile(out) as zf:
            nn = zf.namelist()
            assert len(nn) == 92, f"rebuilt entry count {len(nn)}"
            assert ("paper4_supplementary_v8/verification/"
                    "packaging_refresh_2026-09-19_v41.md") in nn
            ...
            assert NEW_V41_MD5 in rtxt and OLD_V40_MD5 not in rtxt
            assert NEW_V41_TEX_MD5 in rtxt and OLD_V40_TEX_MD5 not in rtxt
        print(f"  rebuilt zip verified: 92 entries; supplement document "
              f"and payload samples byte-identical; the README carries "
              f"the v41 checksums with the v40 md5s absent")
    ...
    print("\nrefresh_package_v41: ALL CHECKS PASS")
    return 0
