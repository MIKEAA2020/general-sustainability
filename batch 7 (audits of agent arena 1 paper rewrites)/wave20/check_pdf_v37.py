#!/usr/bin/env python3
"""Wave-20 / Task 95, part 3: structural PDF verification of
paper4_delay_dynamics_v37.pdf (PyMuPDF), plus page renders for VLM
verification.
...
7. The wave-20 supplementary-pointer gate: the aligned supplement's
   filename renders (space-stripped matching across possible texttt
   line-wraps) and the stale v5 pointer is absent from the rendered
   text.
8. Declarations structure: Data availability + Declaration of competing
   interest as titled subsections; the AI declaration as the FINAL
   subsection with its verbatim text.
9. Figure 1 present exactly once; rejection scan on the rendered text.
10. Renders pages (1, the Section 7 opening, the Section 9 opening, the
    ecological reading, the regime table, the supplementary-pointer
    page, the final declarations) to wave20/logs/ for VLM verification.
"""
...
PDF = LATEX / "paper4_delay_dynamics_v37.pdf"
...
def main() -> int:
    ...
    # the wave-20 supplementary-pointer gate: the aligned supplement's
    # filename renders (space-stripped matching across possible texttt
    # line-wraps); the stale v5 pointer is absent
    nospace = re.sub(r"\s+", "", full_a)
    assert "paper4_supplementary_v6.md" in nospace, (
        "aligned supplementary pointer (v6) missing from rendered text"
    )
    assert "paper4_supplementary_v5" not in nospace, (
        "stale supplementary pointer (v5) present in rendered text"
    )
    nospace_b = re.sub(r"\s+", "", full_b)
    assert "paper4_supplementary_v5" not in nospace_b, (
        "stale supplementary pointer (v5) present in rendered text (b)"
    )
    ...
    print(
        ...
        f"the v6 supplementary pointer rendered with the stale v5 "
        f"pointer absent; Section 7 opens on pdf page {sec7_page + 1}, "
        f"Section 9 on pdf page {sec9_page + 1}, regime table on pdf page "
        f"{regime_page + 1}, ecological reading on pdf page "
        f"{eco_page + 1}, supplementary pointer on pdf page "
