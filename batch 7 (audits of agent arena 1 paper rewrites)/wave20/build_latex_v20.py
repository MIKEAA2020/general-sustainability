#!/usr/bin/env python3
"""Wave-20 / Task 95, part 2: build paper4_delay_dynamics_v37.tex/.pdf from
paper4_delay_dynamics_v37.md (authored by wave20/make_v37.py).

The wave-19 pipeline with every fail-loud check inherited, retargeted to
v37 (the supplementary-alignment round): the paper's Supplementary
paragraph names its accompanying file by version, and the v36
restructuring's renumber map never reached that file --- so the aligned
supplement is published as paper4_supplementary_v6.md (three remapped
main-paper references: 7->9, 9.3->8.3, 9.2->8.2) and the paper's
pointer is updated in a one-line edit.

(a) md level: math-span multiset EQUALITY vs v36 (nothing moved); the
cross-reference resolver (every Section-construct number resolves to an
existing heading); the section-reference multiset IDENTICAL to v36; the
label counts unchanged; content numerics == v36 + the filename version
digit (+6, -5);
...
sentinels PLUS the dropped-caveat regression gates PLUS the aligned
supplementary pointer (tex-escaped form) with the stale pointer absent;
(c) the rejection scanner (35 banned strings) --- zero hits on markdown
AND final LaTeX;
(d) 'if and only if' still exactly 5 times;
(e) tex-level numeric discipline vs the v36 tex by the same stripped
comparison (section refs, headings and theorem labels stripped from
both bodies; delta == the filename version digit only), the tex-level
resolver, and the section-order gate (7 < 8 < 9).

Outputs: arena agent 1/paper rewrites/latex/paper4_delay_dynamics_v37.tex
and .pdf.  v36 tex/pdf remain untouched (version discipline).  Run three
...
PAPERS = ["paper4_delay_dynamics_v37"]
PARENT_MD = "paper4_delay_dynamics_v36"
PARENT_TEX = "paper4_delay_dynamics_v36"
...
V36_NEEDLES_EXTRA = [
    ...
    # wave-20: the aligned supplementary pointer (tex-escaped form)
    "paper4\\_supplementary\\_v6.md",
]
CAVEAT_NEEDLES = [
...
def header_comment(paper: str) -> str:
    return (
        ...
        f"wave20/build_latex_v20.py\n"
        f"% (batch 7 (audits of agent arena 1 paper rewrites)/wave20; md "
        f"authored by\n"
        f"% wave20/make_v37.py from paper4_delay_dynamics_v36.md).\n"
        f"% Task 95, owner-directed: the supplementary-alignment round.  "
        f"The v36\n"
        f"% restructuring's renumber map never reached the accompanying "
        f"supplementary\n"
        f"% file, which still carried three pre-v36 main-paper section "
        f"references\n"
        f"% (Section 7 -> 9; Section 9.3 -> 8.3; Sections 5.1 and 9.2 -> "
        f"5.1 and 8.2);\n"
        f"% the aligned supplement is published as "
        f"paper4_supplementary_v6.md and\n"
        f"% the paper's pointer is updated in a one-line edit.  Everything "
        f"else is\n"
        f"% frozen and machine-verified (all 1,473 math spans "
        f"byte-identical;\n"
        f"% content numerics == v36 + the filename version digit only; "
        f"title,\n"
        f"% abstract, references, declarations, figure byte-identical).\n"
...
def build_paper(paper: str) -> dict:
    ...
    assert md.count("paper4_supplementary_v6.md") == 1, (
        f"{paper}: supplementary pointer missing at md level"
    )
    assert "paper4_supplementary_v5.md" not in md, (
    ...
    assert (n36s - n35s) == Counter({"6": 1}) and (n35s - n36s) == Counter(
        {"5": 1}
    ), (
        f"{paper}: md content-numeric delta != the filename version digit: "
    ...
    assert "paper4\\_supplementary\\_v6.md" in final_flat, (
        f"{paper}: supplementary pointer missing in tex"
    )
    assert "paper4\\_supplementary\\_v5.md" not in final_flat, (
    ...
    if texfile.exists():
        ...
        if "by wave20/build_latex_v20.py" in old:
    ...
    assert (t36n - t35n) == Counter({"6": 1}) and (t35n - t36n) == Counter(
        {"5": 1}
    ), (
