#!/usr/bin/env python3
"""Wave-23 / Task 99, part 3: structural PDF verification of
paper4_delay_dynamics_v40.pdf (PyMuPDF), plus page renders for VLM
verification.

Checks (all fail-loud): the wave-20/21/22 battery inherited and
retargeted, plus the wave-23 phantom-strawman removal gates:
1. Page 1: the TITLE (rendered verbatim --- the Task-98 retitle stands
   unchanged), byline (name / affiliation / clickable ORCID / clickable
   email), pinned date, the abstract's device sentences, keywords; the
   old title's fragments ABSENT from the whole rendered text (retitle
   regression gates).
...
8. The supplementary-pointer gate: the v8 filename renders and the
   stale v7 pointer is absent from the rendered text.
...
10. Figure 1 present exactly once; rejection scan on the rendered text
    (the inherited list + the artifact-class + retitle regression gates
    + THE WAVE-23 GATE: "caricature" must be absent from the rendered
    text in every form).
11. THE WAVE-23 EDIT GATE: the edited Section 1.1 sentence renders in
    its new form ("...analysed within one frame. Fisheries supply the
    motivating instance..."), continuous flow, clause removed.
12. Renders pages (1, the Section 1.1 edit page, the Section 7 opening,
    the Section 9 opening, the ecological reading, the regime table, the
    references pages, the supplementary-pointer page, the final
    declarations) to wave23/logs/ for VLM verification.  This round's
    VLM focus: page 1 (the title block, byline, abstract opening), the
    Section 1.1 edit page, and the final declarations page.
"""
