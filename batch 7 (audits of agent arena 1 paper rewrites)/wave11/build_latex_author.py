#!/usr/bin/env python3
"""Wave-11 build: author front matter for the nine papers' LaTeX/PDF.

Owner directive: "all 9 papers pdf and latex: author: Amin abaee,
independent researcher, amin_abaee@ut.ac.ir, orcid: 0000-0002-0019-1842,
ai declaration: glm (z.ai), qwen (alibaba cloud) and deepseek AI assisted
with drafting and iterative review".

This is the wave-9 pipeline (build_latex.py) with exactly one functional
change: the title block now carries the author, affiliation, contact,
ORCID, and AI declaration (the \\thanks footnote under the author name;
the body is untouched). Fail-loud guarantees:

  1. Every wave-9 integrity check is inherited unchanged (pure-ASCII
     body, emphasis-mis-parse symptom, figure counts, numeric-token
     multiset EXACTLY equal between markdown and LaTeX body, no
     markdown word lost) - the author block lives in the front matter
     only, outside the checked corpus.
  2. BODY IDENTITY: when the existing latex/<paper>.tex is the wave-9
     output, the new file is asserted byte-identical to it after ONLY
     the header-comment and \\author substitutions - so nothing in the
     paper body can drift. When it is already a wave-11 output, the
     rebuild is asserted byte-identical (idempotence).
  3. The author block is asserted present (name, affiliation, email,
     ORCID, AI declaration).
  4. tectonic compile: exit 0, no 'Missing character', no TeX error
     lines; log archived to wave11/logs/.

Outputs: arena agent 1/paper rewrites/latex/<paper>.tex and .pdf (the
canonical deliverables, updated in place; the wave-9 originals remain
recoverable at commit 05b0fe7). Run twice to pin reproducibility.
"""
...
# --- author front matter (owner-supplied, verbatim content) ----------------
# Byline: name + affiliation beneath it (the owner's listing order); the
# \thanks footnote carries contact, ORCID, and the AI declaration.
AUTHOR = (
...
AUTHOR_NEEDLES = (
    "Amin Abaee", "Independent Researcher", "amin\\_abaee@ut.ac.ir",
    "0000-0002-0019-1842", "AI declaration", "DeepSeek",
)
...
def build_paper(paper: str) -> dict:
    ...
    # 5. assemble: front matter now carries the author block (the only
    # functional change from wave 9); the corpus above never sees it.
    final = (
        header_comment(paper, 11)
        + PREAMBLE
        + f"\\title{{{title}}}\n" + AUTHOR + "\n\\date{}\n\\maketitle\n\n"
        + tex.strip()
        + "\n\n\\end{document}\n"
    ...
    if texfile.exists():
        ...
        if "by wave9/build_latex.py" in old:
            # BODY IDENTITY: the new file must equal the wave-9 output with
            # only the header comment and the \author line substituted.
            assert old.count(header_comment(paper, 9)) == 1, f"{paper}: wave-9 header not found verbatim"
            expected = old.replace(header_comment(paper, 9), header_comment(paper, 11), 1)
            assert old.count("\\author{}") == 1, f"{paper}: wave-9 \\author{{}} line not found"
            expected = expected.replace("\\author{}", AUTHOR, 1)
            assert final == expected, (
                f"{paper}: body drifted from the wave-9 output "
                f"(only front matter may change)"
            )
