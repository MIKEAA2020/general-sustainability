...
*Second edition (batch-5 corrections, 2026-08-29): the flat-180-kt infinite-horizon boundary is corrected to the converged fixpoint $2338.3$ kt (the committed table's $2335.4$ was the 300th backward iterate of a recursion that needs $\approx1456$ steps; the corrected value is the same JSON's steady-state entry, and a corrected runner `src/run_intervention_v2.py` with a raised iteration cap and a convergence assertion regenerates the results with this single entry changed); the stress-replay sentence of §2.5 distinguishes the critical-zone rule S1 from the cascade and corrects the 1991 reading; the T=5 classification sentence names both kernels; the displayed operands of the maximal robust flat catch are printed at their exact values; citations are completed. The adjudication record is `BATCH5_JOINT_AUDIT_EVALUATION.md`; every verdict, policy comparison, and retention decision is unchanged.*
...
60 kt and larger ($C\in\{60,120,180,240\}$ kt), and under the cascade (cpm,
whose 1990 stage prescribes 30 kt), the path is below the
LRP already in 1991 (876.5 kt under cpm); zero catch, business-as-usual (5 kt),
and the critical-zone rule S1 — which cuts catch to zero on observing the 1990
stock below the LRP — hold 1991
above the limit (906.5 kt for flat-0 and S1, 901.5 kt for BAU) yet every policy falls below it by 1992 —
...
T=5 classification: under UC-q10 only the 1980s peak years of the 33 observed
states lie inside the T=5 nominal kernels — $\{1985, 1987, 1989\}$ for the
60-kt rules (S1, cpm, flat-60) and, additionally, 1988 for BAU; the entire
post-1990 history and most of the 1980s are
...
The analysis is fully deterministic (no random components). Re-executing the committed runner regenerates both output files, `results/intervention_results.json` and `results/intervention_boundaries.csv`; a verification re-execution in a fresh environment reproduced both files byte for byte. The full intervention protocol is provided in `wave_e_cod/protocol_intervention.md`. The second edition's corrected runner `src/run_intervention_v2.py` (infinite-horizon iteration cap raised from 300 to 20000 with an explicit convergence assertion) writes `results/intervention_results_v2.json` and `results/intervention_boundaries_v2.csv`, identical to the committed results in every entry except the flat-180-kt $T=\infty$ boundary corrected above; the committed first-edition files are retained unchanged as the audited record. The critical-zone rule and cascade vocabulary follows the DFO precautionary-approach framework (DFO 2009); the SSB series and LRP are DFO (2016) Table A2.
