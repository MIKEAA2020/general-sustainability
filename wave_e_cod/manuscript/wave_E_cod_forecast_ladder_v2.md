...
*Second edition (batch-5 corrections, 2026-08-29): the surplus-production law of §3 is stated in its exact piecewise form (the Allee factor is active only when a positive threshold is declared — $\mathfrak s=0$ does not deactivate it); the Brier paragraph of §5.3 now reports the $\Omega_{\mathrm{xte}}$ values it describes, with the conclusion corrected (M1 and M3 improve the one-year Brier score; no model improves the primary RMSE at either horizon); the abstract's ladder range separates the two catch passes; the acoustic-index scaling defines $I_{\mathrm{ref}}$ and $b$; the freeze-discipline caveat of the specification sheet is restated; the reproducibility statement records the environment sensitivity of the optimizer-based rows; citations are completed and the Schijns page range completed. The adjudication record is `BATCH5_JOINT_AUDIT_EVALUATION.md`; all retained verdicts are unchanged.*
...
This paper does not estimate an Allee threshold, identify the cause of the 1990s collapse, or evaluate whether the 1992 moratorium was adequate (A014-L4, L5, L6). It asks whether stock-flow, residual, delay, or prey-informed modules reduce SSB forecast error relative to last-value persistence and to an autonomous surplus-production model.
...
On recovery, M1 and M2 coincide because $C_t\equiv 5$ on both train and test. M1b reports a lower RMSE, but $\mathfrak s\to 0$ and $K$ collapses to the training range: an unidentified Allee parameter, not a biological threshold (A014, Proposition 2 in conditional form). M3's $\phi=0.95$ persists a negative residual and increases error.
...
Murphy et al. (2025) report a 1991 acoustic collapse (1985--90 median 3704 kt versus 1991--2022 median 174 kt). A two-regime $r$ with break 1991 uses the high regime only for forecasts issued before 1991. Capelin is a candidate disturbance class $W$, not a parameter that was fitted.
