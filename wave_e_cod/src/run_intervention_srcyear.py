...
Batch-5 corrected edition (v2).  Two changes relative to run_intervention.py,
both documented in BATCH5_JOINT_AUDIT_EVALUATION.md (finding W02):
  (a) the T="inf" kernel iteration cap is raised from 300 to 20000 with an
      explicit convergence assertion — the committed runner silently returned
      the 300th backward iterate for the flat-180-kt policy, whose fixed point
      needs ~1456 iterations (F'(2338.3) = 1.0155, ~1.5% contraction per step);
      the committed table value 2335.4 was the unconverged iterate, while the
      same JSON's steady_states field already carried the correct fixpoint
      2338.27;
  (b) outputs are written to results/intervention_results_v2.json and
      results/intervention_boundaries_v2.csv so the audited artifacts stay
      untouched.  Every other policy converges within the old cap, so all
      other entries are bit-identical to the committed results.
...
def kernel(policy, fit, e, K, T):
    ...
    cur = K0
    # v2: 20000-iteration cap for the fixpoint pass, with convergence asserted
    # (the v1 cap of 300 silently truncated the flat-180-kt backward recursion,
    # which needs ~1456 iterations at F' = 1.0155 per step).
    max_iter = 1 if T == 1 else (20000 if T == "inf" else int(T))
    converged = False
    for _ in range(max_iter):
        ...
        if T == "inf":
            if len(nxt) == len(cur) and all(
                ...
                return nxt
        cur = nxt
    if T == "inf" and not converged:
        raise RuntimeError(
            "T=inf kernel backward iteration did not converge within "
            f"{max_iter} iterations (tolerance 1e-9); refusing to return an "
            "unconverged iterate"
        )
    return cur
...
def main() -> None:
    ...
    for pid, rr in retention_certified.items():
        print(f"  {pid}: retained={rr['retained']} "
              ...
              f"improves={rr['improves_somewhere']}, "
              f"readings={rr['retained_readings']})")
    print("wrote results/intervention_results_v2.json and "
          "results/intervention_boundaries_v2.csv")
