...
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))   # v48 modules live beside this script
# the transpile helpers write their scratch files into the working directory, so the build runs from v7 as v47 did
...
def main():
    ...
    docs = {
        'article': ('paper3_material_ledgers_v42', 'paper3_material_ledgers_v48'),
    ...
    for key, (old, new) in docs.items():
        ...
        # v48 does not adopt the draft's paragraphs wholesale. The ledger decides verbatim copying one sentence at
        # a time, in `v48_reuse_split.json`, and it is applied after the style pass below: a block-level proof that a
        # paragraph "says the same thing" cannot honour a ruling that a *sentence* inside it may not ship.
        txt2, n_ad, n_kept, b_rep = txt, 0, 0, ('draftbase off (v48): the claim ledger governs verbatim copying at '
                                                'sentence level')
        # ---------------------------------------------------------------- v48: the reuse splice, first half
        # the sentences the claim ledger cleared for verbatim reuse go in now, ahead of the register pass, so the
        # pass styles the paper's own prose and the splice's insertions are checked against it afterwards rather
        # than silently restyled. `v48_splice_v1.py` finds each deposit sentence with the ledger's own
        # segmentation and normal form, and reports everything it cannot place instead of guessing.
        if key == 'article':
            import v48_splice_v1
            LOG.setdefault('v48', {})
            txt2, _spl_done, _spl_log = v48_splice_v1.insert(txt2)
            LOG['v48']['splice_insert'] = dict(_spl_done)
            LOG['v48']['splice_log'] = _spl_log
            _sep = v48_splice_v1.guard(txt, txt2)
            LOG['v48']['splice_separator_style'] = _sep
            print(f"    reuse splice: {dict(_spl_done)}")
        ...
        # v48: a reused sentence writes `240,000 kt` in text where the deposited article writes `$240{,}000$ kt` in
        # maths. `sk.values` splits the second into `240` and `000`, so the figure would read as invented. The
        # invariant is about figures, so the thousands separators are folded on both sides before comparing
        _fold = lambda t: __import__('re').sub(r'(?<=\d),(?=\d)', '', t.replace('{,}', ','))
        _vo, _vp = __import__('collections').Counter(sk.values(_fold(txt2))), \
            __import__('collections').Counter(sk.values(_fold(_base)))
        ...
        for _i, _bl in enumerate(_rb):
            ...
            if not _bl or not sk.is_flow(_bl) or '$$' in _bl or _bl.lstrip().startswith(('#', '|', '>', '`', '*')):
                continue
            # (v48: no draft paragraph has been adopted, so nothing here is exempt from the register pass)
            _nb = sk.register(_bl)
        ...
        _vo3 = __import__('collections').Counter(sk.values(_fold(_t3)))
        _vp3 = __import__('collections').Counter(sk.values(_fold(_base)))
        ...
        txt2 = _n2
        # ---------------------------------------------------------- v48: the reuse splice, second half
        # the residue pass and both conventions rewrite words inside the reused sentences. Every one the pass
        # damaged is restored to the draft's wording, and only then are the two conventions re-applied to the
        # restored text, since citation status and the paper's name for itself are not style surfaces.
        if key == 'article':
            txt2, _spl_prot = v48_splice_v1.protect(txt2, LOG['v48']['splice_log'])
            LOG['v48']['splice_protect'] = _spl_prot
            _sep = v48_splice_v1.guard(txt, txt2)
            LOG['v48']['splice_separator_style'] = _sep
            json.dump(dict(insert=dict(_spl_done), protect=_spl_prot, separator_style_only=_sep,
                           splice_log=LOG['v48']['splice_log']),
                      open('/home/user/revision/v48/v48_splice_log.json', 'w'), indent=1)
            print(f"    reuse splice, after the pass: {_spl_prot}")
        ...
        out = txt2
        if key == 'article':
            # the two document conventions run after the splice and so reach the reused sentences too. What they
            # touch is counted by the gate (`verify_v48_base.py`), not hidden here
            LOG['v48']['conventions_after_splice'] = ['in review -> under review', 'this article -> this paper']
    ...
    import build_companions_v1 as bc
    _art = 'paper3_material_ledgers_v48'
    ...
    for base in ('paper3_material_ledgers_v48', 'paper3_supplementary_v18', 'companionA_certification_procedure_v9',
    ...
    open(f'{R}/revisions_v48_base_log.json', 'w').write(json.dumps(LOG, indent=1))
    ...
    _placed = {l['id'] for l in LOG.get('v48', {}).get('splice_log', []) if l['state'].startswith('inserted')}
    _FLAWS = [('D0158', '`each` where the deposit reasons with `every`'),
              ('D0089', 'a taxonomy the article states four ways, none of them an exhaustive division'),
              ('D0129', 'the principle that a review cycle closes at the rate of use, not of the calendar'),
              ('D0108', 'the deposit\u2019s only definition of `Survey (USGS)`, and its `U.S.` spelling'),
              ('D0309', 'the locator `S6` the deposit carries'), ('D0530', 'the phrase `recorded in S5`'),
              ('D0618', 'the locators `S2` and `S14`'), ('D0620', 'the same two locators in the next sentence')]
    _in = [x for x in _FLAWS if x[0] in _placed]
    _out = [x for x in _FLAWS if x[0] not in _placed]
    _flag_note = ('* eight rows carry a defect that the line read or the pointer read found and recommended for '
                  'regeneration, and none of the eight is in the author\u2019s overrule file. '
                  f'{len(_in)} of them were placed by the splice and so ship reused against that recorded '
                  'recommendation: ' + '; '.join(f'{i} ({w})' for i, w in _in) + '. '
                  + (('The other ' + str(len(_out)) + ' - ' + ', '.join(i for i, _w in _out) +
                      ' - were not placed at all: the splice refused them for a reason of its own ('
                      'the sentence after the one it would replace takes its antecedent from it), so the '
                      'deposited wording stands in their place. That is the outcome the recommendation wanted, '
                      'arrived at by a guard rather than by a ruling, and it is stated as such rather than '
                      'counted as a fix.') if _out else 'All eight are therefore reported, none repaired silently.'))
    spl = ent.get('splice_insert') or {}
    prt = ent.get('splice_protect') or {}
    sp = json.load(open('/home/user/revision/v48/v48_splice_log.json')) if \
        os.path.exists('/home/user/revision/v48/v48_splice_log.json') else {}
    split_md = json.load(open('/home/user/revision/v48/v48_reuse_split.json'))
    not_placed = split_md['counts']['reuse'] - spl.get('inserted', 0)   # `fuzzy` counts how a row matched, not a refusal
    L = ['# The v48 line: the draft as the exemplar, the prose written from the deposited article', '',
         ...
         f'* the claim ledger (`v48/claim_ledger_v1.py`, {558} sentences extracted from the draft, 0 skipped, 0 with no',
         ...
         f'* {split_md["counts"]["reuse"]} sentences cleared for verbatim reuse - {split_md["counts"]["reuse_high_supported"]} supported at high or certain match, '
         f'{split_md["counts"]["reuse_signposting"]} signposting - are put into the text as the draft wrote them, and',
         f'* {split_md["counts"]["regen"]} were regenerated from the deposit in the draft\u2019s register: {split_md["counts"]["regen_medium_low_supported"]} where the match',
         '  was medium or low, '
         f'{split_md["counts"]["regen_flagged"]} where the line-by-line read flagged a value, attribution or scope problem, and',
         f'{split_md["counts"]["regen_by_overrule"]} the author marked for regeneration over a notation defect;',
         f'* {spl.get("inserted", 0)} of the {split_md["counts"]["reuse"]} cleared sentences were placed in the article, {not_placed} could not be',
         f'  placed - {spl.get("fragment", 0)} are halves of sentences the extractor cut at a display, {spl.get("display-entangled", 0)} run',
         f'  through a display or table, {spl.get("not-in-document", 0)} are not in the document verbatim once the ledger\u2019s',
         f'  line filters are applied - and an unplaced row ships from the deposited article, which is the direction the',
         f'  ruling asks for; {prt.get("restored", 0)} inserted sentences the register pass had rewritten were set back to the',
         f'  draft\u2019s wording after the pass, and {prt.get("left_with_the_pass", 0)} could not be found again;',
         ...
         f'  inside maths, the swap leaves {len(sp.get("separator_style_only", []))} figures written with another separator style '
         '  (`240,000` for `$240{,}000$`), and the gate compares folded keys so that is counted as the same figure.',
         ...
         f'* {refused} draft paragraphs were refused by the earlier block-level test and no longer apply; the',
         ...
         'The list is the deposited article\u2019s own: 39 entries, 8 of them carrying a DOI, and the year letters '
         'that separate this author\u2019s 2026 papers (`2026a/b/c`) intact; the gate asserts the whole back matter '
         'is byte-identical to the verified line. The draft\u2019s list is a PDF text-layer reflow in which these '
         '39 works survive as 21 glued blocks with the year letters dropped, so it is reported, not adopted. Two '
         'consequences belong to the author rather than to the pipeline. The list carries no DOI for Martinez-Alier, '
         'Munda and O\u2019Neill (*Ecological Economics* 26(3), 277\u2013286, 1998; `10.1016/S0921-8009(97)00120-1`, '
         'read back from the publisher\u2019s record), where eight other entries do; and the draft\u2019s four '
         '`Author, D., et al., in review` placeholder sentences did not reach the shipped text, because the one '
         'reuse row that would have carried one (D0218) was refused by the figure guard: the anonymisation a blind '
         'submission needs is intact here by a guard rather than by design, and a run whose reuse set grows could '
         'ship it. The same guard is why `in review` appears nowhere in the shipped file and the author\u2019s own '
         '`under review` is the single status sentence.', '',
    ...
    L += ['', 'The register is judged as the distance to the draft\u2019s own profile, per 1000 words of flowing prose,',
          ...
          '```', '']
    open(f'{R}/structure_v48_base.md', 'w').write('\n'.join(L))
    print('\nlog: revisions_v48_base_log.json')
    print('note: structure_v48_base.md')
    print('next: verify_v48_base.py, then build_v48_package.py')
