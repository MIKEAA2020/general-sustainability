...
# 10. v4 restorations (the sentences upgrade2 prompted)
v4 = ["four different predicates", "routinely slides between them", "A mass-balanced ledger can be chemically impossible",
      "Thermodynamic admissibility \u2014 energy conservation", "requires structure outside the present scope",
      "**What is not claimed.**", "No thermodynamic admissibility is claimed",
      "not recoverable at all", "compaction, subsidence or saline intrusion",
      "recoverable only on a timescale longer than it is taken", "overlapping readings of the same pool",
      "data-reconciliation practice of material flow analysis", "Bookkeeping balance, stoichiometric conservation",
      "slowly regenerating flow of services"]
miss = [p for p in v4 if p not in t]
chk(not miss, "v4 restorations present (%d checked, %d missing: %s)" % (len(v4), len(miss), miss))
chk(t.count("\\square") == 24, "proof QED marks unchanged at 24 (%d)" % t.count("\\square"))
chk(not re.search(r'\u25a1|\(\\square\)\s*\(\\square\)', t), "no duplicated QED mark")
chk(len(re.findall(r'[A-Z\u00c0-\u00de][a-z\u00e0-\u00ff]{2,}\s+\([^)]*\)\.', t)) > 0, "restored sentences read as sentences (sanity)")

# 11. v4 §6.3 taxonomy + equality discipline
v4b = ["Two disciplines attach", "equality at the hitting time", "regime-shift threshold",
       "minimum service-supporting stock", "reserved for \\(S_m=0\\)", "barrier violation"]
miss = [p for p in v4b if p not in t]
chk(not miss, "§6.3 threshold taxonomy present (%s)" % (miss or "all 6"))
chk(len(re.findall(r'thermodynamic', t, re.I)) >= 4, "thermodynamic predicate restored (%d hits)" % len(re.findall(r'thermodynamic', t, re.I)))
chk(len(re.findall(r'recoverab', t, re.I)) >= 5, "recoverability restored (%d hits)" % len(re.findall(r'recoverab', t, re.I)))
chk(len(re.findall(r'reconcil', t, re.I)) >= 2, "reconciliation lineage present (%d hits)" % len(re.findall(r'reconcil', t, re.I)))
