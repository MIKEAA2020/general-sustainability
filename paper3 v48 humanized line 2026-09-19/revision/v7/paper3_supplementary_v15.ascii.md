...
distribution. The variance closure does not exist as stated. $\square$
...
left-hand side of any displayed equation and is not given by a constitutive law in the template. $\square$
...
attributable because the underlying stock parameter is held fixed within each domain pair. $\square$
...
- **Theorems with displayed proofs:** Theorem 1 (support-saturated logistic limit); Theorem 2 (registered-
  ...
  Theorem 18 (first-passage law, Chhikara and Folks, 1989; Redner, 2001); Theorem 20 (It$\ddot{o}$ lemma, $\text{\O}$ksendal,
...
Exhibit (two components, `w = (0.5, 0.5)`, declared bounds `ell = (-4, -4)`, `u = (10, 10)`), from `code/certification_lp.py`:
...
`T = H_loc/(1 - kappa)` reproduces the closed form exactly for every `kappa < 1`. At and above `kappa = 1` the integral diverges at the barrier,
...
240,000 kt/yr (USGS *Mineral Commodity Summaries* 2026, as tabulated in $\S$6.5.3), `tau = 308.33` yr, `g = 0.03 yr1`:
...
The first-order condition for the ratio to be conservative is `eta >= 12gtau = 4.625`, outside the admissible interval `0 <= eta <= 1`: the
reserve-life ratio is optimistic in a direction fixed by the model class, independent of the data.

**3. Persistence index on the declared trend class (Proposition 32)**. 400 replicates, `beta = sigma = 1`, seed 7, `index = (a_n - min_k a_k)/(-fitted slope)`:
...
in $\S$6.5.1--6.5.4 of the main text. The table collects them so that the promotion test is checkable in one
...
ratio, or a pressure scale, which is the status $\S$6.5 assigns.

---

## S9.3 - Reproduction

`code/persistence_index_simulation.py` (Proposition 32), `code/curvature_and_crossover.py` (Propositions 26--28), `code/certification_lp.py` (premium, `delta*_j`, Proposition 31, and the vectors of S-C.5).
...
Main counter 1--47, with the layering counter (Propositions 1--2) separate, per $\S$3.1. Every statement numbered
...
Two unnumbered notes sit alongside the inventory: *Certification state* in $\S$3.1 and *Antecedents* in $\S$1.2,
together with the refute/alarm clause in $\S$10.1, the joint-minimal reporting note in $\S$6.3, and the review-
interval note in $\S$9. The disturbance/residual material is not unnumbered: it is Definition 23 in $\S$3.4.
...
Status vocabulary is the main text's own ($\S$3.1): *established*, *not established*, *not applicable*. A value
...
resource--product--waste--detritus structure of Section 2.3 (phosphate rock $\rightarrow$ fertilizer $\rightarrow$ soil pool $\rightarrow$ runoff,
...
NFA_2018_edition_kaggle.csv 11857080 bytes $\cdot$ 0dd766d975cd5e85f2a2d39cff1f914b92c514186ce507cb1f721a63be57b6a5
NFA_2017_edition_kaggle.zip.csv 13195855 bytes

**Arithmetic.** Exactly the main text's, on the publisher's `World` rows: component totals in gha for `record in {BiocapTotGHA, EFConsTotGHA}`, then
