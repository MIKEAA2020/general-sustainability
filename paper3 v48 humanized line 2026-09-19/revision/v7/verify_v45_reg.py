#!/usr/bin/env python3
r"""verify_v45_reg.py - the gate for the v45 line, which is a register line, not a content line.
...
PAIRS = (('paper3_material_ledgers_v42', 'paper3_material_ledgers_v45'),
         ('paper3_supplementary_v13', 'paper3_supplementary_v16'),
         ('companionA_certification_procedure_v4', 'companionA_certification_procedure_v7'),
         ('companionB_standards_horizon_v4', 'companionB_standards_horizon_v7'))
KEYS = {'paper3_material_ledgers_v45': 'article', 'paper3_supplementary_v16': 'supplementary',
        'companionA_certification_procedure_v7': 'companionA', 'companionB_standards_horizon_v7': 'companionB'}
...
LG = json.load(open(f'{R}/revisions_v45_reg_log.json'))
for old, new in PAIRS:
    ent = LG[KEYS[new]]
...
print('[1] the v45 line reverses to v42, edit for edit')
LOG = json.load(open(f'{R}/revisions_v45_reg_log.json'))
LG = json.load(open(f'{R}/revisions_v45_reg_log.json'))
for old, new in PAIRS:
    ent = LG[KEYS[new]]
    if ent.get('carried_from'):
...
for old, new in PAIRS:
    ...
    for base in (new, old):
        t, _n = pdf_text(base)
        open(f'{R}/.v45gate/{base}.pdftext', 'w').write(t)
    gq = audit(f'{R}/.v45gate/{new}.pdftext')[0]
    gq0 = audit(f'{R}/.v45gate/{old}.pdftext')[0]
    ...
    FR = "'X, not Y' frames"
    print(f'       {new}: flowing prose, v45 vs v42 -- mean {gf["mean words/sentence"][0]} / '
...
sup = open(f'{R}/paper3_supplementary_v16.md').read()
art = open(f'{R}/paper3_material_ledgers_v45.md').read()
