#!/usr/bin/env python3
r"""Build the v43 supplementary package: the exhibit bundle restated for readers, and one archive of everything.
...
V3 = f'{R}/code_v3'
PKG = f'{R}/package_v2'
ZIP = f'{R}/paper3_supplementary_package_v2.zip'
...
def package():
    ...
    if os.path.isdir(PKG):
        shutil.rmtree(PKG)
    root = f'{PKG}/paper3_supplementary_package_v2'
    for sub in ('manuscript', 'code', 'analysis/nfa_tau/source', 'builders'):
        os.makedirs(f'{root}/{sub}', exist_ok=True)
    for base in ('paper3_material_ledgers_v43', 'paper3_supplementary_v14',
                 'companionA_certification_procedure_v5', 'companionB_standards_horizon_v5'):
        for ext in ('.md', '.tex', '.pdf'):
            shutil.copy(f'{R}/{base}{ext}', f'{root}/manuscript/{base}{ext}')
    shutil.copy(f'{R}/paper3_supplementary_v14.ascii.md', f'{root}/manuscript/')
    ...
    for f in ('build_v42_formal.py', 'build_v42_package.py', 'tablekit_v2.py', 'verify_v42_build.py',
              ...
              'build_v43_style.py', 'verify_v43_style.py', 'stylekit_v1.py', 'structure_v43_style.md',
              'revisions_v43_style_log.json'):
...
* **The supplementary document** is `manuscript/paper3_supplementary_v14.pdf` (19 pp., sections S1 to S17). It
  ...
  section that reports the overshoot-date recomputation (S17). It is typeset from `paper3_supplementary_v14.md`,
  whose ASCII transliteration `paper3_supplementary_v14.ascii.md` is included because that is the source the
...
companions are papers in their own right, not appendices: `companionA_certification_procedure_v5.pdf` specifies
the certification procedure the article's predicates define, and `companionB_standards_horizon_v5.pdf` places the
...
`builders/structure_v42.md`. The text of these four files is the v43 register line of the same paper: a
punctuation-and-voice pass over the v42 sources that moves no figure, no label, no table cell and no
equation, and that `python3 builders/verify_v43_style.py` checks by reversing every edit it recorded and by
comparing the two lines value by value. `builders/structure_v43_style.md` says what the pass is permitted to
...
'''
    ...
    if os.path.exists(ZIP):
        os.remove(ZIP)
    subprocess.run(['zip', '-qrX', ZIP, 'paper3_supplementary_package_v2'], cwd=PKG, check=True, timeout=900)
