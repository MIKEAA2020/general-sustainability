...
- **Theorems whose proofs rest on classical cited results:** Theorem 18 (first-passage law, Chhikara and Folks, 1989; Redner, 2001); Theorem 20 (It$\ddot{o}$ lemma, $\text{\O}$ksendal, 2003); Theorem 10--11 (tangent-cone invariance, Aubin, 1991, in the compartmental lineage of Jacquez and Simon, 1993).
...
## S7 - Proof obligations of the certification state (Sec. 3.1)
...
| `Balanced` | the accounting identity `S_T v + B_T u_partial = b_dot` holds as written | $\S$3.2 | flux table and incidence matrix | an entry is missing, duplicated, or routed wrongly |
| `Conserved` | `ell^T C d_x` carries a declared budget and the invariants follow | Definition 23, Proposition 1 | left-null vectors of `S_T`, disturbance budget `eps_ell` | conservation is asserted, not certified; the statistical discrepancy is unbounded |
...
| `Closed` | demand lies in the closure cone, and the deficit is declared at a stated timescale | Definitions 21--22 | stationary-flux LP, process graph, `tau_use` | the shortfall is being carried by the support pool or the sink, in a quantity not stated |
...
```
maximise   lambda
subject to S_T v + B_T u_partial = 0
           0 <= v <= v
           lambda D <= P v
```
Inputs: incidence operator, declared capacities `v`, the boundary-transfer schedule, the demanded
use vector `D`. Feasibility for `lambda >= 1` is the regime in which the cycle closes at the demanded
...
```
If feasible, `T <= V(x_0)/(y_req - beta)` is available with `beta = max lambda^TGb` over the declared box.
...
service rate `y_req`, and the units assigned to each `lambda_j`.
...
**1. Curvature correction (Proposition 27), family `phi(u) = c u^p`, `c = 1`, `u_0 = 1`:**
...
The first-order condition for the ratio to be conservative is `eta >= 12gtau = 4.625`, outside the
admissible interval `0 <= eta <= 1`: the reserve-life ratio is optimistic in a direction fixed by the
...
|---|---|---|
| reserve-life ratio `R/P` | `R` is a fixed physical inventory, `R_dot = -P`, `P` constant, and no discovery, price response or reclassification occurs | $\S$6.5.3: the reserve classification is economic; the premises are carried, not discharged |
...
| removals-only pressure time `log(B/B_min)/F` | `B_dot = -FB` with `F` constant, no growth, no recruitment, no catch misreporting | $\S$6.5.4: exact hitting time of the declared removals-only law, nothing more |
...
documents) and the 2017 edition (1961--2013), both deposited by the publisher under CC BY-SA 4.0. The tables are held with the analysis record so that the run can be repeated without a network; they are not redistributed with the deposit, whose copy of the record carries `source/MANIFEST.md` with the two retrieval commands and the hashes that settle what was read. 
...
checksums of the two tables as read: 60968f7c9959537f8e67f915aca4259662b5cd42c3a0ec02d094677b4c280ef6  NFA_2018_edition_kaggle.csv  11857080 bytes $\cdot$ 0dd766d975cd5e85f2a2d39cff1f914b92c514186ce507cb1f721a63be57b6a5  NFA_2017_edition_kaggle.zip.csv  13195855 bytes
