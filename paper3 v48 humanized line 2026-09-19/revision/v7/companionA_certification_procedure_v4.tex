...
% Companion paper. Typeset from companionA_certification_procedure_v4.md; the LaTeX source, the markdown mirror and the compiled PDF are
...
The main text states eight certification obligations and three linear programmes as \emph{results}: what they decide, what they cannot decide, and what a failure to satisfy one does and does not imply. It does not state them as \emph{procedures}, because a journal article is not the place for input schemas, exit conditions, solver caveats and a run transcript.
...
The upper bound \texttt{lambda\_bar} is not a numerical convenience: it is what makes the budget finite, and it must be declared. If the system is feasible, the article's budget inequality \texttt{T <= V(x0)/\allowbreak{}(y\_req - bet\allowbreak{}a)} is available with \texttt{beta = max l\allowbreak{}ambda\textasciicircum{}TGb} over the declared box. \textbf{Infeasibility carries no conclusion}: it shows that this multiplier family does not close, and the tool must print that sentence rather than "no safety margin", because the second is a claim about the ledger and the first is a claim about the search. Inputs: the affine margin map \texttt{Gx + a}, the weights \texttt{c}, the flux bounds, \texttt{y\_req}, and the units assigned to each component of \texttt{lambda} --- an unlabelled multiplier is uninterpretable, since the reading of the bound depends on the unit of \texttt{lambda}.
...
Two keys are load-bearing and often omitted elsewhere. \texttt{witness} must name an object, not a feeling: a \texttt{established} without a witness is printed as \texttt{not establis\allowbreak{}hed} by the tool. And \texttt{vintage.\allowbreak{}content\_test\allowbreak{}s} must be a \emph{result of running the checks of Section 7}, not a copy of the citation string, which is exactly the distinction the main text needed in its own data-availability statement.
...
\texttt{code/\allowbreak{}MANIFEST.\allowbreak{}md} records the checksums of the files, the run command, the environment of the recorded run and the scope limits, one of which matters in review: no script consumes the G3P basin rows whose provenance is recorded in the supplementary's S5.4, because those rows are quarantined and nothing in the exhibits depends on them. The analysis record carries its own manifest in \texttt{source/\allowbreak{}MANIFEST.\allowbreak{}md}, and its scope limit is of the same kind: the two edition tables are not redistributed beyond the working copy needed to re-run the arithmetic, because the licence that permits the re-run is the same licence that requires the attribution the manifest records.
...
The index stays at the noise-to-trend scale while the stock implied by the same series falls by five orders of magnitude (the \texttt{n usable} column, omitted from the excerpt above, is 400 on every row): a statistic that is bounded on the declared class is not a horizon, and Proposition 32's content is the bound, not the trend. The crossover grid makes the same point in the opposite direction for reserve-life ratios --- proportional depletion gives \texttt{T \textasciitilde{}= 276.\allowbreak{}310 yr} against a local horizon of \texttt{20.\allowbreak{}000 yr}, constant depletion gives \texttt{1999.\allowbreak{}998 yr} against \texttt{1999.\allowbreak{}998 yr}, accelerating depletion \texttt{phi(A) = cA\textasciicircum\allowbreak{}{}\{0.\allowbreak{}4\}} gives \texttt{83.\allowbreak{}312 yr} against \texttt{50.\allowbreak{}000 yr} --- so the sign of the frozen-rate error depends on the law, and a reporter who publishes one number per stock is publishing a law-free guess.
...
Two obligations are not currently checkable from the archived material, the author included. What would make each checkable is stated here, so that the gap is a declared requirement rather than a silence.
