#!/usr/bin/env python3
r"""verify_v46_base.py - the gate for the v46 line, which is a register line, not a content line.
...
PAIRS = (('paper3_material_ledgers_v42', 'paper3_material_ledgers_v46'),
         ('paper3_supplementary_v13', 'paper3_supplementary_v17'),
         ('companionA_certification_procedure_v4', 'companionA_certification_procedure_v8'),
         ('companionB_standards_horizon_v4', 'companionB_standards_horizon_v8'))
KEYS = {'paper3_material_ledgers_v46': 'article', 'paper3_supplementary_v17': 'supplementary',
        'companionA_certification_procedure_v8': 'companionA', 'companionB_standards_horizon_v8': 'companionB'}
...
LG = json.load(open(f'{R}/revisions_v46_base_log.json'))
for old, new in PAIRS:
    ent = LG[KEYS[new]]
    ...
    ck(not re.findall(r'(?m)^Not [a-z]|(?<=[.!?] )Not [a-z]', body),
       f'{new}: no sentence begins with "Not", which would be a fragment left behind by the frame repair')

print('[1] every paragraph is attributed: the draft supplies the surface, this paper supplies the content')
LOG = json.load(open(f'{R}/revisions_v46_base_log.json'))
...
for old, new in PAIRS:
    ...
    bad_new = [e for e in edits if e.get('mode') not in ('transplant', 'sentences')
    ...
    print(f'       {new}: {len(_real)} paragraphs taken from the draft ({sum(1 for e in _real if e.get("mode")=="framing")} '
...
for old, new in PAIRS:
    ...
    for base in (new, old):
        t, _n = pdf_text(base)
        open(f'{R}/.v46gate/{base}.pdftext', 'w').write(t)
    gq = audit(f'{R}/.v46gate/{new}.pdftext')[0]
    gq0 = audit(f'{R}/.v46gate/{old}.pdftext')[0]
    ...
    CORPUS_LED = {'em-dashes', "'X, not Y' frames", 'semicolons'}
    worse = []
    ...
    FR = "'X, not Y' frames"
    print(f'       {new}: flowing prose, v46 vs v42 -- mean {gf["mean words/sentence"][0]} / '
...
sup = open(f'{R}/paper3_supplementary_v17.md').read()
art = open(f'{R}/paper3_material_ledgers_v46.md').read()
