#!/usr/bin/env python3
r"""Build the v45 supplementary package: the exhibit bundle restated for readers, and one archive of everything.
...
V3 = f'{R}/code_v3'
PKG = f'{R}/package_v4'
ZIP = f'{R}/paper3_supplementary_package_v4.zip'
...
def package():
    ...
    if os.path.isdir(PKG):
        shutil.rmtree(PKG)
    root = f'{PKG}/paper3_supplementary_package_v4'
    for sub in ('manuscript', 'code', 'analysis/nfa_tau/source', 'builders'):
        os.makedirs(f'{root}/{sub}', exist_ok=True)
    for base in ('paper3_material_ledgers_v45', 'paper3_supplementary_v16',
                 'companionA_certification_procedure_v7', 'companionB_standards_horizon_v7'):
        for ext in ('.md', '.tex', '.pdf'):
            shutil.copy(f'{R}/{base}{ext}', f'{root}/manuscript/{base}{ext}')
    shutil.copy(f'{R}/paper3_supplementary_v16.ascii.md', f'{root}/manuscript/')
    ...
    for f in ('build_v42_formal.py', 'build_v42_package.py', 'tablekit_v2.py', 'verify_v42_build.py',
              ...
              'build_v45_reg.py', 'verify_v45_reg.py', 'stylekit_v1.py', 'structure_v45_reg.md',
              'revisions_v45_reg_log.json'):
...
* **The supplementary document** is `manuscript/paper3_supplementary_v16.pdf` (19 pp., sections S1 to S17). It
  ...
  section that reports the overshoot-date recomputation (S17). It is typeset from `paper3_supplementary_v16.md`,
  whose ASCII transliteration `paper3_supplementary_v16.ascii.md` is included because that is the source the
...
companions are papers in their own right, not appendices: `companionA_certification_procedure_v7.pdf` specifies
the certification procedure the article's predicates define, and `companionB_standards_horizon_v7.pdf` places the
...
`builders/structure_v42.md`. The text of these four files is the v45 line of the same paper: the main text's prose was
rewritten by adopting the wording of the humanized draft in `humanize/` wherever a content guard could prove the
two said the same thing (its numbers, its status words, its bare quantities, its cross-references and the
sentence's opening referent), and the three companion documents are the v43 register line carried unchanged,
because the humanized draft is a humanization of the main text alone. On top of that wording the main text also
took the draft's register as a measured target: the humanization's own edit rules were mined by aligning
`uploads/paper3_material_ledgers_v32.pdf` (the source the draft was made from) with the draft, and the plain-verb,
de-nominalising, dash-gloss and semicolon-join rules that came out of it are in `builders/stylekit_v1.py` as the
GEM_* tables. The gate reads the result as a profile and asserts the distance to the draft's profile, per feature
and in aggregate, instead of asserting my own numbers: a
punctuation-and-voice pass over the v42 sources that moves no figure, no label, no table cell and no
equation, and that `python3 builders/verify_v45_reg.py` checks by reversing every edit it recorded and by
comparing the two lines value by value, and by asserting that more of the article's sentences now read
word for word as the humanized draft did than read that way before it. `builders/structure_v45_reg.md` says what the pass is permitted to
touch and what it is not, with the sentence-length measurements before and after. They are the production machinery, not part of the text.
...
'''
    ...
    if os.path.exists(ZIP):
        os.remove(ZIP)
    subprocess.run(['zip', '-qrX', ZIP, 'paper3_supplementary_package_v4'], cwd=PKG, check=True, timeout=900)
