#!/usr/bin/env python3
r"""build_v43_style.py - the v43 line: the humanized register applied to the v42 corpus.
...
self-reference words. `verify_v43_style.py` proves it by comparing the numeral, label, citation and table multisets
of v42 and v43, and by reversing every logged edit.
"""
import json
import os
import re
...
def polish_tex_body(src):
    ...
    assert not [c for c in out if ord(c) > 127], 'non-ascii survived into the LaTeX body'
    return head + out + tail, n


# --------------------------------------------------------------------------------- the build
def main():
    os.makedirs(f'{R}/build_v43', exist_ok=True)
    docs = {
        'article': ('paper3_material_ledgers_v42', 'paper3_material_ledgers_v43'),
        'supplementary': ('paper3_supplementary_v13', 'paper3_supplementary_v14'),
        'companionA': ('companionA_certification_procedure_v4', 'companionA_certification_procedure_v5'),
        'companionB': ('companionB_standards_horizon_v4', 'companionB_standards_horizon_v5'),
    }
    LOG = {}
    for key, (old, new) in docs.items():
        ...
        txt2, edits, before, after = sk.polish(txt, keep=KEEP, maxw=(24 if kind == 'article' else 21), emax=(24 if kind == 'article' else 18), kind=kind,
                                 mark_budget=(20 if kind == 'article' else 12))
        out = txt2
    ...
    bt.transpile(f'{R}/paper3_supplementary_v14.md', 'paper3_supplementary_v14')
    print('  supplementary transpiled')
    for base in ('companionA_certification_procedure_v5', 'companionB_standards_horizon_v5'):
    ...
    head = texkit.header_for('Typed Flux Ledgers and Depletion Arithmetic: Conservation, Componentwise '
                             ...
                             'paper3_material_ledgers_v43.md')
    ...
    open(f'{R}/paper3_material_ledgers_v43.tex', 'w').write(tx)
    ...
    for base in ('paper3_material_ledgers_v43', 'paper3_supplementary_v14', 'companionA_certification_procedure_v5',
                 'companionB_standards_horizon_v5'):
    ...
    open(f'{R}/revisions_v43_style_log.json', 'w').write(json.dumps(LOG, indent=1))
    print('\nlog: revisions_v43_style_log.json')
    print('next: build_v43_package.py, then verify_v43_style.py')
