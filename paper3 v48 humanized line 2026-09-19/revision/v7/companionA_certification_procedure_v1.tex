...
\section*{8. The reproduction bundle, and its v2 relabelling}\label{8-the-reproduction-bundle-and-its-v2-relabelling}
...
\texttt{code/MANIFEST.md} at v2 carries the versions, the run command, the relabelling table above and the scope limits, including one that matters for review: no script consumes the G3P basin rows whose provenance is recorded in the supplementary's S5.4, because those rows are quarantined and nothing in the exhibits depends on them.
