#!/usr/bin/env python3
r"""build_v46_base.py - the v46 line: the corpus’s own wording where it can be proved to say the same thing, and the
...
self-reference words. `verify_v46_base.py` proves it by comparing the numeral, label, citation and table multisets
...
def main():
    ...
    docs = {
        'article': ('paper3_material_ledgers_v42', 'paper3_material_ledgers_v46'),
        'supplementary': ('paper3_supplementary_v13', 'paper3_supplementary_v17'),
        'companionA': ('companionA_certification_procedure_v4', 'companionA_certification_procedure_v8'),
        'companionB': ('companionB_standards_horizon_v4', 'companionB_standards_horizon_v8'),
    }
    LOG = {}
    ...
    for key, (old, new) in docs.items():
        ...
        txt2, n_ad, n_seen = sk.baseline(txt, open(CORPUS_MD).read(), keep=_keep, log=humlog, ratio=0.55,
                                         corpus_sents=CORPUS)
        _base = sk.join_blocks([x for x in sk.split_blocks(txt) if x.strip()])
        assert sk.values(txt2) == sk.values(_base), 'the baseline pass moved a verified value'
        counted = [(n_ad, n_seen)]
        ...
        for _i, _b in enumerate(_hb):
            if re.match(r'^#{1,6}\s', _b.strip()):
                _seen_h = True
                continue
            if not _seen_h or not sk.is_flow(_b) or '$$' in _b or '|' in _b or '```' in _b:
                continue
        ...
        assert sk.values(_n2) == sk.values(_base), 'the self-reference map moved a verified value'
        txt2 = _n2
        ...
        print(f'    {n_ad} of {n_seen} flowing blocks are now the draft\u2019s own paragraphs')
        out = txt2
    ...
    bt.transpile(f'{R}/paper3_supplementary_v17.md', 'paper3_supplementary_v17')
    print('  supplementary transpiled')
    for base in ('companionA_certification_procedure_v8', 'companionB_standards_horizon_v8'):
    ...
    head = texkit.header_for('Typed Flux Ledgers and Depletion Arithmetic: Conservation, Componentwise '
                             ...
                             'paper3_material_ledgers_v46.md')
    ...
    open(f'{R}/paper3_material_ledgers_v46.tex', 'w').write(tx)
    ...
    for base in ('paper3_material_ledgers_v46', 'paper3_supplementary_v17', 'companionA_certification_procedure_v8',
                 'companionB_standards_horizon_v8'):
    ...
    open(f'{R}/revisions_v46_base_log.json', 'w').write(json.dumps(LOG, indent=1))
    ...
    ent = LOG['article']
    pre = open(f'{R}/paper3_material_ledgers_v46_prebaseline.md').read()
    ...
    L = ['# The v46 line: the humanized draft as the surface of record', '',
         ...
         f'* {ent["counted"][0][0]} of {ent["counted"][0][1]} flowing paragraphs of the article are now the draft\u2019s text;',
    ...
    L += ['', 'The register is judged as the distance to the draft\u2019s own profile, per 1000 words of flowing prose,',
          ...
          'conflict instead of satisfying the cap; on this text the line is the semicolons.', '', '## Reproduce',
          '', '```',
          ...
          '```', '']
    open(f'{R}/structure_v46_base.md', 'w').write('\n'.join(L))
    print('\nlog: revisions_v46_base_log.json')
    print('note: structure_v46_base.md')
    print('next: build_v46_package.py, then verify_v46_base.py')
