...
*Corrected edition (batch-5, 2026-08-29): the epistemic constraint type is operationalized (an epistemic projection and judgment component are added, as the successor monograph does); the bottleneck diagnostic's slack symbol is made well-typed over the two bound families; the post-arrival horizon symbol of §8.2 is defined; the assume–guarantee discussion carries its citation. Adjudication record: `BATCH5_JOINT_AUDIT_EVALUATION.md`.*
...
- \(\tau_j\): physical, functional, normative, relational, or epistemic type (the epistemic type is operationalized in §6.2A: it contributes the epistemic projection \(K_E\) and the judgment component \(E_\Omega\); the provenance axis \(\rho_j\) below carries the *claim-status* side of epistemics — provenance, evidence, and claim type — so the two axes are orthogonal, one recording what kind of constraint is declared, the other what is currently known about it);
...
### 6.2A Epistemic projection

For the epistemic type define

\[
K_E=\bigcap_{\tau_j=E}\{z:p_j(z)\},
\]

the set of states satisfying the declared evidential-support constraints (a constraint of epistemic type asserts that the claimed guarantee must be knowable and implementable under the declared information and policy classes). Leaving \(K_E\) — insufficient evidential basis for the declared confidence — caps the claim's certification maturity level; it does not change whether the underlying physical trajectory is possible. This operationalization follows the successor monograph's §6.3A.
...
with \(E_\Omega(\tau)\) the epistemic component of §6.2A (unity when the claim for \(\tau\) is certifiable and implementable under the declared information process and policy class).
...
If the reference equals the bound, use an absolute or problem-specific normalization. The bottleneck diagnostic is the minimum over both bound families,

\[
M(t)=\min\Bigl(\min_i\sigma_i^-(t),\;\min_i\sigma_i^+(t)\Bigr),
\]

with \(\sigma_i^-\) the lower-bound and \(\sigma_i^+\) the upper-bound normalization above (a bare \(\sigma_i\) would be ambiguous between the two).
