...
*Corrected edition (batch-5, 2026-08-29): the coupled ecological–governance model's resource balance subtracts \(H_{actual}\) (authorized plus unregulated extraction), restoring the governance-to-resource feedback severed in the committed edition, and the policy vector's harvest entry is declared as the authorized component; the principles-continuity sentence states the two replacements exactly; Lemma 8's cross-reference gains its formal-operator pointer. Adjudication record: `BATCH5_JOINT_AUDIT_EVALUATION.md`.*

The previous manuscript’s twelve principles are carried forward as interface or operational principles rather than undifferentiated axioms — ten of the twelve are retained with type tags; two are replaced to match the typed-dependency architecture (#8, the nested-systems principle, becomes the typed-dependency principle: obligations follow declared support, impact, and normative edges rather than mere nesting, inverting the nested rule; #9, the burden-displacement principle, becomes the burden-allocation principle: a transfer discharges responsibility only if it satisfies the registered allocation and affected-population constraints, converting a conservation statement into an allocation-permissibility rule).
...
If \(a_{\min}(z)>a_{\max}(z)\), no action at state \(z\) simultaneously satisfies the relevant lower and upper bounds. Structural transformation follows only under the stronger reachability condition stated in Section 7.2 (formalised as the reach–avoid–maintain operator of Section 8.2).
...
\[
u=(H_{authorized},A,J_E,J_K,J_G,\theta_1,\ldots,\theta_n),
\]

— the harvest entry of \(u\) is the authorized extraction; the unregulated complement of §23.3 enters the resource balance through \(H_{actual}\), so weak governance feeds back into the resource through the leakage term (with the first edition's symbol \(H\) in \(u\) read as \(H_{authorized}\)).
