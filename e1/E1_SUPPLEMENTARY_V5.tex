...
\title{Supplementary Information (v5): Forecasting biomass under structural non-stationarity — surplus-production model ladder for Northern cod}
...
(v5)}\label{supplementary-information-v5}

\emph{v5 corrects the SI-7 scale-invariance claim (margins are NOT
invariant - optimizer bounds track the rescaled state - but every
verdict is) and adds the q\# sensitivity table. Earlier sections
unchanged.}
...
Script \texttt{campaign\_e1\_survey\_qsensitivity.py}; output committed
as \texttt{results/e1\_survey\_qsensitivity.csv}.
