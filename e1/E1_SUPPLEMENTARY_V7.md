...
## SI-8 Restored full-length passages from the pre-condensation record

The register condensation of the manuscript compressed four interpretive passages that existed in full in the earliest complete carrier. Two pairs are restored here verbatim (mathematical content unchanged) because they remain referee-relevant: the fixed-window narratives (they certify Table 3's and the archived Spec B window scores' reading), and the depensation-boundary / scope-limits observations (they certify how M1b's algebra behaves at data boundaries and what the autonomous-map proposition does not claim). Screened out during same review: the earlier DM "Readings" narrative (superseded by the stronger seeded 20,000-replication layer in SI-3), the implied-maxima and net-production analyses (present in full in the main text), the parameter-estimation and log-RMSE floor narrations (carried by A.2/A.4 and SI-4), and the Edwards Aquifer cross-system paragraph (single cross-reference sentence suffices at this venue). Every numeral in the restored text was machine-verified against the current manuscript, the frozen data/result files, or direct recomputation with the registered engine (e.g., the terminal disclosure's two one-step least-squares minima, $128.35$ vs $127.84$ kt$^2$, reproduce exactly from `run_ladder.py` at the stated constant catches $5.0$ and $3.19$ kt).
