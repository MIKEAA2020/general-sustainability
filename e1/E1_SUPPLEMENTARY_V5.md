# Supplementary Information (v5)

_v5 corrects the SI-7 scale-invariance claim (margins are NOT invariant - optimizer bounds track the rescaled state - but every verdict is) and adds the q# sensitivity table. Earlier sections unchanged._
...
Script `campaign_e1_survey_qsensitivity.py`; output committed as `results/e1_survey_qsensitivity.csv`.
