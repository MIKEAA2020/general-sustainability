...
This supplement supports the main article (Abaee, *Does structural elaboration improve biomass forecasts?*, Northern cod forecast-ladder series). Section numbers are cited in the main text
...
## SI-8 Extended interpretive passages

This appendix carries, verbatim, two pairs of extended interpretive passages that were shortened in the main text for length: the fixed-window narratives (which certify the readings of Table 3 and of the archived Specification B window scores) and the depensation-boundary and scope-limit observations (which certify how M1b's algebra behaves at data boundaries and which claims the autonomous-map proposition does not make). Material considered for this appendix but not reproduced here: the original Diebold–Mariano "Readings" narrative (its content is subsumed by the seeded 20,000-replication uncertainty layer of SI-3), the implied-maxima and net-production analyses (present in full in the main text), the parameter-estimation and log-RMSE floor narrations (carried by Appendix A.2/A.4 and SI-4), and the Edwards Aquifer cross-system paragraph (a single cross-reference sentence suffices at this venue). All numerals in the restored text reproduce exactly from the manuscript's frozen data and result files or from direct recomputation with the registered engine (e.g., the terminal disclosure's two one-step least-squares minima, $128.35$ vs $127.84$ kt$^2$, reproduce exactly from `run_ladder.py` at the stated constant catches $5.0$ and $3.19$ kt).

### 8.1 Fixed-window narratives, Specifications A and B
...
### 8.2 Window numerical disclosure (annual vs. coarse-constant catches)
