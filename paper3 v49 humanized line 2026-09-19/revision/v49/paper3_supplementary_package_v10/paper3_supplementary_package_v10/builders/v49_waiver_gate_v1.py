...
def g1b(base, built, prev, dep_txt, shipped=None):
    ...
    dp, bp, bb, dd = norm(prev), norm(base), norm(built), norm(dep_txt)
    for t in terms:
        lt = norm(t)
        if not lt or lt in bb or lt not in dp or lt in bp:
            continue
        # the counts used to be taken off the wrong surfaces: `in_previous_line` counted the
        # ADAPTATION (so a term v48 carried and the adaptation dropped read as "0 in v48", which said
        # nothing), and the sentence never told the reader where the term went. Both fixed: the counts
        # name their surface, and the whole shipped document is searched, because a term that moved
        # from the introduction into the abstract is not a term lost.
        where = []
        if lt in bb:
            where.append('waived region')
        if shipped is not None:
            sh = norm(shipped)
            if lt in sh:
                where.append('the shipped document')
        out.append({'check': 'G1b', 'severity': 'disclose',
                    ...
                    'term': t, 'in_v48_region': dp.count(lt), 'in_the_adaptation': bp.count(lt),
                    'in_the_deposit': dd.count(lt), 'where_it_is_now': where or ['nowhere in v49']})
    return out


def run(base_txt, built_txt, label, dep_txt=None, prev_txt=None, absorbed='', ruling=(), shipped=None):
    if shipped is None and os.path.exists(f'{V7}/paper3_material_ledgers_v49.md'):
        shipped = open(f'{V7}/paper3_material_ledgers_v49.md').read()
    ...
    f1b = g1b(base_txt, built_txt, prev_txt if prev_txt is not None else base_txt, dep_txt, shipped)
