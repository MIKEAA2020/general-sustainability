...
**Verification.** The mathematical disputes between the attempts were adjudicated numerically — two before consolidation (`reaudit/verify_joint_disputes.py`) and the owner's post-implementation challenge to the E2.B2(a) consolidation upon it (`reaudit/verify_e2b2a_selector.py`); results are reproduced inline in §I.3. All 13 A2 verification suites (444 assertions) and the A1 developments are on file in `batch 4/`.
...
| Distinctive strengths | Form A / Form B framing of B1 (resolves C4); the L_B/L_G separation; strongest-negative reading of E4.Thm2 | Found errors **the audit itself made** (B10 semicontinuity; B9 reverse-inclusion refutation; the E4 witness-gap); the co-Lipschitz companion (E4.Lem1(E)); the uniform-exhaustion hypothesis for B7(1); the E3.C2 Farkas-variant correction; **the constructive Borel selector + Castaing representation for E2.B2(a) — the only attempt to repair the existence half, removing the external KRN citation entirely and diagnosing the recorded '`X` (hence `S`) is Polish' clause as false-and-unnecessary** | Sharp quantitative forms everywhere (moduli, tight radii, reach formula); **two new findings no other attempt caught** (B6 part (2) sign error; the E4 r≡0 vacuity D3); the two-depth B1 theorem; dynamical closure for A3.Thm1; the reparametrized A3 metric |
| Defects found by the joint process | E3.C6.3 (⟸) was graded "PROVEN" — too generous: the landing gap (`x(t_d) ∈ Viab_full`) was missed; the conjecture-demotion was unnecessary; **Finding 5 was graded "closed by the one-line metric decomposition" — the existence half (the Step-4 selector) was left as a KRN citation carrying the recorded '`X` (hence `S`) is Polish' slip, and the first implementation followed that reading into the repaired source file** (caught by the owner; Dispute 3, §I.3) | No per-finding numerical verification; B6 part (2) left untreated; B1 formulated only at the two special depths rather than the general (R, r) pair | **Three mathematical errors**, the first two caught by A1 and adjudicated numerically (§I.3), the third caught by the owner after consolidation: B9(c) union-over-splits "completeness" is false; B10(1) "both values attained" is false for the pessimistic value; E2_B2A's disposition "Steps 1, 2 and 4 are correct as written" is wrong about Step 4 — "`X` and `S` are Polish" is false for a general measurable `S` (Dispute 3, §I.3) |
...
5. **E2.B2(a) Step 3** (weak measurability) is closed by the one-line metric decomposition `O = ⋃ₙ{dist(·, U∖O) ≥ 1/n}` — this much is common to all three attempts. The **existence half** (the Step-4 measurable selector) is *not* closed by it: the one-liner produces a measurable *set* (`A_W^-(O)`, an `F_σ`), not a *function*. A1 alone constructs the selector (nested-vanishing-diameter Borel construction) and diagnoses the recorded '`X` (hence `S`) is Polish' clause as false for a general measurable `S` and unnecessary (KRN's Polish hypothesis is on the codomain); M and A2 stop at the KRN citation (Dispute 3, §I.3).
...
## I.3 The adjudicated disputes (A1 vs A2, and the owner's challenge to the consolidation), settled numerically
...
### Dispute 3 — E2.B2(a): does the one-line `F_σ` decomposition close the theorem? (owner's challenge to the consolidation)

The consolidated register first read: "E2.B2(a): PROVEN (repaired — one-line metric decomposition; conclusion unchanged)", and the repaired source file's Step 4 was carried over verbatim: "`X` (hence `S`) is Polish, `U` is Polish, … KRN yields a Borel-measurable `u*`". The owner challenged exactly this: *the one-liner repairs weak measurability; the selector still has to be produced; if the register cites only the one-liner, the existence half is again a citation of KRN with the domain-Polish slip the recorded proof already had*.

**Verdict: the challenge is correct — A1's treatment is the only complete one, and the first consolidation had undersold it** ("A1's extras adopted as remarks" — the constructive selector is not a remark, it is the existence half). The slip: `X` is Polish (compact metric), but a general **measurable** `S ⊆ X` need not be — a subspace of a Polish space is Polish in the relative topology iff `G_δ` (`ℚ ∩ X` is Borel, meager, dense, hence not `G_δ` by Baire, hence not Polish) — and the clause is **unnecessary**: KRN's Polish hypothesis is on the **codomain**, the domain being an arbitrary measurable space. M's blueprint graded the finding "closed by the one-liner"; A2's dossier asserts "Steps 1, 2 and 4 are correct as written" with "`X` and `S` are Polish" — both stop at the citation. A1's dossier (§7–§9) constructs the selector: nested correspondences of vanishing diameter, `i_n(x) := min{i : G_n(x) ∩ B(q_i, 2^{-n}) ≠ ∅}`, `G_{n+1} := G_n ∩ B̄(q_{i_n}, 2^{-n})`, singleton intersection, `u*` = uniform limit of the countably-valued Borel maps `g_n = q_{i_n(x)}` — no external selection theorem — plus the Castaing family `σ_{jk}` dense in `A_W(x)`, which is what Filippov-type approximation arguments actually consume. **Adjudicated numerically** (`reaudit/verify_e2b2a_selector.py`: the construction runs on a concrete compact-valued correspondence, the diameter bound `2^{1-n}`, membership `u*(x) ∈ A_W(x)`, the uniform rate `|u* − g_n| ≤ 2^{-n}`, interval-valued (hence Borel) pieces, Castaing density, and — decisively — the identical construction running on a non-Polish measurable domain `S`, since no step queries anything about `S` beyond trace-measurability). **Consolidation (applied):** the source file's Step 4 now carries the construction; the recorded clause is struck as false-and-unnecessary; KRN is restated correctly (Polish codomain, measurable domain) as a remark. See Finding 5.
...
**Reading (A1).** The original conflated *tangential* feasibility (`d ∈ T_𝒢(x̄)` — feasible curves with tangent `d`, possibly leaving the ray) with *ray* feasibility (`x̄ + sd ∈ 𝒢` for small `s`). The witness's `d = (1,0)` is in `T_𝒢(x̄)` yet the ray leaves `𝒢` immediately. Downstream consumers must say which they mean; E3.C2/E7.Thm2 consume the *strict local alternative* and the linear case, both intact. *Post-audit completion:* the ray half of A1's localized dichotomy — strictly feasible `d` is ray-feasible with explicit radius `t*` (`ω(t*) ≤ η/2`; for `Λ`-Lipschitz `∇g`, `t* = min(r_in, η/(2Λ))`) — is carried in the source file as clause (1)(e).
...
**Disposition.** The margin definition admits vacuous pairs (`b ≥ ℓ·r̄_g` makes the inclusion read as `K_{g+1}` at every tested `r`), so the recorded "no uniform `(ℓ, b)` with `b < ∞` exists" is false of *every* `K`-preserving family. Consolidated repair = non-vacuity + A2's inradius-extending witness + A1's (B) and (E). *(Owner's post-implementation challenge verified: the consolidated witness is the inradius-extending family — the recorded vanishing-neighbourhood family with non-vacuity alone would NOT suffice, per A1's (B); the implemented repair — source file, register row, and the N5/N7 numerical suite — carries the correct combination.)*
...
**Status.** E4.Lem1: `PROVEN (repaired — non-vacuity hypothesis b < ℓ·r̄_g added; witness extended to the inradius; co-Lipschitz+exterior companion recorded)`. *Post-audit completion:* item (iv) (A1's (F)) is carried in the E4 source file's statement and proof, together with the Lipschitz-upper-bound/margin-lower-bound asymmetry note.

---

## Finding 5 — E2.B2(a) (Class 2: Step-3 gap; Step-4 citation slip) — CONSOLIDATED (corrected per Dispute 3)

**Disposition.** Step 3 proved the closed-set statement and asserted the open-set one KRN requires — that gap is closed by the one-line metric decomposition, and this much is common to all three attempts. But the one-liner repairs **weak measurability only**: it produces a measurable *set* (`A_W^-(O)`, an `F_σ`), not a *function*. The existence half of B2(a) — the measurable selector — is a separate deliverable, and there the attempts differ: M graded the finding "closed by the one-liner (conclusion unchanged)" and A2 declared "Steps 1, 2 and 4 are correct as written" — both leave the selector as a citation of KRN whose recorded premise "`X` (hence `S`) is Polish" is **false** for a general measurable `S` (a subspace of a Polish space is Polish iff `G_δ`; `ℚ ∩ X` is Borel and not Polish) and **unnecessary** (KRN's Polish hypothesis is on the *codomain*; the domain is an arbitrary measurable space). A1 alone repairs the existence half: a constructive Borel selector with no external selection theorem, plus a Castaing representation. The first implementation of this consolidation followed the weaker (M/A2) reading and carried the slip into the repaired source file; the owner caught it (Dispute 3, §I.3); the source file now carries A1's construction.

**Consolidated repair.** (i) *Weak measurability (Step 3):* for open `O ⊆ U`, `O = ⋃ₙ Fₙ` with `Fₙ = {y : dist(y, U∖O) ≥ 1/n}` closed, `Fₙ ↑ O`; hence `{x : A_W(x) ∩ O ≠ ∅} = ⋃ₙ {x : A_W(x) ∩ Fₙ ≠ ∅}` is `F_σ`. The **metric** hypothesis is exactly what is needed (in a general regular space open sets need not be `F_σ`) — stated rather than left implicit. (ii) *Selector (Step 4 — A1's (E), applied to the source file):* with `{q_i}` countable dense in `U`, set `G_1 := A_W|_S` and inductively `i_n(x) := min{i : G_n(x) ∩ B(q_i, 2^{-n}) ≠ ∅}`, `G_{n+1}(x) := G_n(x) ∩ B̄(q_{i_n(x)}, 2^{-n})`; the pieces `{i_n = i}` are measurable (Step-3 machinery), `diam G_{n+1} ≤ 2^{1-n}`, the nested nonempty compacts intersect in the singleton `{u*(x)} ∈ A_W(x)`, and the countably-valued Borel maps `g_n(x) = q_{i_n(x)}` converge **uniformly** to `u*` (a uniform limit of Borel maps is Borel). No external selection theorem is invoked; the classical KRN, correctly stated (Polish **codomain**, measurable domain), is retained as a remark — the construction is that theorem's classical proof made explicit. (iii) *Castaing representation (A1's (F)):* applying the same construction to `Γ_{jk}(x) := A_W(x) ∩ B̄(q_j, 2^{-k})` when nonempty (else `A_W(x)`) yields Borel selectors `σ_{jk}` with `{σ_{jk}(x)}` dense in `A_W(x)` — the object Filippov-type approximation and convexified-envelope arguments consume. (iv) *Honesty note (A1's):* B2(a) does **not** close R02 Field 12 (a different correspondence — inflating it would be a false promotion). (v) *Effective domain (A1's (B), post-audit completion):* `S* = {x : A_W(x) ≠ ∅}` is closed, hence Polish — the natural domain of the selector problem is Polish while an arbitrary measurable `S ⊆ S*` need not be, and need not be; carried in the source file as a remark. (vi) *Vietoris–Borel (A1's (D), post-audit completion):* `x ↦ A_W(x)` is Borel measurable into `𝒦(U) ∪ {∅}` (`∅` isolated) — hits of open sets are `F_σ` by (i), containments are complements of closed-set hits by compactness; carried in the source file as a remark.

**Status.** E2.B2(a): `PROVEN (repaired — Step 3 by the metric F_σ decomposition; Step 4 selector CONSTRUCTED (nested-vanishing-diameter Borel construction) + Castaing representation; the recorded 'S is Polish' clause struck as false-and-unnecessary; KRN restated with the codomain-Polish hypothesis only)`.

---

## Finding 6 — E2.B1(a) (Class 2: backwards inheritance) — CONSOLIDATED

**Consolidated repair.** Knaster–Tarski core retained. The sentence "consistency is inherited by subfamilies" is **false** (A2's 2-point counterexample `Γ({1}) = {2}`; A1 and the audit give 3-point witnesses — all correct). Post-fixed sets are closed under **joins**, not subsets. Correct transfer: `Γ(C) ⊆ Γ(𝒱*) = 𝒱*` for every `C ⊆ 𝒱*`, so `R02.Thm1` applies to `𝒱*` itself and the recursion may be *started* from any `C ⊆ 𝒱*` provided certificate states are *tracked in `𝒱*`*. (REG)(ii) makes a smaller family *harder* to certify — `𝒱*` imposes the weakest closure obligation while containing every reachable certificate state. *Post-audit completion:* A1's complementary **downward-(REG)-transfer lemma** (on downward-closed families with monotone implementation, (REG) passes to closed subsets with the same witness — the inheritance the recorded sentence was reaching for, running through R02's own hypothesis rather than Γ-monotonicity) is carried in the source file as a remark, and the source's *Statement* — which had been left carrying the refuted "applies to every subfamily" clause while the proof carried the repair — is now corrected to match.
...
**Consumer note.** Paper 2 should cite (a) for the characterisation and (b) for certificates; it must not cite a fixed-split (or union-over-splits) equality. *Post-audit completion:* the consumer note is carried in the source file's restriction honesty.
...
- The C-e sandwich keeps the sharp outer bound `{B ≥ Φ⁻_T}` (the recorded `{B ≥ Φ⁺_T − Φ⁻_T}` repeats the F⁺-for-F⁻ substitution of Finding 16 and is weaker than the proof). *Post-audit completion:* the C-e source file's *Statement* display — which the first application had left carrying the struck outer bound and the opposite `B`-convention (`xᵀMx − c`) while clause (iv) and the register row carried the sharp form — is reconciled to `B = c − xᵀMx` and the sharp conditional outer bound (with the general admitted-class form `D̃_T`).
...
| E2.B2(a) | PROVEN (reconstructed) | PROVEN (repaired: Step-3 F_σ decomposition; Step-4 selector constructed + Castaing; KRN codomain-Polish only) | F5 |
...
**Implementation completion and obligations executed (2026-08-26, post-Task-26 audit).** A full sweep of the three attempts' surviving points against the repaired source files (session worklog Task 27; three parallel cross-checks covering A1's seven dossiers, A2's fifteen REPAIRED dossiers, and the computation artifacts) confirmed the 27-finding consolidation complete at source level, and found a residue of adopted-but-unlanded and half-applied items — all now landed, one commit each: the E2.B1(a) *Statement* still carried the refuted "applies to every subfamily" clause (the Finding-6 repair had reached the proof and the register but not the statement); the C-e *Statement* display still carried the struck outer bound and the opposite `B`-convention (Finding 13 half-applied); A1's E4.Lem1 (F) honesty clause (Finding 4(iv)) had been adopted here without landing in the E4 source; A1's E2.B2(a) effective-domain and Vietoris–Borel results, A1's downward-(REG)-transfer lemma, A1's B6 ray lemma, and A2's B1 value-over-Lemma-2 note, B9 consumer note, and B10 "iff"-necessity argument are landed in their source files. The deferred grep obligations are **executed** (this date): `b/(1−ℓ)` and "geometric budget" — zero live hits (every occurrence is a repair document's own corrective citation or a session record); the A4 positive-α sweep over `revised_articles/` and the manuscripts — zero live hits (live erosion statements already carry the margin convention); E7's `L_G = 0` / `D⁻_T − F⁺_T` sweep — zero live hits. The completed computations require **no** correction: all 15 reaudit suites reproduce the documented post-repair reading; Wave E pinned hashes verify 30/30; the E5 admission script uses the correct erosion sign; the B4 prefactor-aware artifacts stand.
