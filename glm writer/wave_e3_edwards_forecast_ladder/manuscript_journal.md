...
% RUNNING: Scored forecast evaluation, Edwards Aquifer J-17
% KEYWORDS: viability; forecast evaluation; Edwards Aquifer; J-17; model ablation; persistence
% CONTRIBUTION: A preregistered, scored forecast evaluation on the 1934–2023 J-17 index-well record finds that no causal module beats a simple AR(1) or persistence on out-of-sample RMSE at short horizons, with the information-layer rent quantified by an oracle benchmark.

# Abstract

This evaluation follows a preregistered retention rule: additional model structure is retained only when it improves early warning, out-of-sample prediction, or intervention selection. This paper applies that test to the San Antonio Pool of the Edwards Aquifer, Texas.
...
One-year rolling root-mean-square error (RMSE) is 13.23 ft for last-value persistence and 12.84 ft for a univariate AR(1); the AR(1) is retained as an output-only model, by a margin of 0.39 ft. The causal stock-flow specification, which persists the most recent recharge and pumpage, has RMSE 14.70 ft and is rejected. A climatological stock-flow variant reaches 12.28 ft but is listed only and then declined on class grounds: under constant fluxes it collapses to an affine AR(1), so its numerical advantage is not extra structure. Residual and delayed variants do not improve on persistence. Recharge has lag-1 correlation 0.17; using last year's recharge as next year's recharge therefore adds variation that is only weakly related to the increment being forecast. When the same map is given realized future recharge and pumpage, RMSE falls to 7.55 ft — an information-layer rent of nearly half the persistence error that no signal available at the annual origin recovers. Climate-informed recharge forecasts known at the origin — September–November Niño 3.4, lagged climate-division precipitation, and their combination — lie within 0.02–0.13 ft of AR(1) at the one-year horizon, AR(1) on recharge itself being 0.4 ft worse, and all four have higher RMSE than persistence at the five-year horizon (where training-mean climatology, 16.80 ft, beats persistence, 21.11 ft). Contemporaneous precipitation tracks recharge ($r=0.78$), but next year's precipitation is not known at the forecast origin. ENSO and last year's precipitation are not substitutes for it.

No two-pool, solute, or institutional-kernel claim is made. The evaluated specification is a one-pool approximation of the aquifer's head map. The two-pool exchange module remains conditionally admissible and is not closed here.

# 1. Introduction

Forecasting aquifer heads at index wells is a recurring operational problem of groundwater management: drought-stage declarations, springflow protection, and permit adjustments all key on forecasted water levels. The methodological literature has responded with an increasingly rich inventory of groundwater-level forecasting models, from conceptual water balances to data-driven regressions and, prominently, artificial-neural-network and wavelet–neural-network conjunction models trained on measured heads (Daliakopoulos et al. 2005; Adamowski and Chan 2011). Whether added structure improves out-of-sample forecasts — as opposed to in-sample fit — is a separate, testable question, and the general forecasting literature has made the benchmark discipline explicit: across the M4 competition's 100,000 series, sophisticated methods did not uniformly beat simple statistical baselines (Makridakis et al. 2020). The minimal bar for any proposed module is therefore the forecast that nothing changes — last-value persistence — together with the training mean.

This evaluation follows a preregistered retention rule: additional model structure is retained only when it improves early warning, out-of-sample prediction, or intervention selection. The instrument is a scored model-ablation framework — a ladder of nested water-balance models evaluated against the two naive baselines. Complexity is kept only if it improves a preregistered score, decided on out-of-sample error on the predictand itself.

We selected the Edwards (Balcones Fault Zone) Aquifer J-17 index well because its long (1934–2023) daily head record, its explicit institutional thresholds, and its management relevance make it a natural test bed for scored forecast evaluation. The San Antonio Pool passes each design criterion the evaluation requires: a measured basin state, independent recharge, dated implementable use, a declared threshold, and a measured service series. The aquifer is also a karst system in which regional flow has long been represented — and debated — through equivalent porous media and lumped approaches (Scanlon et al. 2003), which makes the fate of a deliberately simple one-pool water-balance map a live hydrogeologic question rather than a straw man.
...
A companion study under separate review applies the same scored design to a marine fishery stock (Northern cod, NAFO 2J3KL); the two systems' scores are never pooled, and no retention verdict is transferred between them. The paper does not assess whether the aquifer is sustainable, does not close the two-pool exchange module's blocking list, and does not treat springflow or reconstructed storage as co-primary predictands. No solute or water-quality module is opened.

The remainder of this article is organized as follows. Section 2 states the data and the specification. Section 3 defines the one-pool map and the model ladder. Section 4 states the evaluation design — windows, rolling origin, scores, and the retention rule. Section 5 reports the results: the series (Section 5.1), the fixed windows (Section 5.2), the rolling origin (Section 5.3), climate-informed recharge (Section 5.4), and the service series scored after the retention freeze (Section 5.5). Section 6 discusses the findings and their limits, and Section 7 concludes. Data and code availability and the references close the paper.

# 2. Data and specification

Table 1 types each field as follows: D, data; E, empirical construct; M, model; N, normative threshold.

**Table 1.** Specification $\Omega_{\mathrm{SA}}$ — the San Antonio Pool specification: J-17 annual-mean elevation, 1934–2023.
...
The scoring protocols for Pass 1 and Pass 2 were frozen and dated (2026-08-25) before the corresponding RMSE tables were computed; the frozen protocol documents are archived with the analysis code. The design is a fixed computational protocol rather than a prospective clinical-style registration.
...
| M2\_Renso | 12.82 | 24.42 | 528 |
| M2\_Rprecip | 12.80 | 25.38 | 545 |
...
M2\_Rar loses at $h=1$ (13.25 ft; 0.41 ft worse than M1). Autoregression on nearly white recharge is not a recharge forecast.
...
![Figure 5](fig5_fibre.png)

**Figure 5.** Comal annual mean versus J-17. Contemporaneous $r=0.986$. The service series — the observation fibre, a measured channel of the same state — is a measured service, not an independent information source.
...
The evaluated specification is a one-pool approximation of the aquifer's head map. It does not transfer an interval-verified linear template (a companion methodological study; that template is not this specification), does not instantiate a closed-loop information filter, and does not close the two-pool module. Model selection used $z$ only. No result is treated as established without specification matching and independent verification.
