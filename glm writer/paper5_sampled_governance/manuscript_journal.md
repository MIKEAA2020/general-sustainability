% TITLE: Sampled Governance, Empirical Identification, and Falsification Design: The Sample-and-Hold Model of Periodic Review, Spectral and Power Screens, and the Northern Cod Case at Its Exact Causal Status
% VENUE: ICES Journal of Marine Science
% TYPE: Methodology and case study
% RUNNING: Sampled governance and falsification design
% KEYWORDS: fisheries governance; sample-and-hold; aliasing; empirical identification; falsification; Northern cod
% CONTRIBUTION: This article models periodic assessment-review cycles as sample-and-hold control, screens 42 fish stocks for review-band spectral signatures, places the northern cod collapse at its exact causal status, and specifies falsifiable prospective designs.

# Abstract

Fisheries management runs on periodic review — assessments, decisions, and held catch or effort controls at discrete times — and this article treats that cadence as a sampling-control problem. The representation is a sample-and-hold model: between reviews a logistic stock evolves under held effort, a filtered nonnegative deficit signal carries institutional memory, and a projected explicit effort update (the extractive controller SD-E-B3, an explicit controller discretisation, not a generic harvest-control rule) resets effort at each review, with assessment separated from observation and latent state. The sampled state space is forward invariant (positivity, signal nonnegativity, effort admissibility, by induction over review intervals). The rapid-review limit is a finite-horizon consistency statement (an $O(T_r)$ Gronwall-type estimate) establishing neither stability at any positive review interval nor the reported response regions. Delayed-recruitment calculations, classified from long-horizon trajectories rather than Poincaré-map multipliers, locate exploratory response regions near 3–4 yr (anchovy-class) and 6–12 yr (sprat-class) review intervals, with cod-class convergence over the tested 1–20 yr grid and slow-stock transitions near 30–50 yr. A multiplicity-controlled Lomb–Scargle screen of a 42-stock annual-review cohort (RAM Legacy v4.66; AR(1) red-noise null; Benjamini–Hochberg FDR control) found no robust target-band peak, and injected-signal power on 100–200 yr synthetic records is high only in favourable noise and record-length regimes (sprat-class power 1.0 at $\sigma=0.1$ but 0.24–0.58 at $\sigma=0.3$; anchovy-class 0.02–0.14). A structured search across more than thirty systems returned zero eligible cases under four confound-exclusion criteria. The northern cod case (NAFO 2J3KL) is carried at its exact causal status: the positive result is the two-window split — crash interpretation formulation-dependent, non-recovery unexplained in both formulations — with the scalar-autonomous phase-line obstruction as the mathematics. Five falsification criteria and five prospective designs — governance-event panels, quasi-experimental timing, out-of-sample mechanism comparison, randomized human-in-the-loop experiments, and closed-loop management strategy evaluation — complete the falsification programme.
...
Fisheries governance is sampled governance. A stock assessment is produced at discrete times; a management body meets and decides at discrete times; and the resulting total allowable catch, effort control, or harvest rule is held — often for a year or more — until the next assessment cycle resets it. The control engineering of such loops is well developed: sampling a feedback loop changes its dynamics, a sample-and-hold controller has its own spectrum, and the interaction between a review cadence and a resource's response timescale — the sampling-control analogue of aliasing — can relocate or delete boundaries that a continuous idealization would place elsewhere. Yet in fisheries advice the review interval is usually a scheduling fact rather than a modelled object, and the causal status of retrospective collapse narratives is rarely pinned to what the data can identify. This article treats the review cycle as a sampling-control problem and builds the identification and falsification layer that follows.

Two failure modes motivate the question. The first is *architecture substitution*: a continuous delay differential equation assumes a delayed signal entering a continuously evolving control law, while many real institutions observe or assess a resource at discrete times, choose a rule-based action, and hold that action until the next review; replacing one architecture by the other can move or delete stability boundaries, so the governance architecture itself must be tested, not merely re-parameterised. The second is *causal promotion*: spectral peaks, cohort cycles, and crisis-driven monitoring records circulate as evidence for institutional-delay mechanisms when they carry no controller-sign information at all. This article builds the layer that resists both failures: the review interval is treated as an operator with its own spectrum, every diagnostic is typed, and every empirical record carries its evidentiary level with its restrictions on the line. The observation and epistemic layer (§4) fixes, at the level of principle, when observation-based certification is possible at all and when information that arrives too late is operationally equivalent to no information — governance implications that bound what adaptive review cycles can certify before any data analysis begins.

## 1.2 What enters this article

This article is the empirical-identification and falsification-design paper of a five-paper research programme. Its retained set consists of the 57 statement-inventory rows routed to it by the programme's destination allocation: the periodic-review source's model family, response regions, robustness and spectral diagnostics, screen, power analysis, and case search (23 rows, the empirical core); the northern cod case (10 rows); the eligible adaptive-capacity and distributive-measurement material (6 rows); the dimensionless identifiability chart and the empirical screen of the unified applied source (2 rows); the observation and epistemic layer (15 rows across five sources — 4, 3, 5, 1, and 2 rows); and the sampled-review variant registration (1 row). Per-statement provenance keys link every inventory-sourced statement to a 409-row statement inventory (source location, canonical module, mapping type, evidence status, destination); the keys themselves are carried in Appendix A. Six manuscript-native entries carry the article's own local content: the governance-time ontology, the forward-invariance (positivity) proposition, the SD-E-B3 instance of the finite-horizon consistency statement, the four prospective identification designs, the closed-loop management-strategy-evaluation design, and the falsification criteria (§9). Four further manuscript-native entries restate source-declared content that carries no inventory row: the architecture-level empirical hypotheses (§5.2), the assessment-table values of the cod case's two windows (§6.3), the alternative-mechanism enumeration behind the case-search criteria (§6.1), and the fixed-$\tau$ idealisation caveat (§7.1).

## 1.3 Claim-status discipline

Every statement below carries a status label from the research programme's hierarchy (the canonical source's own table, adopted programme-wide):
...
Two rules govern this article. **No promotion:** a conditional theorem is never stated as a theorem; an exploratory response region is never stated as a stability boundary; an arithmetic transformation is never stated as an independent observation; a formulation attribution is never stated as a causal claim. **No silent transfer:** a status proven for one model class does not transfer to extensions, reductions, or applications without a declared map, and the diagnostic no-transfer rule of the architecture is binding throughout — **a diagnostic is not a causal claim**.

## 1.4 Provenance and auditability

All 57 of this article's inventory-sourced statements are closed at content level: each was verified against its source manuscript in a dated full-read campaign (the architecture, canonical, and periodic-review sources on 2026-08-27; the remaining seven sources on 2026-08-28), with per-statement confirmation of existence, kind, proof presence, module, and mapping type. They are stated below at exactly those statuses, with no promotion. Content-level acceptance means the statement's existence, kind, proof presence, module, and mapping type were verified against the source; it is not a theorem-status promotion, and the cross-module interface contract remains an open obligation recorded per row. Of the 409-row statement inventory, 354 rows are content-verified; the 27 rows that remain open are exactly the three conditional-paper sources — none of them behind this article.

## 1.5 Relationship to the companion studies

This article is one of five companion papers of a research programme, prepared under separate review. One companion owns the typed canonical architecture, including the diagnostic types and their no-transfer rule; this article carries a Minimal Working Realization of the observation–governance objects it needs and never transfers a status across modules. Another companion owns the theorem atlas: the sampled, hybrid, and information-state kernel family and the observation and epistemic-viability family are stated and proved there, and where this article needs one of those objects it states the local instance and cross-references the owning entry — the atlas is the map, this article owns the empirical identification and falsification layer. A third owns the conserved-material ledgers, the componentwise depletion diagnostics, and the worked fisheries ledger cases; the spectral screen's input cohort is an analysis-side criterion, not a ledger example, and the boundary is stated at the screen (§5.3). A fourth owns the named retarded systems, their bifurcation analysis, and the delayed-recruitment registration object; the response-region outputs read here ride that registration at its declared status (§3.4). The research programme's four scored empirical manuscripts (two forecast ladders and two intervention-selection legs on the northern cod and Edwards Aquifer systems) are outside this article's scope; they enter as context at the cod case (§6.3) and in the admission-standard discussion (§9). No paper depends on another for a locally load-bearing definition: the sampled model, its conventions, the screen, and the designs are stated in full here.

## 1.6 Roadmap

The remainder of this article is organized as follows. §2 states the sample-and-hold model of periodic review: the governance-time ontology, the process, observation, and assessment equations, the review map and the controller family, and the registration conventions. §3 establishes forward invariance of the sampled process, states the finite-horizon rapid-review consistency result and what it does not establish, fixes sampled-data stability terminology, records the trajectory-classified response regions and the robustness and diagnostic spectra, and states the two-operator discipline. §4 carries the observation, implementation-delay, and epistemic layer: the observation-fibre certification limit, information monotonicity, the informational failure modes, the ideal benchmark and the epistemic-institutional kernel, the value-of-information templates, the model-object/proxy discipline, and an open problem. §5 states the dimensionless identifiability chart, the empirical hypotheses with declared tests, the cross-sectional spectral screen, and the power and detectability analysis. §6 reports the structured case search with its zero count and carries the northern cod case at its exact causal status, including the phase-line obstruction and the two-window split. §7 specifies the five prospective designs. §8 states the distributive and adaptive-capacity constraints where reproducible. §9 declares the falsification standard. §10 records provenance, reproducibility, and limits. Appendix A carries the statement inventory in two tables; the references and the data and code availability statement close the article.
...
The model's declaration layer separates the time objects of governance:

| Object | Definition |
|---|---|
| Observation interval $T_\mathrm{obs}$ | Time between raw measurements |
| Assessment interval $T_\mathrm{assess}$ | Time between formal state estimates |
| Review interval $T_r$ | Time between opportunities to change the command |
| Decision lag $\tau_\mathrm{dec}$ | Assessment completion to formal decision |
| Deployment lag $\tau_\mathrm{dep}$ | Decision to implemented change in extraction pressure |
| Ecological response lag $\tau_\mathrm{eco}$ | Implementation to detectable ecological response |
...
Let review times be $t_n=nT_r$. Between reviews, extractive effort is held at $E_n$ and the resource stock follows the logistic process under held effort:
...
$$
\widehat Z_n=\mathcal A_n\!\left(\{Y_j:t_j\le t_n-\tau_\mathrm{dec}\}\right),
\qquad
...
The review map is the projected explicit step:

$$
E^\mathrm{cmd}_{n+1}=\Pi_{[0,E_{\max}]}\left\{E_n+T_rF_B(E_n,\widehat Z_n)\right\},
\tag{4}
$$
...
\left[
\eta E\left(\frac{Z}{\Delta_\mathrm{ref}}-\frac{E}{E_{\max}}\right)
+\delta_0\frac{Z}{Z_\mathrm{ref}+Z}
\right],
$$
...
Equations (1)–(4) with $\Phi=\Phi_k$ constitute the extractive controller **SD-E-B3** — the sample-and-hold counterpart of the M3-B extractive mobilisation law of the registered delay family (the delay-dynamics companion owns the continuous family). It is an explicit controller discretisation, not a generic harvest-control rule and not a model of protective quota reduction. Alternative controllers receive distinct identities: **SD-P** (protective update, response to decline entering with the opposite sign), **SD-F** (fixed multi-year plan with no state-responsive update between scheduled resets), and **SD-H** (hybrid controller with limits on annual change, emergency triggers, or legal overrides). SD-P, SD-F, and SD-H are declared comparators for the prospective management-strategy evaluation (§7.5), not completed numerical experiments; a fixed plan and a state-responsive plan with the same nominal review period are not the same intervention.

## 2.4 Registration conventions and the sampled representation as the default

The deterministic record fixes a one-step flow-then-update convention: $E_n$ is held on $[t_n,t_{n+1})$ and the assessment indexed by $n$ defines the next command; assessments are contemporaneous and exact, $\tau_\mathrm{dec}=\tau_\mathrm{dep}=0$, no decision or deployment queue is present, and multiplicative assessment error enters only the designated robustness experiment; a positive $\tau_\mathrm{dep}$ requires an explicitly registered hold or interpolation rule. These are registration requirements declared by the source and not discharged: the solver configuration and initial histories are not attached, and until the computational record is complete the stage-output values carry exploratory status rather than the status of reproducible numerical propositions (§10).

The sampled-governance representation is the default institutional form of the research programme's institutional-feedback model class: at review times institutions observe data and update prescriptions, and the implemented control is held or dispatched by a stated rule until the next review. The registered bridge discipline is binding: a continuous-delay approximation to the effort equation is admissible only after specifying the review interval, hold rule, implementation lag, and approximation error — a sampled map can exhibit flip, Neimark–Sacker, or border-collision behaviour not captured by continuous-delay Hopf terminology. The sampled-review variant itself is a registered obligation at the stress-test registry's no-artifact status (the delay-dynamics companion owns the registry; the obligation is carried here). The formal safety objects for this architecture — the sampled robust-viability kernel and the inter-sample-safe held-control tube kernel — are the theorem-atlas companion's; this article states the model and its conventions, and the kernel constructions are cited, not reproduced.
...
The companion statement for kernels is the information-refinement monotonicity: if $\mathcal I_1$ causally refines $\mathcal I_2$ while dynamics, authority, implementation, uncertainty class, and safety constraints are unchanged, then $\operatorname{IViab}^{\mathcal I_2}_T\subseteq\operatorname{IViab}^{\mathcal I_1}_T$ — a controller with finer information can ignore it and implement a coarser-information strategy. The inclusion requires the two kernels represented in a common physical initial-state space or a declared map between belief-state spaces; the fuller information-refinement theorem of the primary source is the canonical form (the atlas companion owns it). For the sampled-governance layer these are the formal counterparts of the review cadence question: coarsening the information structure (longer $T_\mathrm{obs}$, delayed assessments) can only shrink what is achievable, and by exactly how much is a kernel-level question, not a parameter drift.
...
**Example (hidden-mode conflict).** Let an unobserved parameter satisfy $\theta\in\{-1,+1\}$, with $\dot z=\theta u$, $u\in\{-1,+1\}$, $z\ge 0$. At $z=0$: if $\theta=+1$ only $u=+1$ is safe; if $\theta=-1$ only $u=-1$ is safe. Both states are individually robustly viable, but $B=\{(0,+1),(0,-1)\}$ admits no common safe action, so $B\notin\operatorname{ERViab}$. This is a purely informational failure — no stochasticity or estimation quality is involved — and it is an impossibility witness for common-action viability, not a positive specialisation.

**Theorem (delayed-information obstruction).** If every possible action at belief $B_0$ allows a disturbance and a compatible state to reach a locally nonviable boundary point before the next informative observation time $T_\mathrm{obs}$, then $B_0\notin\operatorname{ERViab}$. A sufficient condition is the existence of a constraint function $q$ and $\varepsilon>0$ with $\inf_{x\in B_t}\inf_{d}D^{+}q(x;f(x,u,d))\le-\varepsilon$ throughout an uncertainty branch and $T_\mathrm{obs}>\inf_{x\in B_0}q(x)/\varepsilon$. Information may be accurate but arrive too late.
...
**Proposition (scalar-autonomous phase-line obstruction).** A nonconstant solution of a locally Lipschitz scalar autonomous ODE is monotone between equilibria and cannot cross an equilibrium in either direction. Consequently, an exact path that repeatedly rises and falls across a common interval is incompatible with any single fixed scalar autonomous model.
...
The case's positive result is a descriptive partition: **the exact data split the phenomenon into two events — the crash interpretation is formulation-dependent, and the non-recovery is unexplained in both formulations. The split is the positive result, not a new mechanism.**

The crash window (1991–1995) is documented by the assessment table (DFO CSAS SAR 2016/026, Table A2): spawning stock biomass 735, 382, 101, 31, 10 kt across 1991–1995 with estimated natural mortality 1.002, 2.214, 2.575, 2.331, 0.288 yr⁻¹ — the crash-window mortality 2.2–2.6 is roughly ten times pre-collapse levels. These are rounded renderings of the assessment table (the underlying values are 381.95, 101.05, 30.55 kt, and so on), and the survival column is $\exp(-M)$ after rounding — a transformation of the reported instantaneous mortality estimate, not an independently observed survival series.

The interpretation of the crash is a formulation attribution, not a causal claim: the NCAM M-shift formulation allocates most estimated mortality to natural death — NCAM's $M$ is an estimated unobserved-death component conditional on model structure, and assessment-framework proceedings explicitly caution that unreported fishing deaths may enter de-facto $M$ — while the constrained-M formulation attributes the crash to unreported catch. The constrained-M quantities (crash window $M=0.46$, $F=1.37$, unreported catch 257.8 kt yr⁻¹ — 102.5% of mean spawning biomass; non-recovery window $M=0.43$, $F=0.25$, 3.7 kt yr⁻¹) are unreproduced: they are reproduction targets requiring equations, source series, code, units, windows, and uncertainty, registered on the open-problem docket and stated here as hypotheses, not results.

The assessment table's later rows give the two windows their empirical grounding (DFO CSAS SAR 2016/026, Table A2): in the non-recovery window the estimated spawning stock biomass is 16.05 kt (1996), 34.42 kt (2000), and 20.07 kt (2004), with estimated natural mortality 0.341, 0.717, and 0.362 yr⁻¹ — low and non-monotonic, rising and falling across a common interval, the repeated direction reversal of the §6.2 obstruction — and in the recovery window 25.18 kt (2005), 96.91 kt (2010), and 298.65 kt (2015), with natural mortality 0.288, 0.696, and 0.278 yr⁻¹, the substantial increase after the mid-2000s. These are the assessment table's reported estimates (DFO CSAS SAR 2016/026, Table A2, NCAM M-shift formulation — the state-space assessment model of Cadigan 2016); the corresponding survival entries — 0.711, 0.488, and 0.696 in the non-recovery window, 0.750, 0.499, and 0.757 in the recovery window — are $\exp(-M)$ after rounding, transformations of the reported instantaneous mortality estimates rather than an independently observed survival series.

The non-recovery window (1996–2004) remains unexplained in both formulations: residual catch is first-order at low biomass, weak depensation is a live alternative (Shelton and Healey 1999), and the predator-pit and assessment-bias discriminants are untested. Context from a companion study, one sentence: the research programme's cod forecast-evaluation study applies preregistered persistence-benchmark scoring to this case, and its negative certificate — a declared, published finding of non-retention under preregistered scoring, distinct from a statistical null result, here that the persistence benchmark was not defeated — is that study's result and is not transferred here.
...
For each candidate system, compare at least an environmental-forcing model, a cohort- or demographic-resonance model, an institutional-delay model with registered controller sign, a combined model, and a null time-series model. Models make predictions before the held-out block is scored, using declared predictive and calibration criteria; model weights or posterior probabilities require an explicit likelihood and prior, and predictive ranking alone does not identify a causal mechanism. The displacement rule: a delay explanation is weakened when it cannot reproduce the preregistered phase ordering, and it is displaced when a competing mechanism predicts the held-out observations better — complexity is retained only on scored evidence, never by accumulation. The registered domain templates of the ledger companion (phosphorus, groundwater) carry the same discipline for their physical and institutional ladders.
...
Three disciplines frame the list. First, the diagnostic no-transfer rule: a diagnostic is not a causal claim — the spectral null, the response regions, the power values, the case calculations, and the cod split carry their declared types and no more, and no accumulation of diagnostics converts to causal evidence. Second, the admission standard: this research programme's scored-forecast methodology — preregistered scoring against declared baselines, with a negative certificate published when a benchmark is not defeated — is the rule the prospective programme inherits, and the research programme's four scored empirical manuscripts (two forecast ladders and two intervention-selection legs on the cod and Edwards Aquifer systems) are separate studies whose results are not transferred here. Third, the designs of §7 do not convert the retrospective findings into causal evidence; they replace them, and until they are executed the summary of the empirical layer is the one stated in this article: a well-posed mechanism, an exploratory computational record, a selected-cohort spectral null with limited power, a zero-count case search that is not disconfirmation, and one case whose positive result is a descriptive split.

# 10. Provenance, reproducibility, and limits

**Provenance.** Every inventory-sourced statement carries its provenance key in Appendix A; the key links the statement to its source location, canonical module, mapping type, evidence status, and destination. The ten source manuscripts behind the retained set were each read in full by the dated scientific passes recorded in the statement inventory. The manuscript-native entries (Appendix A) carry their declaration on the line: the governance-time ontology and the four designs are source-declared objects, the forward-invariance proposition is the periodic-review source's complete formal result with its proof reproduced in full, the consistency instance rides a conditional theorem of the atlas companion (a cross-reference to that companion's entry, not a retained row of this article), and the falsification criteria are the source's declared outcomes. The shared bibliography of the periodic-review source is a declared but uncommitted file dependency, so the bibliography is reconstructed from the cited works at camera-ready. Four manuscript-native entries restate source content that carries no inventory row, at its source-declared status: the architecture-level empirical hypotheses, declared with their tests and not executed (§5.2); the assessment-table values for the non-recovery and recovery windows, source-reported estimates of DFO CSAS SAR 2016/026 Table A2 with an $\exp(-M)$ survival column (§6.3); the alternative-mechanism enumeration as the non-identification rule behind the case-search criteria (§6.1); and the observation that the fixed-$\tau$ form is a modelling idealisation, an interpretive caveat (§7.1).

**Reproducibility.** The empirical and computational records are stated at their source-declared statuses and no higher. The computational record of the periodic-review source is incomplete: until it is complete, the stage-output values have exploratory status rather than the status of reproducible numerical propositions; the code and machine outputs are not committed, and the retrospective computational and data results remain unreproduced. The registration requirements not discharged are: the initial histories and solver configuration; the RAM stock identifiers and eligibility table; the processed series and spectral routines; the power simulation code and seeds; and the case-screening table and query log. On the open-problem docket: the constrained-M reproduction package, the community-level social-data pipelines with their locked query, extract, filters, crosswalk, and reproduction code, the poverty-line vintage correction before any income-axis use, and the archived table query and population mapping of the social-ecological bridge. Computational claims in this research programme carry a certification hierarchy — nominal result, re-execution-verified, independently re-executed, certified (interval or rigorous arithmetic) — stated per claim and never implied; nothing in this article reaches beyond the nominal tier, and the statuses above say so.

**Limits.** (i) Diagnostics are not causal claims: the spectral null, the response regions, the power values, the case calculations, and the cod split carry their declared types and no more. (ii) The response regions are exploratory finite-grid, trajectory-classified records pending complete stage registration and multiplier analysis; no Poincaré-map multiplier classification is claimed, and the values are not reproducible numerical propositions until the computational record is complete. (iii) The two review-map operators are distinct: statements computed on the hold map, on the stage-structured map, and on the continuous-delay equation do not transfer to one another, and every review-window statement names its operator. (iv) The screen is a selected-cohort consistency check whose power is high only in favourable noise and record-length regimes; the null is not proof of absence and adjudicates nothing about controller sign. (v) The zero-count case search is not independent disconfirmation under its own eligibility criteria. (vi) The cod case establishes a descriptive partition — the crash interpretation is formulation-dependent, the non-recovery is unexplained in both formulations, and no mechanism is identified; the constrained-M quantities are reproduction targets, and the arithmetic record of the case remains subject to archived recalculation. (vii) The obstruction mathematics concerns exact trajectories of the fixed-parameter, fixed-removals autonomous class and is not a rejection under measurement error, process noise, age structure, migration, time-varying mortality, or state-space observation models. (viii) The social object is not operationalized: the human series is not assembled, the normative floors are unoperationalized, and the gap is the result. (ix) The prospective designs are preregistration targets, not executed studies, and they convert nothing retroactively. (x) The three conditional sources are outside this article's retained set, and no claim here depends on them.

# Appendix A. Statement inventory

This appendix inventories the formal and empirical statements of the article in two tables. The **identifier** column carries the research programme's inventory row codes (`CC-A0dd-ddd`) or manuscript-native keys (`MS-Native-n`); the codes key each statement to the source-to-canonical statement inventory maintained in the research programme's repository and are provenance keys, not citations. Every inventory-sourced entry was verified against its source manuscript in the dated full-read campaign of 2026-08-27/28 (full-source reads with per-statement confirmation of existence, kind, proof presence, module, and mapping type); this is content-level verification of provenance, not a promotion of any entry's status.

**Legend and disclaimer.** Entries categorized as *Definition*, *Registry entry*, *Registration requirement*, *Scope*, *Boundary record*, or *Interpretive caveat* are stipulated or declared — they carry no empirical truth-value and need no proof. Entries categorized as *Theorem*, *Proposition*, *Lemma*, *Example*, or *Counterexample/limit* are established under the assumptions stated where they appear (proved or reproduced in this article, or verified present in the identified source). Entries categorized as *Conditional theorem*, *Conditional/open*, or *Conjecture* retain their hypotheses and restrictions as mathematical content. Empirical records (screen results, response regions, power values, case calculations, case syntheses) carry their source-specific evidentiary level in the status column, and "row-verified" in the evidence column means verified against the source in the dated full-read campaign described above. The formal validity of any entry within the declared framework does not by itself imply applicability to an empirical system (§10, Limits). No status is promoted anywhere in this inventory; the manuscript-native entries MS-Native-1 through MS-Native-6 are this article's own statements of source-declared objects, results, and designs, and MS-Native-7 through MS-Native-10 restate source-declared content that carries no inventory row, all at the statuses declared above.

**Table A1. Stipulated definitions, model objects, declared registrations, obligations, hypotheses, and designs.**

| Identifier | Statement | Status | Evidence | Section |
|---|---|---|---|---|
| CC-A002-045 | Observation aggregation | empirical hypothesis (test requirements declared) | row-verified 2026-08-27 · conditional/open | §5.2 |
| CC-A002-046 | Governance phase ordering (frequency-domain caveat) | empirical hypothesis (test requirements declared) | row-verified 2026-08-27 · conditional/open | §5.2 |
| CC-A002-047 | Substitution certificate | empirical hypothesis (test requirements declared) | row-verified 2026-08-27 · conditional/open | §5.2 |
| CC-A003-013 | Sampled-review variant (registry entry) | registered obligation (no artifact) | row-verified 2026-08-28 · defined source object | §2.4 |
| CC-A011-001 | Between-review logistic process with held effort | definition (model object) | row-verified 2026-08-27 · defined source object | §2.2 |
| CC-A011-002 | Filtered nonnegative deficit signal ($\tau_m>0$) | definition (model object) | row-verified 2026-08-27 · defined source object | §2.2 |
| CC-A011-003 | Projected explicit effort update | definition (model object) | row-verified 2026-08-27 · defined source object | §2.3 |
| CC-A011-004 | Extractive controller SD-E-B3 | definition (named controller; discretisation, not a rule) | row-verified 2026-08-27 · defined source object | §2.3 |
| CC-A011-005 | Comparator classes SD-P, SD-F, SD-H | definition (declared comparators, not experiments) | row-verified 2026-08-27 · defined source object | §2.3 |
| CC-A011-012 | 42-stock RAM annual-review cohort | empirical cohort definition (screen input layer) | row-verified 2026-08-27 · source-specific empirical status | §5.3 |
| CC-A011-018 | Initial histories and solver configuration | registration requirement (not discharged) | row-verified 2026-08-27 · source status accepted, artifact pending | §2.4 |
| CC-A011-019 | Code and machine outputs | reproducibility obligation (not discharged) | row-verified 2026-08-27 · source-specific empirical status | §10 |
| CC-A011-020 | RAM stock IDs and eligibility table | registration requirement (screen selection layer) | row-verified 2026-08-27 · source status accepted, artifact pending | §5.3 |
| CC-A011-021 | Processed series and spectral routines | registration requirement (not discharged) | row-verified 2026-08-27 · source status accepted, artifact pending | §5.3 |
| CC-A011-022 | Power simulation code and seeds | registration requirement (not discharged) | row-verified 2026-08-27 · source-specific empirical status | §5.4 |
| CC-A011-023 | Case-screening table and query log | registration requirement (not discharged) | row-verified 2026-08-27 · source-specific empirical status | §6.1 |
| CC-A011-024 | Shared bibliography file | declared file dependency (not committed) | row-verified 2026-08-27 · source status accepted, artifact pending | §10 |
| CC-A014-001 | Strong-Allee constitutive model | illustrative constitutive model (not a fit) | row-verified 2026-08-28 · source-specific empirical status | §6.2 |
| CC-A014-014 | B6 data-collection bridge | registered bridge obligation | row-verified 2026-08-28 · status crosswalk | §6.5 |
| CC-A016-002 | Worst-off constituency (D1) | definition (population/geography requirements) | row-verified 2026-08-28 · defined source object | §8.1 |
| CC-A016-003 | Measured floor (D2) | definition (measurement object) | row-verified 2026-08-28 · defined source object | §8.1 |
| CC-A016-004 | Componentwise-margin rule (M1) | methodological rule (exact conjunction) | row-verified 2026-08-28 · defined source object | §8.2 |
| CC-A016-007 | Global MPI measurement bridge | measurement bridge (release/vintage locks) | row-verified 2026-08-28 · defined source object | §8.3 |
| CC-A016-011 | Social-ecological conjecture (C1) | conjecture (open; coincidence is not causal) | row-verified 2026-08-28 · conditional/open | §6.5 |
| CC-A016-012 | Anti-domination residue | normative limitation record (boundary; must not be filled) | row-verified 2026-08-28 · defined source object | §8.5 |
| CC-A024-001 | Model hitting time vs empirical proxy | scoping distinction | row-verified 2026-08-28 · defined source object | §4.6 |
| CC-A024-008 | Parameter/barrier/observation uncertainty cautions | boundary record (what is not provided) | row-verified 2026-08-28 · conditional/open | §4.6 |
| MS-Native-1 | Governance-time ontology (seven objects, no-collapse rule) | definition (source-declared) | stated in full, §2.1 | §2.1 |
| MS-Native-4 | Four prospective identification designs | preregistration targets (not executed) | source designs, stated in full | §7 |
| MS-Native-5 | Closed-loop MSE crossing design | design (six components; crossing) | source design, stated in full | §7.5 |
| MS-Native-6 | Five falsification criteria | declared outcomes counting against the mechanism | source criteria, stated in full | §9 |
| MS-Native-7 | Architecture-level empirical hypotheses: juvenile-compartment models outperform adult-only models when recruitment suppression is the dominant pressure; spatially aggregated predictions fail systematically where measured variance places the nonlinear moment correction outside its uncertainty bound; exergy/capacity constraints alter deployment and threshold risk in directions predicted before fitting the outcome | declared empirical hypotheses (tests declared; not executed) | source's declared hypothesis list with the registered-minimal-model/estimator/falsification-rule requirement (A010, §12) | §5.2 |
...
| MS-Native-10 | Fixed-$\tau$ form as a modelling idealisation (institutions change instrument sets rather than hold a response law fixed; an estimated $\tau$ is a summary of a changing control architecture, not a structural constant) | interpretive caveat | source observation (A018, §7) | §7.1 |

**Table A2. Theorems, propositions, examples, empirical records, and case results.**

| Identifier | Statement | Status | Evidence | Section |
|---|---|---|---|---|
| CC-A001-013 | Information monotonicity of recovery time | theorem (proof verified present) | row-verified 2026-08-27 · proof present, line check | §4.2 (atlas Prop 7.7) |
| CC-A001-027 | Hidden-mode conflict (no common safe action) | example — impossibility witness | row-verified 2026-08-27 · status crosswalk | §4.3 (atlas Ex 6.5) |
| CC-A001-028 | Delayed-information obstruction ($T_\mathrm{obs}$) | theorem — failure boundary | row-verified 2026-08-27 · proof present, line check | §4.3 (atlas Thm 6.6) |
| CC-A001-096 | Binary-sensor threshold discretization | open problem | row-verified 2026-08-27 · conditional/open | §4.7 |
| CC-A006-003 | Conditional full-information viability benchmark | conditional benchmark (ideal ODE limit) | row-verified 2026-08-28 · conditional/open | §4.4 |
| CC-A006-007 | Information-refinement monotonicity | theorem (common-space requirement) | row-verified 2026-08-28 · proof present, line check | §4.2 |
| CC-A006-012 | Robust information value | remark/template (attainment caveat) | row-verified 2026-08-28 · status crosswalk | §4.5 |
| CC-A006-013 | Informational recovery | remark/template (viable-information-state requirement) | row-verified 2026-08-28 · status crosswalk | §4.5 |
| CC-A006-015 | Normative monotonicity | remark (aligned-class nesting) | row-verified 2026-08-28 · status crosswalk | §4.5 |
| CC-A010-001 | Observation-fibre certification criterion | theorem (scope note preserved) | row-verified 2026-08-28 · proof present, line check | §4.1 |
| CC-A011-006 | SD-E-DR-AN response near 3–4 yr | exploratory response region (finite grid) | row-verified 2026-08-27 · source-specific empirical status | §3.4 |
| CC-A011-007 | SD-E-DR-SP response near 6–12 yr | exploratory response region (finite grid) | row-verified 2026-08-27 · source-specific empirical status | §3.4 |
| CC-A011-008 | SD-E-DR-CO convergence over 1–20 yr | convergence-over-tested-grid record | row-verified 2026-08-27 · source-specific empirical status | §3.4 |
| CC-A011-009 | SD-E-DR-SL transition brackets 30–50 yr | exploratory response region (one-sided slow-$r$ pattern) | row-verified 2026-08-27 · source-specific empirical status | §3.4 |
| CC-A011-010 | 30% assessment-error robustness | robustness summary (declared perturbation only) | row-verified 2026-08-27 · source-specific empirical status | §3.5 |
| CC-A011-011 | Observable-specific spectral peaks | diagnostic spectral record (period caveat preserved) | row-verified 2026-08-27 · source status accepted, artifact pending | §3.5 |
| CC-A011-013 | Multiplicity-controlled spectral null (three-way restriction) | empirical screen result | row-verified 2026-08-27 · source-specific empirical status | §5.3 |
| CC-A011-014 | Injected-signal power values | power analysis (conditional simulations) | row-verified 2026-08-27 · source-specific empirical status | §5.4 |
| CC-A011-015 | More-than-30-system search, zero eligible | case-search record (not disconfirmation) | row-verified 2026-08-27 · source-specific empirical status | §6.1 |
| CC-A011-016 | Case-specific calculations ($R^2$, CV, periods, lags) | case calculations (author calculations) | row-verified 2026-08-27 · source-specific empirical status | §6.1 |
| CC-A014-002 | Extra-loss threshold-shift lemma | conditional proposition (existence/production-maximum conditions) | row-verified 2026-08-28 · conditional/open | §6.2 |
| CC-A014-003 | Fixed-autonomous-scalar incompatibility | incompatibility result (failure boundary) | row-verified 2026-08-28 · source-specific empirical status | §6.2 |
| CC-A014-004 | Scalar-autonomous phase-line obstruction | theorem (proof verified present) | row-verified 2026-08-28 · proof present, line check | §6.2 |
| CC-A014-006 | SAR 2016/026 Table A2 crash-window series | empirical table (rounded source values; $\exp(-M)$ column) | row-verified 2026-08-28 · source-specific empirical status | §6.3 |
| CC-A014-007 | M-pulse vs constrained-M comparison | NCAM-formulation attribution (formulation-dependent) | row-verified 2026-08-28 · status crosswalk | §6.3 |
| CC-A014-008 | Two-window split | case synthesis (the positive result is the split) | row-verified 2026-08-28 · source-specific empirical status | §6.3 |
| CC-A014-010 | Ecosystem context (harp seal, capelin) | background only (causal use unverified) | row-verified 2026-08-28 · status crosswalk | §6.4 |
| CC-A014-015 | Minimal-embarrassment toy test | supplement (executed; scope limited) | row-verified 2026-08-28 · source-specific empirical status | §6.4 |
| CC-A018-012 | Dimensionless identifiability groups | theorem (proof verified present) | row-verified 2026-08-28 · proof present, line check | §5.1 |
| CC-A018-018 | Empirical screen and prospective review-interval hypotheses | empirical screen + prospective hypotheses (two-operator discipline) | row-verified 2026-08-28 · source-specific empirical status | §3.6, §6.1, §7 |
| MS-Native-2 | Forward invariance of the sampled process | theorem (proof reproduced in full) | source's complete formal result | §3.1 |
| MS-Native-3 | SD-E-B3 instance of finite-horizon rapid-review consistency | consistency statement with scope exclusions (atlas theorem CC-A002-034 — cross-reference, not a retained row) | source remark, stated in full | §3.2 |
| MS-Native-8 | Assessment-table values for the non-recovery window (SSB 16.05, 34.42, 20.07 kt at $M$ 0.341, 0.717, 0.362 yr⁻¹ for 1996, 2000, 2004) and the recovery window (SSB 25.18, 96.91, 298.65 kt at $M$ 0.288, 0.696, 0.278 yr⁻¹ for 2005, 2010, 2015; survival column $\exp(-M)$) | empirical table (source-reported assessment estimates; $\exp(-M)$ column a transformation) | corrected-article assessment table (A014; DFO CSAS SAR 2016/026 Table A2, NCAM M-shift formulation) | §6.3 |

No status is promoted anywhere in this inventory; the manuscript-native entries MS-Native-1 through MS-Native-6 are this article's own statements of source-declared objects, results, and designs, and MS-Native-7 through MS-Native-10 restate source-declared content that carries no inventory row, all at the statuses declared above.
...
Hutchings, J. A., and Myers, R. A. 1994. What can be learned from the collapse of a renewable resource? Atlantic cod, *Gadus morhua*, of Newfoundland and Labrador. *Canadian Journal of Fisheries and Aquatic Sciences* 51(Suppl. 1): 2126–2146.
...
Shelton, P. A., and Healey, B. P. 1999. Should depensation be dismissed as a possible explanation for the lack of recovery of the northern cod (*Gadus morhua*) stock? *Canadian Journal of Fisheries and Aquatic Sciences* 56: 1521–1524.
...
Programme sources. The statement-inventory provenance record — the source-to-canonical inventory linking every inventory-identified statement to its source location, canonical module, mapping type, evidence status, and destination, with the 57 rows of Appendix A content-verified — is maintained in the research programme's repository; the ten source manuscripts behind the retained set were each read in full by the dated scientific passes recorded there, as listed in §10 (Provenance).

# Data and code availability

The screen's input layer is the RAM Legacy Stock Assessment Database v4.66 (Ricard et al. 2012); the cohort is defined by the annual-review eligibility screen stated in §5.3, and the RAM stock identifiers and the annual-review eligibility table are a declared registration requirement not yet attached. The cod case's assessment values are the reported estimates of DFO CSAS SAR 2016/026, Table A2 (NCAM M-shift formulation; the state-space assessment model of Cadigan 2016); the social-data inputs are Statistics Canada Tables 38-10-0167-01 and 38-10-0168-01; the ecosystem context is the mass-balance record of Tam and Bundy (2019); the distributive instruments are the global Multidimensional Poverty Index (OPHI/UNDP releases) and the World Bank Poverty and Inequality Platform, each with release and vintage locks as declared in §8.3. The screen itself is described in full reproducible detail in the manuscript — eligibility screen, detrending rule, Lomb–Scargle periodogram (Lomb 1976; Scargle 1982), predeclared power bands (4–8 yr biomass, 12–60 yr effort), per-series AR(1) red-noise null, Benjamini–Hochberg false-discovery-rate control, and the robustness criteria a peak must survive — so that the analysis can be reimplemented from the text. The code and machine outputs of the periodic-review computations are, however, not committed, and the retrospective computational and data results remain unreproduced: the initial histories and solver configuration, the processed series and spectral routines, the power-simulation code and seeds, and the case-screening table and query log are declared registration requirements not discharged, and the constrained-M reproduction package and the community-level social-data pipelines remain on the open-problem docket. Computational claims in this research programme carry a declared certification hierarchy (nominal result; re-execution-verified; independently re-executed; certified interval or rigorous arithmetic), stated per claim and never implied; nothing in this article reaches beyond the nominal tier.
