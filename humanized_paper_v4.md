...
The paper is organized as follows. Section 2 fixes the typed framework and notation, Section 3 defines the assessment operators, Section 4 states and proves the separation and its bounding results, Section 5 gives the interpretation, positioning, scope, and policy implications, and Sections 6–7 discuss limitations and conclude.
...
- z ∈ Z: a state; a: an action; d ∈ D: a disturbance; s = (s₁, …, sₙ): typed floors, normalized so that sᵢ ≥ 0 is the binding constraint; x: the reserve stock (bridge stock) on the witness.
- W₊ = ℝⁿ₊ \ {0}: the full nonnegative scalarization cone excluding the origin (§3.1); w ∈ W₊: a nonnegative scalarization weight; a subfamily is written W, as in Proposition 4; r = w₂/w₁: the weight ratio used on the witness (the resource increment of §5.5 is κ).
- S = S^phys ∩ {sᵢ ≥ 0, i = 1..n} and G = G^phys ∩ {s ≥ 0}: typed safe and destination sets; S^w, G^w: their scalarized counterparts at weight w; S^phys, G^phys: their physical counterparts; on the witness S = S₀, the transition-safe set of §4.5.
- Tube(a,d): the full set of states visited during the interval under action a and disturbance d; End(a,d): the endpoint values visited; Succ(a,d): the successor set after any endpoint reset.
- E_typ(z), E_w(z), E_tube,phys(z), E_end(z), E_end,typ(z): the assessment operators of §3.1.
- V[E] = {z : E(z) ≠ ∅}: the accepted-state set of operator E; in particular V_typ = V[E_typ], V_w = V[E_w], V_weak = ⋂_{w∈W₊} V_w, V_phys = V[E_tube,phys], V_end = V[E_end].
- Witness datum (§4.5): phase state (q, x, s₁, s₂); gain vector e = (¼, ¼); rescue cost c = 1; depth of worst-case dip 2; menu {NO-SWITCH, FAST, SLOW, STAGED}.
- Discrepancy sets (§4.6): Q — aggregate-versus-direct-floor discrepancy region; R — rescue set; I — impossibility region; FP_agg = V_weak \ V_typ — the genuine acceptance gap; ρ₁, ρ₂ — the per-weight licensing thresholds.
- A: the action set (§§4.1 and 5.5); Aug_κ: the resource-augmentation map, with increment κ and minimal rescue threshold κ\*.
...
High-s₂-weight assessors (r > ρ₂) license FAST only; low-s₂-weight assessors (r < ρ₁) license SLOW only; intermediate assessors license both. On I no single action serves every weight — ⋂_w E_w(z) = E_typ(z) = ∅ by Proposition 3(ii) — while on R the STAGED action is typed-admissible and hence serves every weight.
