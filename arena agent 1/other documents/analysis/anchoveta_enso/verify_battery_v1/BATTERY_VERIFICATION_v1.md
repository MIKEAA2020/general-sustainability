...
producing script. Executable companion: `forensics.py` (all checks PASS).
...
- **Uploads reconstructed** (behaviorally exact; see
  `/home/user/uploads/README_reconstructed_inputs.txt`): `shortened.txt`
...
proof), unmodified rerun outputs + logs, the executable `forensics.py`
...
cp ../verify_battery/frozen_inputs/nina*.anom.data .
cp ../verify_battery/frozen_inputs/soi.data ./soi_ok.data
python3 battery.py   # -> battery_results.json (bit-exact deterministic blocks)
mkdir run2 && cd run2
cp <repo>/analysis/anchoveta_enso/battery_v2.py .
cp ../verify_battery/frozen_inputs/nina*.anom.data .
cp ../verify_battery/frozen_inputs/soi.data ./soi_ok.data
# place reconstructed uploads at /home/user/uploads/{shortened.txt,SAU Taxa 600004 v50-1.csv}
python3 battery_v2.py  # -> battery_v2_results.json (bit-exact deterministic blocks)
# Forensics (no pyEDM needed): cd verify_battery && python3 forensics.py  # ALL PASS
```
