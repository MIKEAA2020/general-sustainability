% =========================================================================
% dde_diagnostic.m — decisive numeric check for the browser sandbox.
% Runs tau=5.57 from (40,0.1,8) for T=500, and PRINTS the tail amplitude
% (max-min of N over the last 20%) and the final N.
%   - If tail_amp > 15  => correct integration (large cycle present)
%   - If tail_amp ~ 0   => browser integration is wrong (plateau/decay)
% Compare with the validated Python reference: tail_amp ~ 31.6, final N ~ 87.
% =========================================================================
clear; clc; close all;
...
% tail amplitude = max-min over last 20% of the run
...
if tail_amp > 15
  ...
  fprintf('  => WRONG: no cycle (browser integration differs from validated)\n');
end
% also print a coarse N profile so we can see the shape
