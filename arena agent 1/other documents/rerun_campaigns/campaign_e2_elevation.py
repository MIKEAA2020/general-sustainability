...
def main():
    ...
    assert abs(fit["train_residual_sd"] - 135.0) < 1.0, fit["train_residual_sd"]
    assert abs(e_min + 460.0) < 1.0, e_min
    assert abs(e_q05 + 318.8) < 1.0, e_q05
    assert abs(e_q10 + 114.85) < 0.5, e_q10
    ...
    UC_comm = {"UC_min": e_min, "UC_q05": e_q05, "UC_q10": e_q10}
    n_checked, n_empty_match = 0, 0
    for pid, c in pol_c.items():
        for ucid, e in UC_comm.items():
            for T in (1, 3, 5):
                c_iv = committed["kernels"][pid][ucid][str(T)].get("nominal")
                c_lo = float(c_iv[0][0]) if c_iv else None
                a_lo = t_step_boundary(c, e, T, r0, K0, K_STAR, S_HI)
                assert (a_lo is None) == (c_lo is None), \
                    f"{pid}/{ucid}/T={T}: analytic {a_lo} vs committed {c_lo} (emptiness)"
                if a_lo is not None:
                    assert abs(a_lo - c_lo) < 1.0, \
                        f"{pid}/{ucid}/T={T}: analytic {a_lo} vs committed {c_lo}"
                    n_checked += 1
                else:
                    n_empty_match += 1
    print(f"  analytic recursion reproduced {n_checked} committed T=1/3/5 boundaries "
          f"and {n_empty_match} committed emptiness rows")
