...
Drought management in permitted groundwater systems is built on triggers — thresholds at which pumping reductions activate — and robustness has been a recurring concern of water-resources planning. This study follows a preregistered retention rule: a governance module is kept only if it improves a declared protection-and-supply outcome, with the protocol frozen before any score is computed. Robust viability kernels of the J-17 index-well head (San Antonio Pool, Edwards Aquifer) are computed for a declared pumping-policy family — business-as-usual, flat caps, a Stage-I reactive rule, and a critical-period-management cascade — under persistent drought-floor recharge classes, with a certified layer that converts the fitted map's declared defect into an erosion margin. Four results follow. (1) At the 618-ft physical threshold, business-as-usual is not robustly viable against a perpetual drought-of-record floor: its kernel empties beyond about 13 years, while every flat cut of 10% or deeper — and both reactive rules — makes the whole safe set robustly invariant; the smallest securing cut is 7.2% of mean pumping. (2) At the 660-ft institutional threshold, a negative certificate — a machine-verified non-retention finding, distinct from a statistical null — holds for every policy: the threshold is protected by wet years, not by the declared pumping family. (3) The certified layer is defect-bound: every demand-management policy's certified kernel is empty beyond T = 3 years. (4) Positive selection: the reactive rules are retained, matching flat-cap protection at 3.3–16.2% more permitted supply (Stage I) and 50.6% more (cascade).

**Keywords:** Edwards Aquifer; critical-period management; viability kernels; pumping policy; drought triggers
...
Business-as-usual (BAU) holds pumping at the training mean, P ≡ P̄ = 282.16 × 10³ acre-ft yr⁻¹. Flat caps prescribe ρP̄ with ρ ∈ {0.9, 0.8, 0.7, 0.6, 0.5, 0}. The Stage-I reactive rule (S1) cuts pumping 20% when H < 660 ft (the reduction verified against the Authority's published rule). The critical-period-management (CPM) cascade prescribes cumulative 20/30/35/40% cuts at H < 660/650/640/630 ft; Stage I is verified, and stages II–IV are declared scenarios, not verified institutions.
...
- **Institutional threshold: nothing retained** (all policies ≡ BAU).
