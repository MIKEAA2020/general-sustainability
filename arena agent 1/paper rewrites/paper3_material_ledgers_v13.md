...
Depletion numbers circulate under one label while answering different questions. This article separates them with a typed stock–flow accounting layer: conservation from the ledger's incidence structure, positivity from donor limitation, services as readouts, and every depletion quantity carrying its classification. Three certification layers — accounting consistency, conservation consistency, barrier safety — carry a flux-reconstruction identity, a conservation-law reduction, and a flux-bounding envelope theorem, proved in full. Specialized to a closed finite-donor ledger (living stock, active pool, geological donor, detritus), the theorem set is established: the natural-block mass identity, orthant invariance, no interior rest at positive effort, the vanishing-extraction rest set (extinction, carrying capacity, frozen-biomass face), and extraction integrability on the finite donor budget. Depletion time separates into three non-interchangeable quantities — gross turnover intensity, frozen-rate ratio, scenario-conditioned hitting time — extended to upper barriers, with uniform-drift bounds and their failure mode exhibited. Three public-data applications classified at their exact status: the G3P anomaly-persistence index is a statistical index, not a physical stock ratio; the phosphate reserve-life ratio is arithmetic, not an exhaustion forecast; the fisheries removals-only time is a gross-loss pressure scale, not a net-depletion diagnostic or hitting time. The ledger proves what it does not support: no nonnegative weighting certifies componentwise nonnegativity, and five double-counting rules block phantom mass. First-passage semantics on stochastic surrogates (inverse-Gaussian groundwater, geometric-Brownian fisheries) carry explicit non-claims; an interface contract fixes the shared object with the delay-based dynamics, recording five reasons the closed ledger does not reduce to the open working system.
...
Behind the first of these failure modes sits a second sense of the productivity illusion, dynamical rather than arithmetic. A measured yield can exceed the true sustainable yield when it is maintained by drawing down the supporting pool — groundwater, soil carbon, bioavailable nutrients — rather than by that pool's regeneration: the resource appears productive while what sustains it is liquidated silently. The two-pool architecture of this article exists to detect exactly this yield-inflation sense: the applied tables of Section 6.5.2 report each resource beside its supporting pools, so a stock's apparent adequacy cannot hide the silent liquidation of what sustains it. The compensatory sense is the aggregation obstruction of Section 10.1; the yield-inflation sense is a dynamical channel, and the depletion diagnostics of this article are built for it. The temporal asymmetry that makes the yield-inflation sense dangerous has a mechanical picture. An elevator rated for ten people will hold fourteen for a long time: the cable does not part on the fourteenth passenger but only after the accumulated wear it never shows. During the overshoot nothing announces the damage — the wear is diffuse, gradual, and invisible to everyone riding the car — while the snap, when it comes, is sudden and total. The yield-inflation sense of the productivity illusion is the wear phase: measured yield stays up because the supporting pool is being drawn down, and nothing in the yield series records the drawdown. The two-pool tables of Section 6.5.2 report each resource beside its supporting pool for exactly this reason — to make the wear measurable while the cable still holds, not to predict the snap (Section 7 states what the passage-time results do and do not condition on).
...
These donor primitives instantiate one of three recharge laws that appear across this article and its companions; the three are distinct objects, tabulated once so that none is silently substituted for another:
