...
We develop an obstruction calculus — a set of finite, checkable certificates that no observation-based policy exists. The certificates are sound sufficient conditions for nonviability and do not exhaust the complement of the epistemic kernel (the observation-based counterpart of the viability kernel). Five mechanisms are proved, with a sixth exhibited under a policy-class restriction. The central common-action obstruction shows that when the safe controls of compatible states intersect emptily, no observation-based policy is viable — though every compatible state is individually viable under full information. The further four cover a finite-time exit certificate under an Isaacs-type drift condition (the disturbance moves after the control and forces violation within a computable time), an epistemic-emptiness construction where a constant observation merges states whose safe controls differ, a delayed-information obstruction with an explicit timing bound, and a certification limit (an exact observation-only certifier exists exactly when safe-set membership is constant on the observation fibres — the sets of physical states consistent with one observation). The exhibited sixth, a certainty-equivalence trap (a controller that applies a fixed state-feedback law to the uncorrected observation), shows that under a biased observation such a controller can empty a nonempty perfect-information kernel.
