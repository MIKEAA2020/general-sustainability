...
This section registers the macro-reduction conjecture that underlies the main article's reduction from the five-state core to the working three-state system. The conjecture's slow-fast bound is stated, its five hypotheses are itemised, and the present status of each is given. Two hypotheses are now established and one remains open; the remaining two are standard regularity hypotheses carried over from the audit document.
...
**Status of the hypotheses (verified 2026-09-03; audit `audits/tikhonov_unh_verification.md`).** The precise finite-time theorem — setting, hypotheses H1–H5, conclusion, and proof sketch (boundary layer on the fast clock with the delay as a slowly drifting parameter; retarded Gronwall closure) — is stated in full in the audit document. Two hypotheses are now established, one remains open:
