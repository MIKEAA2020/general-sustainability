...
Depletion numbers circulate under one label while answering different questions. To separate them, this article develops a typed stock–flow accounting layer: conservation is proved from the incidence structure of the ledger, positivity follows from donor limitation, services are readouts, and each depletion quantity carries its classification. The accounting rests on three separated certification layers — accounting consistency, conservation consistency, and barrier safety — with a flux-reconstruction identity, a conservation-law reduction, and a flux-bounding envelope theorem proved in full. The layer is then specialized to a closed finite-donor ledger — living stock, active pool, geological donor, detritus — on which the theorem set is established: the natural-block mass identity, orthant invariance, absence of any interior rest at positive effort, the vanishing-extraction rest set (extinction, carrying capacity, and the frozen-biomass face), and extraction integrability on the finite donor budget. With the ledger in place, depletion time separates into three non-interchangeable quantities — gross turnover intensity, a frozen-rate ratio, and a scenario-conditioned hitting time — extended to upper barriers, with uniform-drift bounds and their failure mode exhibited. Three public-data applications are then classified at their exact status: the G3P anomaly-persistence index is a statistical index, not a physical stock ratio; the phosphate reserve-life ratio is arithmetic, not an exhaustion forecast; and the fisheries removals-only time is a gross-loss pressure scale, not a net-depletion diagnostic and not a hitting time. The ledger also proves what it does not support: no nonnegative weighting by itself certifies componentwise nonnegativity, and five double-counting rules block phantom mass. Complementing the classifications, first-passage semantics on stochastic surrogates (inverse-Gaussian groundwater passage, geometric-Brownian fisheries passage) are supplied with explicit non-claims. Finally, an interface contract fixes the shared object with the delay-based dynamics and records five reasons the closed ledger does not reduce to the open working system.
