#!/usr/bin/env python3
"""
Computational viability certification, edition 9 — verification.
...
paper2_computational_certification_v10.tex, the separate declaration
...
import re as _re
hyg = True
for f in ("paper2_computational_certification_v10.tex",
...
tex = open(os.path.join(HERE, "paper2_computational_certification_v10.tex"),
           encoding="utf-8").read()
tex_raw = tex
tex = " ".join(tex.split())
NEEDLES = [
    ...
    "paper2_computational_certification_v10_verification.py",
    ...
    "and the induced pair-delay hierarchy (Section~\\ref{ladder})",
    "Four further extension directions are recorded, each a construction of its own "
    "rather than an increment of the present edition",
    ...
    "is a recorded next step, alongside the tooling directions of Section~\\ref{library}",
...
check("introduction contribution list references the ladder subsection "
      "exactly once", tex_raw.count("\\ref{ladder}") == 1,
      tex_raw.count("\\ref{ladder}"))
...
_pars = [f for f in ("paper2_computational_certification_v10.tex",)
         if True]
