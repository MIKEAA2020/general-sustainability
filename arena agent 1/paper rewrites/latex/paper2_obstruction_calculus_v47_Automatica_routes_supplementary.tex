...
certificate (main text, Section 7), with the display corrected to the
...
\emph{Remark (the main text's \(\phi_{U}\)-display and the attainment
condition).} The main text states the certificate with the minimum
\(\phi_{U}(g) = \min_{v \in U} g^{\top} v\) in place of \(h_{U}\) and
the opposite firing inequality. That form is sound exactly when the
realized control pairings attain their pointwise minima, i.e.\ when the
policy's signals coincide with the \(\phi_{U}\)-extremal selection; the
chaining step ``weighted safety implies the bracket is nonnegative''
requires this attainment and does not hold for an arbitrary admissible
signal. The pooled construction of the three-branch instance satisfies
...
\(\phi_{U}(0) = h_{U}(0) = 0\) and every signal attains the bound --- so
the instance's exact critical delay is unchanged. In general the sound
