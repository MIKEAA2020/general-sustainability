...
\section{What else is not claimed}

The design formulas (the dual-margin ratio for safe inspection intervals
and the Lipschitz quantization bound) remain plausible scalings, verified
only instance-wise. The stochastic-bridge identification (the dual measure
as a worst-case prior of the stochastic selector) remains unproven; the
masked-backup mechanics of that lineage must be checked before any bridge
is claimed. Scope: continuous-time, control-affine systems with convex
control sets; fibre crossing of the target boundary remains governed by
the source paper's fibre criterion; no completed item is modified.
...
\texttt{minimax\_dual\_certificates\_v3\_verify.py} is publicly available
