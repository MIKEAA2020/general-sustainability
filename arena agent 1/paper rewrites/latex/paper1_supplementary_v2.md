...
(manuscript v5, Environmental Modelling & Software submission).
...
version 1.2.0; all quantities are exact rationals unless stated as
...
├── SHA256SUMS                      # integrity manifest (43 entries)
...
├── src/safetransition/             # 9 modules, 1,542 lines
└── tests/                          # 39 tests, 470 lines
```

`sh run_all.sh` executes, in order: the test suite (39/39, including the
certificate-protocol and adversarial exactness tests), the exact benchmark
checks (24/24), certificate emission plus the independent checker over the
emitted certificates, the three worked examples (the certificates demo
ends in tamper rejection), and — when matplotlib/Pillow are installed at
the versions pinned in `requirements-figures.txt` (3.10.9 and 12.3.0) —
figure regeneration, including the graphical-abstract TIFF. A SHA256SUMS
