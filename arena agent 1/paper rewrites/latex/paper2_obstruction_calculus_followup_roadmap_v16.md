# Paper 2 — Obstruction Calculus: Follow-up Roadmap (v16)
...
**v16 changes:** The SIXTH JOINT-AUDIT ROUND is closed — a single
grok proposal of a correctly scoped primal–dual selector principle
(`audits2.2.txt`) was verified against the artifacts
(`paper2_selector_principle_joint_audit_v1.md`), with two genuine
findings and one ownership of an under-read prior finding: (1) the
delay-rule round-4 repair was INCOMPLETE — the monitoring note's
"equivalently T_obs < sigma*" is false at the boundary cell
(z0, T_obs) = (2, 1), which is viable; `monitoring_design_v3` ships the
non-strict identity viability iff T_obs <= sigma*(z0) = z0 − 1 with the
boundary case stated (check E9 pins the one strict-form failure and the
zero non-strict mismatches on the 48 cells); (2) the proposal's own
"coarsest adequate monitoring = the partition into maximal common-action
sets" is REFUTED — the two maximal sets of the audited target overlap
in (2,2), so no such partition exists; the correction ships in both the
monitoring edition and the new paper; (3) the proposal's Theorems A/B,
dual-witness table, weak/strong duality scoping, and deficit-dual
reading were verified as ALREADY LATENT in the shipped editions, and
its stale-fix list (empty refs, boxed iff, in-window adaptivity, CE
terminology) probe-verified as fixed. NEW STANDALONE PAPER
`paper2_selector_concordance_v1`: the count concordance (every audited
integer with its meaning and source: 36/28/26/25/24 four-cell product,
12/16/16/256 decentralized with the two sixteens distinguished,
mode-blind coincidence, 48 cells, 7-of-15, sigma* and CE conventions,
the continuous benchmark Y* = 27/5) plus the principle stated once with
its exactness class on the theorem box — completing the
errata/concordance residual item; verification 8/8. The
withdraw/fold/insert blueprint remains OWNER (contradicts the
standalone-lineage discipline and the other reviewers' conflicting
blueprints). Verification: monitoring v3 9/9, concordance 8/8; builds
zero overfull. No changes to the roadmap's item statuses.

**v15 changes:** The FIFTH JOINT-AUDIT ROUND is closed — the
elevation/duality file `audits2.txt` (gemini masterplan; grok elevation
response; gemini's "Universal Minimax Epistemic Duality Theorem" with a
claimed resolution of Open Problem 1, design formulas, and a two-stock
benchmark; plus a byte-identical duplicate of the theorem block
mislabeled as grok's — adjudicated once) was verified against the
artifacts (`paper2_elevation_joint_audit_v1.md`). A NEW STANDALONE PAPER
shipped, `minimax_dual_certificates_v1`: the measure dual of the
common-action obstruction — Sion core and emptiness iff verified exact;
the sparsity proof repaired (Carathéodory/Helly conflation; k+1 bound
certified tight); the singleton corollary corrected (the adversarial
distribution is load-bearing, not one worst disturbance); the
convexity boundary exhibited (a strict two-action minimax gap — the
audited finite codex systems sit outside by design); the state-vs-
epistemic axiom ADOPTED as a three-class taxonomy on the audited grid
(physical corner / epistemic B* / institutional 12→0); the benchmark
REPLACED after the audit found it switching control sets between
evaluations — the corrected model (one fixed demand-floor control set)
solves in closed form: critical aggregate Y* = 27/5, two-point dual
certificate with margin 3/50 at Y = 6, fibre-width formula
Δx1(Y) = Y − 4. The claimed resolution of Open Problem 1 is REJECTED
(nonlocal operator, no viscosity theory, no selection argument); the
dynamic envelope, design formulas, and stochastic bridge are recorded as
conjectures. The residual ledger across all five rounds is dispositioned
in the record: three items SHIPPED this round, the library MVP scheduled
as the top successor item, real-data calibration and consolidation OWNER-
side, the rest already done or refuted. Verification: 8/8 exact checks;
build zero overfull. No changes to the roadmap's item statuses.

**v14 changes:** The FOURTH JOINT-AUDIT ROUND is closed — the five
Track-D catalogue notes D4, D7, D8, D11, D13 (first editions
`vector_floor_certificates_v1`, `policy_class_lattice_v1`,
`monitoring_design_v1`, `hybrid_mode_viability_v1`,
`institutional_observation_v1`) were audited by two owner-supplied
review files, every claim verified against the artifacts before
adjudication (`paper2_trackd_joint_audit_v1.md`), and five second
editions shipped (each first edition retained unmodified; each
verification script extended from six to eight exact check families,
40 checks total, all passing; builds zero overfull): the vector-floor
note repositioned truthfully against the calculus paper's §§2.2/3.2/3.4
(its "one constraint at a time" sentence was false) with the Farkas
support reading (minimal infeasible subsystem) replacing the
misapplied complementarity label; the policy-class lattice's missing
fourth 2×2 cell COMPUTED (full information under the no-repeat codex:
25 of 36) — the four audited classes are a genuine Boolean lattice with
incomparable middle cells, refuting the "only a chain" collapse while
adopting it as the spurring correction; the monitoring note's abstract
delay-rule falsehood fixed (the verified object is the equivalence on
48 cells, not the inequality), the four R-sets stated, and the seven
admissible partitions enumerated with TWO minimal 2-cell generators
(refuting the audit's upper-set arithmetic); the mode-uncertainty
note's singleton generation proven and pinned (the corner (1,1) is the
only mode-blind-dead state — no merged-pair loss), its causal timing
stated, and the phantom "Section 3" reference literalized; the
decentralized note's "delayed common knowledge" renamed to the
truthful delayed own reading, its deadlock transition and 256-law
space declared, its abstract's "command different instruments"
contradiction repaired, and the loss instance {(1,2),(3,1)} named and
pinned. Refuted and recorded: the "verbatim duplication" charge on D4
(the additions are computational and now positioned passage-by-passage),
the "no belief-level phenomenon" charge on D11 (the singleton death is
itself informational), and the programme-level consolidation blueprints
of both reviewers (recorded for the owner; their "undefined system /
unshown scripts" factual premises do not survive verification). No
changes to the roadmap's item statuses.

**v13 changes:** The THIRD JOINT-AUDIT ROUND is closed — the unifying
framework lineage `paper2_viable_selector` (edition 2) was audited by
two owner-supplied reviews (gemini, seven items; grok, seven items),
every claim verified against the actual artifacts before adjudication
(`paper2_viable_selector_v3_addendum.md`, dispositions K1–K16), and
edition 3 `paper2_viable_selector_v3` shipped: the six broken
cross-references literalized; the meta-theorem box given its
finite-model qualifier (Remark 1 added, separating the three selector
objects and recording that the factors NEST — the four-fold
intersection equals its finest factor, refuting the claimed
intersection-vs-nesting contradiction); §6 rescoped to the canonical
certainty-equivalence law with the two-patch system's provenance cited
to the calculus paper's Section 8 (refuting the "never defined" claim
— the system is defined there; the citation was the genuine gap);
§5 restated at the information state; Proposition 2's survival
predicate formalized with the class-indexed sigma*; the ladder's
strict/tight counts (exactly five / exactly four) now asserted exactly;
the dynamic-exit row reworded as the N = 0 singleton base case; the
standing-hypotheses pointer added. Grok's headline "declarations pasted
into §6" and "orphaned fragment after Table 1" claims REFUTED as PDF
text-extraction artifacts (source clean). Verification extended from
seven to twelve exact check families (12/12); build zero overfull;
second edition retained unmodified. No changes to the roadmap's item
statuses.

**v12 changes:** The UTILITY AUDIT ROUND is closed — two owner-supplied
use-case reviews of the programme's applications posture (grok,
gemini) were adjudicated jointly (`paper2_utility_joint_audit_v1.md`):
their object-level citations verified accurate against v48; their
four genuine-use domains and stakeholder table ADOPTED; the timing
formula adopted only with its class scope; the 'Tracks B-D would have
to produce' premise CORRECTED as outdated (the certificate and
belief-state lineages are shipped); the software/calibration gaps
ACCEPTED as open owner-side items. COMPLETED by the new satellite
statement `paper2_applications_note_v1` ('From Obstruction
Certificates to Monitoring Standards'): four design rules with scope
and verified instances, the operational/architectural split, the
stakeholder table, six precise limitations (L1-L6), the
delivered-programme index, and the remaining open items — zero
theorems, every claim a pointer, pointers machine-checked 9/9 by
`paper2_applications_note_v1_pointers.py`; build zero overfull.
Remaining at programme level: L2 deferred numerical campaign, L3
completeness problems (Open Problem 1, residual dynamic gap), L4
applied calibration, L5 user-facing tooling — all recorded in the
statement; no changes to item statuses.

**v11 changes:** D1 is **AUDITED and strengthened** — a joint audit of
`paper2_stochastic_selector_v1` (two owner-supplied reviews,
adjudicated jointly against the artifacts; dispositions in
`paper2_stochastic_selector_joint_audit_v1.md`) produced edition 2
`paper2_stochastic_selector_v2`: the policy-class parameter made
explicit (resolving the audited Proposition-1 scope gap — a
formalization gap, not a computational contradiction; the timing
obstruction stands, class-scoped), model boundary cases completed
(total rows, absorbing observation, masked witnesses), the selector's
attaining set separated from its value-one level, the kink/jump
terminology repaired with exact loci, broken references fixed, and the
class declaration SHARPENED by the audit's own recomputation (classes
coincide at k = 1; differ exactly on the timing cells for k >= 2).
Verification extended from ten to fifteen exact check families (15/15);
build zero overfull; first edition retained unmodified. No changes to
the roadmap's item statuses.

**v10 changes:** Track D is **COMPLETE** — D4 through D13 are drafted
as ten standalone lineages (standalone names from birth, no paperN
prefix), each with an exact-arithmetic verification record (6/6 check
families each, 60 across the catalogue):
`vector_floor_certificates_v1` (D4 vector floors with Farkas
certificates) · `finite_horizon_completeness_v1` (D5 soundness +
completeness + counterstrategy trees; the H/H-2 convention a theorem) ·
`certificate_duality_v1` (D6 four obstruction families as dual
objects) · `policy_class_lattice_v1` (D7 chain 24 <= 26 <= 28 of 36,
strict, with computed distances to viability) · `monitoring_design_v1`
(D8 fibre conditions, coarsest admissible monitoring, delay/bias/
aggregation rules) · `comparison_drift_exit_v1` (D9 comparison-function
exit identities) · `hidden_parameter_learning_v1` (D10 learning-
deadline identity z0 >= 21/10 + T_learn/10) · `hybrid_mode_viability_v1`
(D11 mode-uncertainty boundary: 28/36 blind, all containing (1,1) lost,
observation restores) · `buffered_viability_v1` (D12 obstruction
margins, erosion 28 -> 3 -> 0) · `institutional_observation_v1` (D13
unanimity deadlock: coordinator 28/36, decentralized 12/36, 16 lost
pairs, authority collapse 12 -> 0). The mainline and the long tail are
now fully drafted; what remains is owner-side (venue decisions,
figshare uploads/DOI, master v5) as recorded in v8.
...
value functions — **DONE** (successor §4) · D4 multiple floors / vector constraints — **DONE**
(`vector_floor_certificates_v1`) · D5 complete finite-horizon
certificates — **DONE** (`finite_horizon_completeness_v1`) · D6
certificates as dual objects — **DONE** (`certificate_duality_v1`) · D7
policy-class taxonomy — **DONE** (`policy_class_lattice_v1`) · D8
monitoring design — **DONE** (`monitoring_design_v1`) · D9 beyond
constant drift — **DONE** (`comparison_drift_exit_v1`) · D10 hidden
parameters / adaptive observation — **DONE**
(`hidden_parameter_learning_v1`) · D11 hybrid systems — **DONE**
(`hybrid_mode_viability_v1`) · D12 approximate / buffered viability —
**DONE** (`buffered_viability_v1`) · D13 decentralized / institutional
observation — **DONE** (`institutional_observation_v1`).
...
| 7 | D4–D13 — **DONE** (ten standalone lineages, v10 changes paragraph) | per-item |
