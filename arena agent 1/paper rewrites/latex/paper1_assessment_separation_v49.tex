...
% Amin Abaee. Revision v49 (joint-audit alignment pass: abstract gap sentence aligned with the corrected failure-set formulation of v47 -- the gap is the aggregate-certified part of the reserve-short states, and blends close exactly the aggregate-certified states (Theorem 9); the 25 grid checks are enumerated in new Supplementary S12 (was: cited at S8, which is the citation section); deposit README's stale Section 4.12 pointer updated to 6.3; master deposit DOI 10.6084/m9.figshare.33764023). Compiles with tectonic, pdflatex, or xelatex.
...
\item
  \textbf{No absolute impossibility.} The impossibility region is a negative certificate relative to the specified four-action menu; with a larger menu it can shrink or vanish (see the completeness requirement in Section 5.6).
\item
...
investment in that margin is a candidate remedy; reweighting alone cannot substitute for it on the witness datum (Section 5.6). This applies where a resource-controlled rescue action exists; it is not a universal policy claim.
