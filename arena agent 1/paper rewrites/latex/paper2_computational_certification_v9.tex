...
exactly the optimal shared-braking programs. The finite program is solved by a
...
refinement (Theorem~\ref{thm:bridge}(iii)); and their size is governed by
the rank of the
...
and the induced pair-delay hierarchy (Section~\ref{ladder}); the
...
the compact label set (\(k_a\) in \(L^1\) norm, \(\beta_a\)
absolutely; the \(L^1\) form is the operative one --- sup-norm continuity
is neither needed nor true here, the step kernels below jumping at
\(\sigma = t\) for every \(t\) --- while \(L^1\) continuity is implied
by bounded coefficients and enjoyed by every rational instance below);
...
\(R_U V_U h_u\) form as a special case. The \(\max \bar{e}_a\) form is
the honest one: a loose but verified choice inflates \(\rho\)'s relaxation
...
(Section~\ref{computation}) must be refined with the mesh for completeness
...
estimate above, verified in Section~\ref{instance} for the audited
kernels.
...
(\(\rho \le J\) always --- the program is a relaxation; Table~\ref{tab:mesh}
...
\(\rho = -\tfrac{47}{100}\) at \(h = \tfrac1{10}\), and the honest
bound \(J \le \rho + \bar{e} + \delta_\beta + L h_t = \tfrac{17}{100}
...
\subsection{Reading the certificate prescriptively: the shared-authority ladder}\label{ladder}
...
Section~\ref{instance} are exactly these optima. \emph{(iii)} Braking at
...
critical position stays \(\le 2\) with the velocity rows strict; the
edition's script verifies the full seven-facet scan exactly at rational
...
Four further extension directions are recorded, each a construction of
its own rather than an increment of the present edition; all lie outside
the audited class delimited in Section~\ref{scope}.
\emph{(i)} General measurement channels: the instance already prices
partial observation under delayed label revelation, so the incremental
content is the passage from history partitions to general measurement
maps, by a cell-conditioned lift of labels, kernels, and costs.
\emph{(ii)} Stochastic disturbances: replacing support-function rows with
...
\emph{(iii)} Decentralized and networked controller sets, infinite-horizon
...
parameter change.
\emph{(iv)} A cross-tool comparison study that treats the audited
...
recorded in Section~\ref{library} and the independent
certificate-file parser of Section~\ref{methods} are its computational
prerequisites.
...
importing the model builder is a recorded next step, alongside
the tooling directions of Section~\ref{library}. Timings are single runs on the verification
...
\url{paper2_computational_certification_v9_verification.py},
