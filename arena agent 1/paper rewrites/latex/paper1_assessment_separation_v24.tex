...
companion ledger study (Abaee, 2026a) and is not
...
unconditional ones. Every claim below is therefore labelled with its
logical status --- definition, theorem, conditional statement, or open
question --- distinguishing what is proved, what is verified on a finite
machine artifact, and what is asserted without proof.

The masking formalised here is the following: an aggregate of two typed
floors can stay nonnegative along its worst-case tube while each floor
in turn takes a dip that no common plan accepts. It is a distinct
phenomenon from the \emph{productivity illusion} of adequate delivery
from a quietly reduced base, which concerns the productive base and is
treated in Abaee (2026a), where that base sits inside the ledger. On the
witness constructed below the aggregate is taken over the two service
floors \(s_1, s_2\) themselves; what it fails to see is not the base but
the individual floor mid-interval --- the compensatory form of the
illusion as it arises in aggregation, the acceptance gap of Theorem
5(4).
...
(Abaee, 2026a) --- and the typed-endpoint operator
...
systems: Abaee, 2026b; Abaee, 2026c). No result of this article depends
on a companion: the separation results rest on their displayed proofs,
the machine artifact of Section 4.9 is deposited independently, and the
companion dependence is confined to the introduction's citation of the
ledger study (Abaee, 2026a) and the disciplinary analogy of this
paragraph.
...
Abaee, A. (2026a). Typed flux ledgers and depletion arithmetic:
...
Abaee, A. (2026b). Does a surplus-production ladder improve forecasts of
...
Abaee, A. (2026c). Does a one-pool water-balance model improve forecasts
