...
for partial and noisy observation (Section~\ref{erosion}); the
...
monotonicity of the worked-systems companion (Abaee, 2026c): its three
...
\begin{proposition}[belief-cell certificates]\label{prop:beliefcells}
...
and the singleton cells never obstruct (Proposition~\ref{prop:ladder}).
...
\begin{proposition}[redesign sensitivities]\label{prop:redesign}
...
each a pointer to the published audits (Abaee, 2026b, 2026c). The
...
the audited class delimited in Section~\ref{scope}. The cell-conditioned
...
in Proposition~\ref{prop:beliefcells}, with the instance's compatible-mode
...
dual-certificate companion (Abaee, 2026g) exactly, in either direction,
alongside the tooling directions of Section~\ref{library}. Timings are single runs on the verification
...
monotonicity, stationary policies, and decentralized factoring. Manuscript
submitted for publication.

Abaee, A., 2026g. The measure dual of the common-action obstruction, and
the dynamic envelope. Manuscript submitted for publication.

Abaee, A., 2026f. Exact belief-state computation at scale: the
four-parameter cube, Hamming adjacency, and the antichain census.
...
\url{paper2_computational_certification_v14_verification.py},
