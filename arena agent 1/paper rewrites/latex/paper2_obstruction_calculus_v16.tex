...
altogether. And the information that does arrive may arrive late. This paper addresses that side: under an incomplete observation structure, it develops instruments for certifying that no
...
(Theorem~\ref{thm:common-action} and Proposition~\ref{prop:fibre}) the argument is a finite, checkable test. The drift
certificates of Theorem~\ref{thm:exit} and Theorem~\ref{thm:delayed} are not finite objects --- one exhibits
...
\item
  \textbf{The finite-time exit certificate} (Theorem~\ref{thm:exit}). Under a
...
\item
  \textbf{Epistemic emptiness by admissibility} (Proposition~\ref{prop:emptiness}, a minimal
  ...
  obstruction of Theorem~\ref{thm:common-action} at minimal size --- and Example~\ref{ex:hidden-mode} gives the genuine hidden-mode instance.
\item
  \textbf{The instantaneous common-action obstruction} (Theorem~\ref{thm:common-action},
...
\item
  \textbf{The delayed-information obstruction} (Theorem~\ref{thm:delayed}). If the
...
\item
  \textbf{The fibre certification criterion} (Proposition~\ref{prop:fibre}, Corollary~\ref{cor:certainly-safe}).
...
\item
  \textbf{The certainty-equivalence trap} (Remark~\ref{rem:ce-trap}). Even an
...
(2020). To our knowledge, the obstruction certificates of Theorem~\ref{thm:common-action}, Theorem~\ref{thm:delayed}, and Proposition~\ref{prop:fibre}
--- in particular the certification criterion of Proposition~\ref{prop:fibre} and the
timing bound of Theorem~\ref{thm:delayed} --- have not been stated in this form; the elementary facts on which they rest (quantifier commutation; Dini comparison) are classical.
...
(Theorem~\ref{thm:exit}--\ref{thm:delayed} and Proposition~\ref{prop:emptiness}, Example~\ref{ex:hidden-mode}). Section 4 develops the certification limits
(Proposition~\ref{prop:fibre}, Corollary~\ref{cor:certainly-safe}, Remark~\ref{rem:ce-trap}). Section 5 reviews the sufficiency
...
\(\mathcal{R}_{\mathcal{V}}(x) = U(x)\). Two levels of the tangency condition are distinguished throughout. (i) \emph{Local reading.} The correspondence is applied to the constraint set itself: by Nagumo's theorem in
...
\(y \in O(X)\), in the constructions of Sections 3.2--3.4. The existential (non-robust) counterpart \(\mathrm{EViab}_{\mathcal{I}}(\mathcal{V})\) is the contrast class defined below.
...
(Theorem~\ref{thm:exit}) and \(D_{\eta}(x)\) (Theorem~\ref{thm:common-action}'s proof, local). The upper
...
obstruction of the next section and Example~\ref{ex:hidden-mode}'s hidden-parameter
...
(Theorem~\ref{thm:common-action}'s safety case) at minimal size, produced by the control-set primitive rather than by information; the genuine
...
epistemic infeasibility, the control-space analogue of the material-substitution separation certificates in the assessment-separation analysis of Abaee (2026).
...
\(\inf q / \varepsilon\). Theorem~\ref{thm:delayed} refines Theorem~\ref{thm:common-action} quantitatively:
...
preserves safety on a buffered subset: if \(K_\delta\) is a compact controlled-invariant subset of the interior of the kernel, the state
feedback \(u = k(x)\) is Lipschitz with constant \(L_k\), the observer satisfies \(\|\hat x(t) - x(t)\| \le M e^{-\alpha t}\|\hat x(0) - x(0)\|\), and
the initial estimation error is small enough that
\(L_k M \|\hat x(0) - x(0)\|\) is absorbed by the margin, then \(K_\delta\) is viable under output feedback (observer-to-viability
...
\(\partial K\), and estimation and implementation errors are bounded by \(\eta\), then an eroded set \(K^{-c\eta}\) is invariant
...
information-theoretic failure. The exit certificate (Theorem~\ref{thm:exit}) is the
...
one constant observation --- in the admissibility form of Section 3.2. The common-action obstruction (Theorem~\ref{thm:common-action}) is the static
...
under mild regularity are Maghenem and Sanfelice (2019). Theorem~\ref{thm:exit} is
...
(Proposition~\ref{prop:emptiness}, Theorem~\ref{thm:common-action}, Theorem~\ref{thm:delayed}, and Proposition~\ref{prop:fibre}) act at the level of the \emph{information set itself}:
...
a single class is what removes that exposure; refinement is never harmful (a finer observation merges nothing the coarser one did not), though Theorem~\ref{thm:common-action}
...
failure side lacked a comparable instrument. This paper supplies the missing instrument: the obstruction calculus develops sound, computationally interpretable certificates for nonviability --- sufficient conditions for nonviability, equivalently necessary conditions for viability. They do not exhaust the complement of the epistemic kernel (Section 6.5), and the paper claims no complete characterization.
...
of Sections 3--4. They are stated in full with their scope remarks.

\textbf{A.1 Example (coupling creates viability absent in a factor).}
Take the two-patch logistic system
\(\dot S_i = g_i(S_i) + \gamma\,(S_j - S_i) - h_i, \qquad i, j \in \{1,2\},\ i \ne j,\)
with \(g_i(s) = s(1-s)\), \(\gamma = 0.2\), constraints \(S_i \in [0,1]\) and \(h_i \ge h_{\min,i}\) (harvest floors), and admissible controls the constant harvest vectors \(h = (h_1, h_2)\) with \(h_i \ge 0\). Choose
...
\(\big(\dot S_1, \dot S_2\big)\big|_{p^*} = \left(\tfrac{\gamma}{2}(C_2 - C_1), \tfrac{\gamma}{2}(C_1 - C_2)\right) \ne 0.\)
