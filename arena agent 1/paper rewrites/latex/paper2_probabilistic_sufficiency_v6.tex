...
computed one (the edition's script).
\end{proof}
...
hypothesis into the recursion of Theorem~\ref{thm:recursion}: the finite
...
Proposition~\ref{prop:antichain}(i), \(V_{\ell}(b)\) is non-increasing
...
Theorem~\ref{thm:parametric} are length-independent in the stated
...
above the floor indefinitely; the edition's script certifies the frozen
values on the audited tables.
\end{proof}
...
monotonicity of \(V_{k}\) in \(k\) (Theorem~\ref{thm:support}) gives
\(\Delta_{k}(b) \ge \Delta_{1}(b) \ge \delta\) for every
\(k \ge 1\). (ii) At the deterministic limit
Proposition~\ref{prop:degen} gives \(V_{k}(b) = \max_{S} b(S)\) over
...
attainments are the computed records of the edition's script.
\end{proof}
...
Theorem~\ref{thm:parametric}; subtracting the two case displays, the gap
...
degeneration this paper's Proposition~\ref{prop:degen} records; the
...
Proposition~\ref{prop:additivelaw} at \(d_{0} = 1\), \(e_{p} = 0\):
...
\subsection{Distributionally robust interpolation}\label{dr}

The expected and adversarial readings of Remark \ref{rem:operators} are
...
\begin{proposition}[contamination interpolation]\label{prop:dr}
...
\ref{prop:noisyprobe}, now with its rate). At
...
\label{rem:eps0}
The exact split of Proposition~\ref{prop:learn} is the
...
(Proposition~\ref{prop:noisyprobe}).
\end{remark}

\begin{proposition}[general additive deadline law]\label{prop:additivelaw}
...
the deadline identity is the whole answer. The audited instance has
...
rational arithmetic, standard library only. Every theorem, proposition,
lemma, and corollary carries a proof; standard results are cited rather
than restated (Smallwood and Sondik, 1973; Bertsekas and Shreve, 1978;
Sperner, 1928). Three per-system scripts
cover the audited instances, and one master entry point chains them.
The computational companion in the P2 lineage (exact belief
computation, editions 1--2) carries the scaling record --- exact
point-based evaluation at probed beliefs, the antichain compression of
survivable-set lattices, and the four-parameter cube's Hamming
classification --- built on the alpha-vector representation of
Proposition~\ref{prop:pl}; its scripts chain this paper's seeds but
...
two-witness families are structural, not representative. The contamination interpolation of Section \ref{dr} is the formal home
...
\url{paper2_probabilistic_sufficiency_v6_verification.py} --- which chains
