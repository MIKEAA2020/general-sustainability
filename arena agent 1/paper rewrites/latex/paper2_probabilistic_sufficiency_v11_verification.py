#!/usr/bin/env python3
"""
Probabilistic sufficiency for the obstruction calculus — edition 7 verification.
...
for z0 in Z:
    for T in TS:
        for k in KS:
            ...
            ones += (v == 1)
            halves += (v == Q(1, 2))
check("grid: closed form = enumeration, census 36 ones / 156 halves",
...
tex = open(os.path.join(HERE, "paper2_probabilistic_sufficiency_v11.tex"),
           encoding="utf-8").read()
check("keyword needle: POMDP", "POMDP" in " ".join(tex.split()))
check("keyword needle: Lovejoy", "Lovejoy, W.S., 1991" in " ".join(tex.split()) and "Lovejoy, 1991" in " ".join(tex.split()))
tnorm = " ".join(tex.split())

NEEDLES = [
    "Probabilistic Sufficiency for the Obstruction Calculus",
    "an antichain, Sperner-bounded",
    "why the certified witness census realizes three types",
    ...
    "the certified two-witness families are structural, not representative",
    "paper2_probabilistic_sufficiency_v11_verification.py",
    "with six results",
...
FORBIDDEN = [
    ...
    "Every theorem, proposition", "cited rather than restated",
    "edition's script", "P2 lineage", "audited", "chained",
]
...
ptr_ok = all(tnorm.count(s) == 1 for s in
             ["paper2_probabilistic_sufficiency_v11_verification.py"]
             + SEEDS)
check("code-availability pointers: the verification script and each of the "
      "three per-system scripts named exactly once",
...
hyg = True
for fn in ("paper2_probabilistic_sufficiency_v11.tex",
...
_x = " ".join(tex.split())
_needles = [
    "exact belief computation and its certification",
...
# (16) proof-completeness: every theorem-like environment carries a proof
import re as _r6
_envs = len(_r6.findall(r"\\begin\{(?:theorem|proposition|lemma|corollary)\*?\}", tex))
_proofs = tex.count("\\begin{proof}")
check(f"proof completeness: {_envs} theorem-like environments, {_proofs} "
      "proof environments (parity; standard results cited in-proof)",
      _envs == _proofs and _envs >= 18)

# (17) standard-literature citations present in proofs + code pointer
_n17 = ["Smallwood and Sondik, 1973", "Bertsekas and Shreve, 1978",
        "Sperner", "Mathematische Zeitschrift 27",
        "paper2_probabilistic_sufficiency_v11_verification.py"]
_x17 = " ".join(tex.split())
_miss17 = [s for s in _n17 if " ".join(s.split()) not in _x17]
check("standard-result citations and methods sentence present"
      + ("" if not _miss17 else f" (missing: {_miss17})"),
      not _miss17)
