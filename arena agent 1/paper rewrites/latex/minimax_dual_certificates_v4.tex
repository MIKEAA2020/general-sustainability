...
the source audit; the present paper verifies its
...
\section{The dynamic envelope: root cause and correction}

One dynamic proposal attached to the same audit --- an adversarial measure
curve $t \mapsto \mu^{*}_{t}$ whose expected-margin envelope $Q(t)$ would
decrease at a certified rate, governed by an information-set operator ---
was rejected on three named grounds: the operator is nonlocal in the state
and is not a well-posed Hamilton--Jacobi equation as written; no viscosity
well-posedness or comparison argument was supplied; and an expectation
envelope's crossing of zero is not a trajectory violation without a
selection argument. This section performs the root-cause analysis, corrects
the formulation, and fills the gaps to the extent that exact methods reach;
the honest residue remains conjectural.

\subsection{Root causes}

\emph{(i) A category error in the clothing.} The proposed operator maps a
...
program. The nonlocality was never the defect; the differential clothing
was.
...
even be posed. The correction replaces the curve by a single adversarial
prior $\mu^{*}$ on disturbance paths; conditional expectation then supplies
...
\subsection{The corrected object}
...
Supplementary Material, S1). The recourse certificate is the corrected
dynamic envelope's one-step evaluation.
\end{proposition}
\begin{proof}
...
\begin{conjecture}[the corrected open residue]\label{conj:envelope}
The continuous-time residue of the corrected formulation: (i) an evolution
...
couplings --- under which the envelope obeys a dynamic-programming equation
...
The corrected envelope is one review deep so far. This section certifies
...
sequences and proves it in full; the instance's proof was the template,
and nothing in the general argument goes beyond finite combinatorics and
...
consistency --- each marginal $\mu^{*}_{k}$ is the time-$k$ law of the
one measure $\mu^{*}$ --- is exactly the requirement whose absence sank
the original proposal. Reviews divide the sequence: the review
...
expectation-to-realization bridge (the lemma of the corrected
formulation) converts strict negative expectation into a
positive-measure set of strictly negative realizations.
\end{proof}

\begin{remark}[the recourse certificate and the residue, after the
general theorem]\label{rem:after-general}
...
proposition of the corrected formulation), so the calculus's recourse
...
coupling as a function of the state and review history --- which is the
finite-space shadow of the martingale-transport flavour recorded in the
conjecture. \emph{Third}, refinement monotonicity (iii) is the
...
passage to continuous disturbance sets.
\end{remark}

\section{What else is not claimed}\section*{Declarations}
...
\texttt{minimax\_dual\_certificates\_v4\_verify.py} is publicly available
