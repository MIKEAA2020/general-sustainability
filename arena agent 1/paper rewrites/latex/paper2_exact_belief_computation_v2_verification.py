#!/usr/bin/env python3
"""
Exact belief-state computation at scale II — edition 2 verification.

Re-derives every claim of paper2_exact_belief_computation_v2.tex in exact
...
r = subprocess.run([sys.executable,
                    os.path.join(HERE, "paper2_exact_belief_computation_v1_verification.py")],
                   capture_output=True, text=True)
check("chained script exits green: paper2_exact_belief_computation_v1_verification.py "
      "(11/11)", r.returncode == 0 and "11/11" in r.stdout)
...
tex = open(os.path.join(HERE, "paper2_exact_belief_computation_v2.tex"),
           encoding="utf-8").read()
...
check("code pointer: the verification script named exactly once",
      tnorm.count("paper2_exact_belief_computation_v2_verification.py") == 1)
...
raw_tex = open(os.path.join(HERE, "paper2_exact_belief_computation_v2.tex"),
               "rb").read()
