...
realized by taking, on each block, the piecewise-constant control
attained by the finite optimizer's scenarios, whose exit from the
continuous value is charged exactly by the three error terms --- the
input radius over the control mesh, the moment error, and the Lipschitz
remainder over the time mesh. The asymmetry at its core is
...
\(\sum_j \lambda_j n_j = 0\) --- the unique pooling weights on the
equal-\(\{2,3\}\) family (they satisfy
\(\tfrac38 = 1 - 2 \cdot \tfrac{5}{16}\), so a barycentric rendering
\(\tfrac{6}{16} + \tfrac{5}{16} + \tfrac{5}{16}\) clamps to the same
point). Before observation the pooled kernel is
...
regenerates it. The library is not a reimplementation of this paper's bridge theorems: it carries the exact finite machinery they reduce from, while the bridge's programs live in the campaign scripts of Section~ef{methods}; consolidating those scripts as a 	exttt{viacert.continuous} module is the library's recorded next step (and, with it, the retirement of the present scripts' superseded audit identities), so the title's ``reference library'' is read as this finite-side package plus the certified campaign pipeline.
...
The verification scripts \url{paper2_computational_certification_v2_verification.py},
