#!/usr/bin/env python3
"""
Worked-Systems Audit flagship — master verification.

Layer 1 (chain): runs the seven lineage scripts as subprocesses and asserts
each exits green (8+8+9+8+8+8+8 = 57 checks).
Layer 2 (own): recomputes the crown-jewel counts from the shared audit
system's primitives — the pair family, the codex-threaded winning recursion
(no-repeat threaded as a parameter), the partition refutation, the benchmark
...
paper2_worked_systems_v1.tex.

Exact rational/integer arithmetic; stdlib only; deterministic.
Run: python3 paper2_worked_systems_v1_verification.py
"""
...
# ---------------- Layer 2a: the crown jewels, recomputed -----------------------
...
tex = open(os.path.join(HERE, "paper2_worked_systems_v1.tex"), encoding="utf-8").read()
NEEDLES = [
    "884.6", "27/5", "21/100", "12,999" if False else "(2, 2)",  # placeholder guard
]
NEEDLES = [
    "T_{\\mathrm{obs}} \\le \\sigma^{*}(z_0) = z_0 - 1",
    "(2, 1)", "(2,2)", "\\{(1,2),(2,2)\\}", "\\{(2,1),(2,2),(3,1)\\}",
    "24", "26", "25", "28", "C(8,2)", "C(9,2)",
    "256", "21/100", "3/50", "47/25",
    "-1/10", "(1/2, 1/2)", "(6/5, 4/5)", "27/5",
    "3/2 - (Y-2)/10", "59/50 - (Y-2)/10", "Y - 4",
    "\\lfloor z_0 - 1 \\rfloor", "\\{z_0 \\ge 1 + k\\}",
    "no-repeat codex", "floor", "57",
]
...
check("declarations carry the AI line and the master script name",
      ...
      and "paper2_worked_systems_v1_verification.py" in tex)
