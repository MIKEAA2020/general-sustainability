% LaTeX source generated from paperE3_edwards_forecast_ladder_v15.md by wave11/build_latex_author.py (batch 7 (audits of agent arena 1 paper rewrites)/wave11).
...
13.23 ft), because annual recharge is near-white: corr(R\_t, R\_\{t\ensuremath{-}1\})
= 0.17 against corr(\ensuremath{\Delta}H\_t, R\_t) = 0.74. The univariate AR(1) improves
...
coin-flip retention recorded by a point-RMSE rule, not a skill claim.
The water-balance-identified affine map with climatological fluxes (M2m)
is the best one-step forecaster tested (12.28 ft, the only margin that
separates from noise against persistence); it is listed and then
declined by a protocol class clause outside the frozen retention rule.
Given realized future recharge and pumpage, the same map reaches 7.55 ft
--- a nowcast, not a forecast; no signal at the annual forecast origin
recovers that gap. Climate modules beat persistence and the AR(1) by at
most 0.13 ft and lose to climatological fluxes; none is retained. At
five years, the training mean (16.80 ft) beats persistence (21.11 ft)
