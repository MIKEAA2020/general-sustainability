...
Exact rational arithmetic; stdlib only; deterministic.
  ...
  E6  THREE-FLOOR STACKED INSTANCE: {u <= 2/5} ^ {u >= 3/5} ^ {u <= 9/10} is
      ...
      multiplier zero (support reading: the certificate's carrier is a
      minimal infeasible subsystem), and the certificate remains exact.
  E7  MINIMAL INFEASIBLE SUBSYSTEM (joint audit round 4): in the E6
      three-row stack the certificate's support {row1, row2} is THE minimal
      infeasible subsystem — the other two-row subsystems are feasible
      (witnesses exhibited), so the zero multiplier on the inactive row is
      forced, not assumed (support reading).
  E8  CORNER FACT: under two floors the corner (1,1) has NO instrument
      keeping both floors (singleton nonviable), while under the single
      floor q1 the same corner is viable — the E4 separator's mechanics,
      pinned.
"""
...
check("E6 three-floor stacked instance: certificate lambda = (1,1,0) — the "
      "inactive row receives multiplier zero (support reading: minimal "
      "infeasible subsystem)", e6)

# ---- E7: minimal infeasible subsystem (support reading) — v2 ----
grid_u = [Q(n, 100) for n in range(0, 101)]
f13 = any(u <= b3[0] and u <= b3[2] for u in grid_u)
f23 = any(-u <= b3[1] and u <= b3[2] for u in grid_u)
i12 = not any(u <= b3[0] and -u <= b3[1] for u in grid_u)
check("E7 minimal infeasible subsystem: rows {1,3} feasible (u=0), rows "
      "{2,3} feasible (u=7/10), rows {1,2} infeasible — the support of "
      "lambda=(1,1,0) is THE minimal infeasible subsystem (zero multiplier "
      "on the inactive row forced)", f13 and f23 and i12 and lam3[2] == 0)

# ---- E8: corner fact pinning E4's separator — v2 ----
c_two = not verdict(frozenset({(1, 1)}), full)
c_one = verdict(frozenset({(1, 1)}), full, one_floor)
sepB = frozenset({(1, 1), (1, 2)})
check("E8 corner fact: under two floors {(1,1)} is nonviable (no instrument "
      "keeps both floors) and so is the pair {(1,1),(1,2)}, while under one "
      "floor both are viable — E4's separator mechanics pinned",
      c_two and c_one and not verdict(sepB, full)
      and verdict(sepB, full, one_floor))
