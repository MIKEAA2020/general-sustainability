...
% Amin Abaee. Revision v31 (review build with line numbers; retargeted for JEDC). Compiles with tectonic, pdflatex, or xelatex.
...
\textbf{Keywords:} sustainability; viability theory; robust control;
scalarization; multi-criteria decision analysis; quantifier order;
transition safety.

\textbf{JEL codes:} C61; C62; D81; Q01; Q56.
...
\tightlist
\item
  Scalarized and coordinate-wise sustainability criteria are compared as robust transition-safety problems on a common datum.
\item
...
quantifier orders below make this divergence precise.
...
separately, as in Section 4.5.) Write \(D = \{d_a : a \in \mathcal{A}\}\).}
\emph{Then:}
...
\;=\; \operatorname{conv}(D) + \mathbb{R}^n_+;
\]

\emph{hence the acceptance gap is}
\[
\bigl[\operatorname{conv}(D) + \mathbb{R}^n_+\bigr]
\;\setminus\;
\bigcup_{a \in \mathcal{A}} (d_a + \mathbb{R}^n_+),
\]
...
\subsubsection{4.7 Figures}\label{figures}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.98\linewidth]{figs_p1/fig2_weight_intervals_v31.png}
...
\item
  ...
  collapses into universal rejection. The separation is exhibited for, and scoped to, the specified action-indexed class: the benchmark assumes separable transition risks, with each plan exposing a different floor to its own adverse disturbance. That the coupled regime is not a remote possibility is indicated by evidence that planetary-boundary processes interact and amplify one another (Lade et al., 2020).
\item
...
different ways; Table 1 records them.
...
\midrule
Reweighting & change the weight \(w\) & does not close the full-cone gap \\
...
Menu convexification & replace \(D\) by \(\operatorname{conv}(D)\) (the blend family) & closes the gap exactly on the scalarized feasible region \\
