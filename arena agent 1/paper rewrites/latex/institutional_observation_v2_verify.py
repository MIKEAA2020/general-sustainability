...
coordinator reading (z1, z2) (= the full structure).
  ...
  E6  DELAYED OWN READING: agency 2 with a one-step delayed reading
  ...
  E7  LAW SPACE AND DEADLOCK RULE (joint audit round 4): 256 = 2^4 x 2^4
      law pairs (each agency: four coordinate values, two votes); the
      deadlock transition as declared (u = 0: both coordinates lose one);
      the loss instance {(1,2),(3,1)} pinned viable under each agency
      alone and the coordinator, nonviable under every law pair.
  E8  TWO SIXTEENS DISTINGUISHED: 16 lost pairs (beliefs) and 16
      sustaining law pairs for the audited belief — a coincidence of this
      grid, no identity.
"""
...
check(f"E6 delayed own reading: with agency 2 reading previous-step "
...
# ---- E7: law space + deadlock rule + loss instance pinning — v2 ----
law_ok = len(LAW_PAIRS) == 2 ** 4 * 2 ** 4 == 256
deadlock_rule = all(step(x, 0) == (max(0, x[0] - 1), max(0, x[1] - 1))
                    for x in [(0, 0), (1, 2), (2, 3), (3, 3)])
B_inst = frozenset({(1, 2), (3, 1)})
inst_ok = (verd_full(B_inst, z1only) and verd_full(B_inst, z2only)
           and verd_full(B_inst, full_lab) and not dec_viable(B_inst))
check("E7 law space and deadlock rule: 256 = 2^4 x 2^4 law pairs (four "
      "coordinate values, two votes per agency); deadlock transition as "
      "declared (u = 0: both coordinates lose one); the loss instance "
      "{(1,2),(3,1)} viable under each agency alone and the coordinator, "
      "nonviable under every unanimity law pair",
      law_ok and deadlock_rule and inst_ok)

# ---- E8: the two sixteens distinguished — v2 ----
check(f"E8 the two sixteens distinguished: {n_loss} lost pairs (beliefs) "
      f"and {n_antisy} sustaining law pairs for the audited belief — a "
      "coincidence of this grid, no identity (beliefs are not law pairs)",
      n_loss == 16 and n_antisy == 16)
