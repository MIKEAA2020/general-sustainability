% LaTeX source generated from paperE4_edwards_intervention_v13.md by wave11/build_latex_author.py (batch 7 (audits of agent arena 1 paper rewrites)/wave11).
...
\graphicspath{{../}}
\begin{document}
\title{Governance operators and viability kernels of the Edwards Aquifer: an intervention-selection test at J-17}
...
\textbf{Problem.} Drought management in permitted groundwater systems
relies on triggers and pumping reductions; the protection competing
rules deliver under adverse recharge is rarely scored against a fixed
criterion.

\textbf{Approach.} We compute robust viability kernels of the J-17
annual-mean head for a declared policy family --- training-mean pumping
(282.16 \ensuremath{\times} 10\textsuperscript{3} acre-ft yr\textsuperscript{-1}, not current use), flat caps of 0--90\%, a
Stage-I reactive rule, and the critical-period-management (CPM) cascade
--- under persistent drought-floor recharge. A non-BAU policy is
retained only if at least as protective as training-mean pumping and
more permissive than the smallest securing flat cap, under a protocol
frozen before any score was computed.

\textbf{Results.} At the 618-ft physical threshold under the
drought-of-record floor, training-mean pumping's worst-case attractor
(615.72 ft) lies below the threshold, its kernel emptying beyond 13
years; every flat cut of 10\% or deeper and both reactive rules make the
safe set invariant; the smallest securing cut is interpolated 7.2\% of
training mean pumping (31.5\% of current). The reactive rules are
retained \textbf{nominally} at 618 ft under the mildest floor class
only: their supply margins (+3.3\% Stage I, +0.4\% cascade, against the
kernel-matched flat-90\% cap) are hybrid --- worst-case protection
against wet-year replay entitlement the robust class excludes --- and
under the floor classes a 0.4\% trigger lag. Retention is \textbf{not}
certified (certified positive-pumping kernels empty beyond T = 3 years,
a bound the out-of-sample defect makes optimistic), and \textbf{nothing
is retained at 660 ft}, where the reactive rules are invisible to the
kernel of their own trigger --- a geometric property of trigger design
(the cascade's purpose is springflow protection near 618 ft, not head
invariance).

\textbf{Implications.} Reactive rules match flat-cap protection at the
physical threshold and seem generous only if wet-year pumping counts;
the 660-ft threshold is protected by wet years, not the pumping family;
certified claims are limited to three years by the fitted defect.
...
\textbf{Article Impact Statement.} Reactive trigger rules on the Edwards
Aquifer match flat-cap protection at J-17's 618-ft physical threshold at
3.3\% (Stage I sketch) and 0.4\% (cascade) more permitted supply than
the kernel-matched flat-90\% cap --- nominal, hybrid-supply results; the
660-ft threshold is protected by wet years, not by pumping rules.
...
\subsubsection{2.1 Object and dynamics}\label{object-and-dynamics}
...
1934--2023, drawn from the registered twenty-column analysis panel. The
...
sets}\label{uncertainty-classes-and-safe-sets}
...
\subsubsection{2.3 Governance family}\label{governance-family}
...
layer}\label{retention-rule-and-certified-layer}
...
cut}\label{worst-case-attractors-and-the-minimal-cut}
...
\subsubsection{3.2 Nominal kernels}\label{nominal-kernels}
...
\subsubsection{3.3 Certified kernels}\label{certified-kernels}
...
\subsubsection{3.4 Supply and retention}\label{supply-and-retention}
...
replays}\label{classification-and-stress-replays}
...
supply (post-freeze
layers)}\label{finite-duration-recharge-floors-and-floor-class-supply-post-freeze-layers}
...
layer)}\label{attractor-uncertainty-post-freeze-layer}
...
declared classes, and the certified content is defect-bound to T \ensuremath{\leq} 3
years.
...
Companion forecast-evaluation study.

Author, B., et al.~In review. Surplus-production intervention selection
under a persistent productivity floor. Companion intervention study
