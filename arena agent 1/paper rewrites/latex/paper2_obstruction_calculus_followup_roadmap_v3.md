...
| 2 | Abstract's finite-checkability claim vs the timing obstruction | Phrasing fixed in v31 (verify survives); the substantive close is option (b) — see A5 |
...
| 6 | Flagship timing certificate lacks a general computational instantiation | **Open → A5** |
...
| 10 | "Sound but not complete" never quantified | **Open → A6** |
...
**A5 (theorem candidate — owner-gated; not required for submission).
...
must close or bound. If proven pre-submission it enters Section 3.5;
otherwise it is a revision item.

**A6 (theorem candidate — owner-gated; not required for submission).
...
Upgrades the central concession into a boundary result.
...
## Track B — Companion paper (B1–B5 only): recourse-aware continuous-to-finite certification
...
| 3 | A5/A6 theorem candidates (owner gate; enter §3.5/§6.5 if proven pre-submission, else revision) | 1 in progress |
