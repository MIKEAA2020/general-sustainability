...
the Lipschitz hypothesis (6) extends the bound to every time
(\(|\mathcal F_a(v^{*}) - \mathcal F_{\hat a}(v^{*})| \le L|t - \hat t|
\le L h_t\) for any time label within \(h_t\) of the grid label
\(\hat a\)), giving
\(\max_a \mathcal F_a(v^{*}) \le \rho_{\mathcal H,G} + \max_{\hat a
\in \mathscr S_G} \bar e_{\hat a} + \delta_\beta + L h_t\). Since
\(J_{\mathcal I}(T)\) minimizes over all
admissible policies, the upper bound follows. Together with (4) this gives
the sharpened form; the uniform choice \(\bar e \equiv R_U V_U h_u\)
--- valid by the moment-error bound (2) whenever \(\mathrm{TV}(k_a) \le
V_U\) --- recovers the announced special case. The asymmetry --- averaging arbitrary controls proves
...
\(\gamma = J_{\mathcal I}(T) > 0\) and choose meshes and stored scenarios
making the error in (8) --- including the stored-scenario widths
\(\delta_{\mathrm{init}}\) and \(\delta_{\mathrm{num}}\), which
near-maximizing stores contribute and completeness must drive to zero ---
smaller than \(\gamma\); then \(\rho_{\mathcal H,G} > 0\) and Step 4's
...
(\(\|k_a - k_{a'}\|_{L^1} \to 0\), \(|\beta_a - \beta_{a'}| \to 0\);
the \(L^1\) form is the operative one --- the sup-norm form is false for
the step kernels, which jump at \(\sigma = t\) for every \(t\)),
\[
\sup_\pi \, |\mathcal F_a(\pi) - \mathcal F_{a'}(\pi)|
\;\le\; M_U \, \|k_a - k_{a'}\|_{L^1}
\;+\; |\beta_a - \beta_{a'}| \;\to\; 0,
\]
with \(M_U := \sup_{v \in U} \|v\|\) (H\"older: the policies take
values in \(U\), so each pairing against an \(L^1\) kernel difference
is bounded by \(M_U\) times its \(L^1\) norm),
...
\textbf{Derivation of the moduli from primitive data.} The hypothesis
constants of Theorem 4 are not free assumptions; under the standing bounds
they are computed from the data. Let
\(\|A_\theta\|_\infty \le \bar A\), \(\|B_\theta\|_\infty \le
\bar B\), \(\|E_\theta\|_\infty \le \bar E\),
\(\|g_\theta\|_\infty \le \bar g\), \(\|c_i\| \le \bar c\) on
\([0,T]\); \(U \subset c_U + R_U\mathbb B\),
\(D_\theta \subset R_D\mathbb B\), \(P_\theta \subset R_P\mathbb B\);
\(\mathrm{TV}(B_\theta) \le V_B\), \(\mathrm{TV}(E_\theta) \le
V_E\); and \(M_X := e^{\bar A T}\), so that
\(\|X_\theta(t, \sigma)\| \le M_X\) (Gr\"onwall on the transition
matrix) and \(\tfrac{\partial}{\partial \sigma}
X_\theta(t, \sigma) = -X_\theta(t, \sigma) A_\theta(\sigma)\).

\emph{Total variation of the kernels.} For
\(k_a(\sigma) = \mathbf 1_{\{\sigma \le t\}}
B_\theta(\sigma)^\top X_\theta(t, \sigma)^\top c_i\): on
\([0, t]\) the product rule for functions of bounded variation gives
\[
\mathrm{TV}(k_a; [0, t]) \le \sup_\sigma
\|X_\theta(t, \sigma)^\top c_i\|\, \mathrm{TV}(B_\theta)
+ \sup_\sigma \|B_\theta(\sigma)\| \int_0^t
\|X_\theta(t, \sigma) A_\theta(\sigma)^\top c_i\|\, d\sigma
\le \bar c \bigl( M_X V_B + \bar B M_X \bar A T \bigr),
\]
and the drop to zero at \(\sigma = t\) adds at most
\(\|B_\theta(t)^\top c_i\| \le \bar B \bar c\) (using
\(X_\theta(t, t) = I\)). Hence (7) holds with
\(V_U = \bar c (\bar B + M_X V_B + \bar B M_X \bar A T)\), and
identically for the disturbance kernel with
\(V_D = \bar c (\bar E + M_X V_E + \bar E M_X \bar A T)\).

\emph{\(L^1\) label-continuity modulus.} The same bounds give the
\(L^1\) continuity in the label time that the proof of (9) consumes (the
sup-norm form is false here: the step kernels jump at \(\sigma = t\) for
every \(t\)). For \(t \ge t'\), Gr\"onwall on
\(X_\theta(t, \sigma) - X_\theta(t', \sigma) = \int_{t'}^{t}
A_\theta(r) X_\theta(r, \sigma)\, dr\) gives
\(\|X_\theta(t, \sigma) - X_\theta(t', \sigma)\| \le M_X \bar A
|t - t'|\) uniformly in \(\sigma\), so with
\(k_a = \mathbf 1_{\{\sigma \le t\}} B_\theta(\sigma)^\top
X_\theta(t, \sigma)^\top c_i\),
\[
\|k_t - k_{t'}\|_{L^1} \;\le\; \bar B \bar c\, |t - t'| +
\int_0^{t'} \bar B \bar c\, M_X \bar A\, |t - t'|\, d\sigma
\;\le\; \bar B \bar c\, (1 + M_X \bar A T)\, |t - t'|,
\]
and \(|\beta_t - \beta_{t'}| \le L^{\mathrm{tr}} |t - t'|\) by the
trajectory Lipschitz bound below: the label map is \(L^1\)-Lipschitz in
\(t\) with constant \(\bar B \bar c (1 + M_X \bar A T) +
L^{\mathrm{tr}}\), which is the form the label-continuity hypothesis
consumes.

\emph{Uniform time-Lipschitz constant.} For a trajectory
\(x\) of mode \(\theta\): \(\|x(t)\| \le M_X \bigl(R_P + T(\bar g
+ \bar B (R_U + \|c_U\|) + \bar E R_D)\bigr)\) by Gr\"onwall, and
\[
\bigl| \tfrac{d}{dt} \, c_i^\top x(t) \bigr|
\le \bar c \bigl( \bar A \|x(t)\| + \bar g + \bar B (R_U +
\|c_U\|) + \bar E R_D \bigr)
\le L^{\mathrm{tr}},
\]
with \(L^{\mathrm{tr}} := \bar c \bar A M_X R_P + \bar c
\bigl(1 + \bar A M_X T\bigr) \bigl( \bar g + \bar B (R_U +
\|c_U\|) + \bar E R_D \bigr)\). Each \(\mathcal F_{(\theta,i,t)}(\pi)\)
is a supremum over \((x_0, d)\) of the \(L^{\mathrm{tr}}\)-Lipschitz
functions \(t \mapsto c_i^\top x(t) - b_i\), and a pointwise supremum of
\(L\)-Lipschitz functions is \(L\)-Lipschitz; hence (6) holds with
\(L = L^{\mathrm{tr}}\). For the three-branch instance the closed form
applies (with \(\bar A = 1\), \(M_X = e^{6/5}\), \(R_P = 34/25\),
\(\bar B = 1\), \(R_U = 1\)); the direct kernel computation of
Section~6 gives the sharper instance constants \(V_U = 6/5\),
\(L = 11/5\), \(R_U = 1\), \(\delta_\beta = 0\) used in the
sandwich reading of the campaign.
\rulebar
...
rest; Section~6 of the main text). The witness pair is mesh-generic on
the aligned family: re-running it exactly at the sixth aligned mesh
\(h = \tfrac1{15}\) (\(Q = 48\), \(687\) inequalities) certifies
\(\rho = \tfrac1{25}\) there, extending the law's verified domain
beyond the five campaign meshes. Current run: 9/9 checks pass; solver agreement at most
