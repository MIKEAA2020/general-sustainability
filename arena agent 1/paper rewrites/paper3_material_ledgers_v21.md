...
Depletion numbers circulate under a single label while answering different questions. This article separates them with a typed stock–flow accounting layer. Conservation follows from incidence structure. Positivity follows from donor limitation. Services are readouts rather than conserved mass. Each depletion quantity carries its own classification. Three certification layers carry a flux-reconstruction identity, a conservation-law reduction, and a flux-bounding envelope theorem. The closed finite-donor ledger carries the theorem set: the natural-block mass identity, orthant invariance, no interior rest at positive effort, the vanishing-extraction rest set, and extraction integrability. Depletion time separates into three non-interchangeable quantities — gross turnover intensity, frozen-rate ratio, scenario-conditioned hitting time — extended to upper barriers, with uniform-drift bounds. Three public-data applications are classified at their exact status. The G3P anomaly index is a statistical index, not a stock ratio. The phosphate reserve-life ratio is arithmetic, not a forecast. The fisheries removals-only time is a pressure scale, not a depletion diagnostic. Weak and strong sustainability are two regimes of one system, distinguished by whether the material cycle closes at the rate of use. Weak sustainability is the idealized regime in which slow consumption and population let substitution (dominant, with regeneration far slower) redistribute matter so that it is used as it arises; waste is then only what accumulates when that redistribution fails in time. What the ledger does not support is stated explicitly: no nonnegative weighting certifies componentwise nonnegativity, and five double-counting rules block phantom mass. First-passage semantics on declared stochastic surrogates carry explicit non-claims. An interface contract fixes the shared object with the delay-based dynamics.
