...
## S5. Fisheries cohort statistics — database-version sensitivity

The main-text fisheries column reports the pure-decay proxy $\mathrm{ADH} = F^{-1}\log(\mathrm{SSB}_{\mathrm{now}}/(0.2\max\mathrm{SSB}))$ with median $\approx 1.8$ yr over the archived 43-stock cohort (RAM Legacy extract of the Ricard et al., 2012 vintage; qualifying filter $F > 0$ and $\mathrm{SSB}_{\mathrm{now}} > 0.2\max\mathrm{SSB}$; pull archived in the analysis repository). The quartile summary of $F$ and $\log(\mathrm{SSB}/B_{\lim})$ over that cohort is defined by, and is quoted from, that archived pull alone; no other database version's cohort statistics substitute for it.

Because cohort composition is database-version-dependent, this section records the corresponding statistics of the public database versions under the same filters (most-recent-assessment view per stock; $F$ column; current values at the last finite series year):

| RAM Legacy DB version | cohort size | median ADH (yr) | $F$ quartiles Q1 / median / Q3 | $\log(\mathrm{SSB}/B_{\lim})$ quartiles Q1 / median / Q3 |
|---|---|---|---|---|
| v4.44 (2018, Zenodo 2542919) | 352 | 3.65 | 0.106 / 0.260 / 0.575 | 0.657 / 1.099 / 1.415 |
| v4.495 (2021, Zenodo 4824192) | 362 | 3.72 | 0.100 / 0.238 / 0.524 | 0.667 / 1.051 / 1.421 |

The ER-column convention gives larger cohorts with medians $\approx 6.6$–$7.1$ yr. Neither public version reproduces the archived cohort (43 stocks, median $\approx 1.8$ yr) under the stated filters: the divergence documents the version-dependence that motivates pinning the main-text statistic to the archived pull, and it is why the archived cohort's quartiles are not estimated from any public version.
