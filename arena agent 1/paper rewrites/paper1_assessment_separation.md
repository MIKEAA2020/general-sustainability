...
Sustainability assessments routinely aggregate heterogeneous capital stocks and floors into single indices, on the implicit premise that a deficit in one dimension can be compensated by a surplus in another. This paper proves a separation result for a typed transition datum under exact-tube semantics showing where that premise fails dynamically. For every nonnegative scalarization weight $w$, an action exists that satisfies the aggregate floor $w\cdot s \ge 0$, while no single action satisfies all typed floors $s_i \ge 0$ simultaneously; the per-weight accepted state set therefore strictly contains the common-plan accepted state set. On an explicit rational witness the gap contains a relatively open region, which a resource-controlled action partitions into a rescue set and a genuine impossibility set: states of the former type can be bridged into the noncompensatory assessment at a defined resource cost, while states of the latter admit no typed-safe transition under any weight. The continuum statements are proved in full; a deterministic exact-integer computation verifies the finite rational instance. The result is positioned against the weak-comparability foundation of ecological economics, the indicator-based characterization of sustainability, and the maximin formulation of strong sustainability, and the implications for composite indices are stated at their actual strength.
...
The first implies the second; the converse fails. The failure is not a defect of any particular weight: it is the noncommutativity of the existential quantifier over actions with the universal quantifier over weights. The paper proves the always-valid inclusion (Theorem 3), proves the pointwise identity of the noncompensatory accepted set with the common-plan set over the full cone (Theorem 3(ii)), isolates the general mechanism (Proposition 1), and exhibits an explicit rational witness on which the gap between the two acceptance criteria is a region with nonempty interior — split by a resource-controlled action into a *rescue set*, whose states can be bridged into the noncompensatory assessment at a defined resource cost, and a *genuine impossibility region*, whose states admit no typed-safe transition under any weight (Theorem 5).

### 1.3 Contribution and scope

**Claimed.** (i) The action-set identity $E_{\mathrm{typ}}(z) = \bigcap_{w \in C} E_w(z)$, with the closed-cone choice isolating the separation as purely dynamic (Theorem 3(ii)). (ii) A general quantifier-separation proposition (Proposition 1). (iii) Monotonicity of the accepted-state hierarchy in the weight family (Theorem 4). (iv) An explicit rational witness with a relatively open region of strict separation and the rescue/impossibility split, including the exact per-weight licensing thresholds (Theorem 5). (v) Persistence of the separation under hold-prefix extension of the horizon (Theorem 6).
...
Fix a typed exact-tube transition datum with transition-safe sets and a destination set. For a state $z$ and action $a$, let $\mathrm{Tube}(a,d)$ be the full set of states visited during the interval under disturbance $d$, let $\mathrm{End}(a,d)$ be the endpoint values visited, and let $\mathrm{Succ}(a,d)$ be the successor set after any endpoint reset. Four operators are distinguished; they share the same disturbance quantifier and differ only in constraint structure.
...
The **scalarized aggregate** operator at weight $w \in C = \mathbb{R}^n_+ \setminus \{0\}$:
...
holds for every $w \in C$ (Theorem 3(i) below). The last inclusion is strict in general, though it collapses on the witness of Section 4.5 because the witness's physical constraint touches only the monotone reserve stock.
...
$$\mathcal{V}_{\mathrm{weak}} \;=\; \bigcap_{w \in C} \mathcal{V}_w .$$
...
*Equality holds if and only if, for every $z$ in the right-hand set, the family $\{E_\lambda(z)\}_\lambda$ admits a common selector — equivalently, the family has the finite-intersection property together with a compactness/closedness condition guaranteeing $\bigcap_\lambda E_\lambda(z) \neq \varnothing$.*
...
### 4.2 The closed-cone identity

**Lemma 2 (closed-cone pointwise equivalence).** *For $v \in \mathbb{R}^n$:*
$$v \ge 0 \ \text{componentwise} \iff w \cdot v \ge 0 \ \text{for every } w \in C = \mathbb{R}^n_+ \setminus \{0\}.$$

*Proof.* ($\Rightarrow$) $w \ge 0$, $w \neq 0$, $v \ge 0$ gives $w \cdot v = \sum_i w_i v_i \ge 0$. ($\Leftarrow$) If $v_k < 0$, take $w = e_k \in C$: then $w \cdot v = v_k < 0$. $\square$
...
*(i) Hierarchy.* For every $w \in C$ and every $z$:
$$E_{\mathrm{typ}}(z) \;\subseteq\; E_w(z) \;\subseteq\; E_{\mathrm{tube,phys}}(z) \;\subseteq\; E_{\mathrm{end}}(z).$$

*(ii) Localization.* For every $z$:
$$E_{\mathrm{typ}}(z) \;=\; \bigcap_{w \in C} E_w(z).$$
Hence $\mathcal{V}_{\mathrm{typ}} = \{ z : \bigcap_w E_w(z) \neq \varnothing \}$.

*Proof.* (i) Let $a \in E_{\mathrm{typ}}(z)$. For every disturbance $d$, $\mathrm{Tube}(a,d) \subseteq S = S^{\mathrm{phys}} \cap \{s \ge 0\}$; by Lemma 2 every tube point satisfies $w \cdot s \ge 0$ for every $w \in C$, and likewise every successor state lies in $G^w$; hence $a \in E_w(z)$. The remaining inclusions follow from $S^w \subseteq S^{\mathrm{phys}}$, $G^w \subseteq G^{\mathrm{phys}}$, and $\mathrm{End}(a,d) \subseteq \mathrm{Tube}(a,d)$. (ii) The inclusion $\subseteq$ is part (i) intersected over $w$. For the reverse inclusion, let $a \in \bigcap_{w \in C} E_w(z)$. For every $d$ and every tube point $p \in \mathrm{Tube}(a,d)$, every $w \in C$ gives $w \cdot s(p) \ge 0$, so by Lemma 2 $s(p) \ge 0$ componentwise; hence $\mathrm{Tube}(a,d) \subseteq S$. The same argument with $\mathrm{Succ}$ gives $\mathrm{Succ}(a,d) \subseteq G$. Thus $a \in E_{\mathrm{typ}}(z)$. $\square$
...
**Theorem 4 (price-family monotonicity).** *For a weight family $W \subseteq C$ define $\mathcal{V}_W = \bigcap_{w \in W} \mathcal{V}_w$. Then*
...
Two architectures — extraction ($q = 0$) and regeneration ($q = 1$) — share one review interval $[0,1]$, with phase state $(q, x, s_1, s_2)$: a physical reserve stock $x$ and two typed floors $s_1$ (protected-group service surplus) and $s_2$ (remediation-liability coverage), both normalized to $0$. The transition-safe set is $S_0 = \{ x \ge 0, s_1 \ge 0, s_2 \ge 0 \}$; the destination set is $G = \{ (1, x, s) : x \ge 0, s \ge 0 \}$, with destination maintainability witnessed by the destination hold policy, a declared datum. The disturbance set $\{\beta, \alpha\}$ scales the depth of a worst-case dip of size $2$; the destination reset applies the gain vector $e = (1/4, 1/4)$ componentwise to both typed floors; the rescue cost is $c = 1$ (STAGED spends one unit of $x$). The four actions available from any initial state $(0, x, s)$ with $x \ge 0$, $s \ge 0$:

| Action | Within-interval trajectory (worst case) | Successor |
|---|---|---|
| NO-SWITCH | state constant | $\{(0, x, s)\}$ — misses $G$ |
| FAST | $s_1$ dips to $s_1 - 2$ mid-interval; $s_2, x$ constant | $\{(1, x, s + e)\}$ |
| SLOW | $s_2$ dips to $s_2 - 2$ mid-interval; $s_1, x$ constant | $\{(1, x, s + e)\}$ |
| STAGED | floors grow; $x$ spends linearly to $x - 1$ | $\{(1, x - 1, s + e)\}$ |

Every trajectory is piecewise linear on the breakpoints $\{0, \tfrac12, 1\}$ and monotone on each piece, so every tube is the exact visited set — there is no outer approximation.
...
**(2)** $\mathcal{V}_{\mathrm{weak}} = \bigcap_{w \in C} \mathcal{V}_w = \{ x \ge 1 \} \cup \{ s_1 + s_2 \ge 2 \}.$

**(3)** $\mathcal{V}_{\mathrm{phys}} = \mathcal{V}_{\mathrm{end}} = X_0.$

*Define three sets, distinguished precisely:*
...
- $R = \mathrm{FP}_0 \cap \{ x \ge 1 \}$ — the **rescue set**: aggregate-feasible and typed-transformable, witnessed by STAGED.
- $I = \mathrm{FP}_0 \cap \{ x < 1 \}$ — the **impossibility region**.
- $\mathrm{FP}_{\mathrm{agg}} = \mathcal{V}_{\mathrm{weak}} \setminus \mathcal{V}_{\mathrm{typ}}$ — the genuine compensatory-versus-noncompensatory false-positive set.

**(4)** $\mathrm{FP}_{\mathrm{agg}} = I.$ *The genuine false-positive region is exactly the impossibility region; it is nonempty, and its relatively open interior is $\{ 0 < s_1, s_2 < 2,\ s_1 + s_2 > 2,\ x < 1 \}$.*
...
**Figure 1.** The discrepancy region $\mathrm{FP}_0$ is the portion of the square $0 \le s_1, s_2 < 2$ lying on or above $s_1 + s_2 = 2$: the upper-right triangle with vertices $(0,2)$, $(2,0)$, $(2,2)$, subject to the strict boundaries $s_i < 2$. Its relative interior is $0 < s_1, s_2 < 2$ with $s_1 + s_2 > 2$. **Panel A ($x < 1$, shown at $x = 0$):** the region is the genuine impossibility region $I = \mathrm{FP}_{\mathrm{agg}}$; the interior witness $(s_1, s_2) = (6/5, 6/5)$ is marked. **Panel B ($x \ge 1$, shown at $x = 1$):** the same aggregate-versus-floor region is the rescue set $R$, witnessed by STAGED. The threshold curves $\rho_1, \rho_2$ are drawn only on the open subregion where $s_2 > 0$ and $s_2 < 2$. The point $(x, s_1, s_2) = (\tfrac12, \tfrac1{10}, \tfrac1{10})$ is annotated separately in Panel A's caption, since it is not a point of the two-dimensional $s$-plane alone. The same region is not to be labelled "false positive" without fixing $x$: at $x \ge 1$ it is rescue, at $x < 1$ it is impossibility.
...
$$\mathcal{V}^{\mathrm{typ}}_j \;\subseteq\; \bigcap_{w \in C} \mathcal{V}^{w}_j \;\subseteq\; \mathcal{V}^{\mathrm{phys}}_j.$$
...
The continuum statements of Theorems 3–6 are established by the displayed proofs. A companion software artifact (deterministic, exact integer arithmetic at scale 40; no floating point, tolerances, or randomness) verifies the finite rational instance: it checks the action classifications, the region identities, and the accepted-set identities on a 29,791-state grid, with a finite verification set containing the analytically identified critical weight ratios $\rho_1, \rho_2$ and their midpoint $(\rho_1 + \rho_2)/2$ for every enumerated grid state. All 25 checks pass; re-execution reproduces the outputs exactly. The verification levels are kept distinct:
...
**Claimed.** (i) The action-set identity $E_{\mathrm{typ}} = \bigcap_{w \in C} E_w$ with the closed-cone choice isolating the separation as purely dynamic (Theorem 3(ii)). (ii) The general quantifier-separation proposition (Proposition 1). (iii) The price-family monotonicity theorem (Theorem 4). (iv) The explicit exact-tube witness with a relatively open region of strict separation and the rescue/impossibility split (Theorem 5). (v) Persistence of the witness hierarchy under hold-prefix extension (Theorem 6).
...
- **No separation on every datum.** Where a single action is safe for all weights, the assessments coincide. The theorem is an existence separation with a relatively open region, plus the always-valid hierarchy and localization.
...
The separation theorem has three implications, each stated at its actual strength.
...
**Rescue as an action-synthesis theorem.** Rather than saying the recursion "names the binding resource," define a resource-augmentation map
$$\mathsf{Aug}_r : (x, s, A) \mapsto (x + r,\; s,\; A \cup \{\mathrm{STAGED}_r\})$$
and the minimal rescue threshold
$$r^* = \inf\{\, r : \exists a \in A_r,\; a \in E_{\mathrm{typ}}(z) \,\}.$$
On the witness, for the STAGED action and $x < 1$: $r^* = 1 - x$, the exact resource increment that converts an impossibility-region state into a typed-transformable one. This turns the second policy implication into a theorem.
