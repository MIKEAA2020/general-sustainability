...
Depletion numbers circulate under one label while answering different questions. This article separates them with a typed stock–flow accounting layer: conservation follows from the incidence structure, positivity from donor limitation, services are readouts, and each depletion quantity carries its classification. Three certification layers carry a flux-reconstruction identity, a conservation-law reduction, and a flux-bounding envelope theorem, proved in full. The closed finite-donor ledger carries the theorem set: the natural-block mass identity, orthant invariance, no interior rest at positive effort, the vanishing-extraction rest set, and extraction integrability. Depletion time separates into three non-interchangeable quantities — gross turnover intensity, frozen-rate ratio, scenario-conditioned hitting time — extended to upper barriers, with uniform-drift bounds and their failure mode. Three public-data applications are classified at their exact status: the G3P anomaly index is a statistical index, not a stock ratio; the phosphate reserve-life ratio is arithmetic, not a forecast; the fisheries removals-only time is a pressure scale, not a net-depletion diagnostic. Weak and strong sustainability are located within the ledger as two regimes of one system, indexed by whether substitution keeps pace with depletion component by component; a substitute is a recycled flux or a non-renewable drawdown on a second compartment, and reserve-classification premises bound any horizontal exhaustion estimate. What the ledger does not support: no nonnegative weighting certifies componentwise nonnegativity, and five double-counting rules block phantom mass. First-passage semantics on declared stochastic surrogates carry explicit non-claims; an interface contract fixes the shared object with the delay-based dynamics, recording five non-reduction reasons.
...
The third task is to locate substitution within the ledger, rather than alongside it. Weak and strong sustainability are not competing hypotheses but two regimes of one dynamic system, indexed by whether substitution keeps pace with depletion, component by component (Daly, 1990): in the weak-sustainability regime substitution keeps pace and the flow condition is maintained, and in the strong-sustainability regime it cannot — either because no physical pathway exists, or because depletion outruns the rate at which a pathway can be deployed (Neumayer, 2013; Ekins et al., 2003). Substitution is admissible only where an identified physical pathway exists and does not draw down a different critical stock; no substitute is admitted merely because an aggregate production function or a weighted index permits it. In ledger terms, a substitute is either a recycled or recovered flux returned to the regenerating pool, or a non-renewable drawdown on a second compartment — the two are different ledger entries and carry different statuses (Section 2.3, Section 6). The reserve classification itself encodes this: reserves change with prices, technology, exploration, and regulation, so a horizontal exhaustion estimate built on a reserve figure carries the substitution and technology premises of the classification, not a physical forecast.
