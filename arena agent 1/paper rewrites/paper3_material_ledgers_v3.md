...
Depletion numbers circulate under one label while answering different questions. A typed stock–flow accounting layer is developed in which conservation is proved from the incidence structure of the ledger, positivity follows from donor limitation, services are readouts, and each depletion quantity carries its classification. Three certification layers are separated: accounting consistency, conservation consistency, and barrier safety, with a flux-reconstruction identity, a conservation-law reduction, and a flux-bounding envelope theorem proved in full. On a closed finite-donor ledger — living stock, active pool, geological donor, detritus — the theorem set is established: the natural-block mass identity, orthant invariance, absence of any interior rest at positive effort, the vanishing-extraction rest set (extinction, carrying capacity, and the frozen-biomass face), and extraction integrability on the finite donor budget. Depletion time separates into three non-interchangeable quantities — gross turnover intensity, a frozen-rate ratio, and a scenario-conditioned hitting time — extended to upper barriers, with uniform-drift bounds and their failure mode exhibited. Applications are classified at their exact status: the G3P anomaly-persistence index is a statistical index, not a physical stock ratio; the phosphate reserve-life ratio is arithmetic, not an exhaustion forecast; and the fisheries removals-only time is a gross-loss pressure scale, not a net-depletion diagnostic and not a hitting time. The ledger also proves what it does not support: no nonnegative weighting by itself certifies componentwise nonnegativity, and five double-counting rules block phantom mass. First-passage semantics on stochastic surrogates (inverse-Gaussian groundwater passage, geometric-Brownian fisheries passage) are supplied with explicit non-claims. An interface contract fixes the shared object with the delay-based dynamics and records five reasons the closed ledger does not reduce to the open working system.
...
The three recharge laws that appear across this article and its companions are distinct objects, tabulated once so that none is silently substituted for another:
...
| Working derived target | $A^{\mathrm{eq,W}} = A^{\mathrm{eq,intrinsic}} + \kappa_A K/\omega_A$ | The companion's working completion (Section 9): not a closed-block law, and the reason the two systems do not reduce. |

No derived target appears: recharge is donor-limited and cannot run backward ($e_{GA} = 0$ at $A^{\mathrm{geo}} = 0$; the positive-part convention $[\cdot]_+$, read as a one-way valve at a nonpositive target, never binds here because the registered intrinsic target is positive), and mining $C^{A,\mathrm{lim}}$ is donor-limited the same way extraction is. With $A_{g0} > 0$ the donor fraction $\sigma$ is smooth and strictly increasing in the donor level. Under the institutional-failure specialization ($\mu = \nu = \rho = 0$, $C^A = 0$) the closed natural block is
$$\begin{aligned}
\dot N &= R - qEN, \\
...
The basin rows are reported extractions from the G3P v1.12 basin series, used here only to exhibit the index construction of Section 6.5.1. The window-minimum column is implied by the displayed trend and horizon through the index formula of Section 6.5.1 — arithmetic, not product-endorsed: every basin row, and the Indo-Gangetic magnitude in particular (an order of magnitude beyond published basin-mean groundwater-equivalent trends, typically a few cm yr$^{-1}$), must be re-derived from the product's basin masks before any numerical reuse. The rows are retained only as the worked instance of the index construction, and the classification status assigned below does not depend on the magnitudes.
