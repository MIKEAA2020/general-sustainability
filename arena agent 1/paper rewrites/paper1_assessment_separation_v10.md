...
The masking this paper formalises has a concrete picture. A farm may keep one healthy hen — or one full orchard — while the pond behind it is drained and the soil is mined to keep it that way: each component produces its own flow, and an aggregate of services can appear adequate when it is read off a single component while the supporting capital is liquidated silently. This is the productivity illusion in its compensatory sense, and it is the structure the separation results of this paper examine: for every nonnegative scalarization weight, some action can hold the weighted aggregate floor along its tube while no single action holds every typed floor — the acceptance gap of Theorem 5(4), the impossibility region. The silent component of the picture, the one the aggregate does not see, is the typed floor that the common-plan criterion refuses to compromise.
