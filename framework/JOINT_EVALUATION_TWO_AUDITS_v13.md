...
## 4. Remaining points — register and adjudication (2026-09-13)

The two audits were re-checked point-by-point against v13 and the source CSVs. Defect-class points are confirmed implemented; the remaining items are registered and **adjudicated before implementation** in `framework/REMAINING_POINTS_TWO_AUDITS_v13.md`, which also fixes three discrepancies between this record and the v13 text/archive:

1. "h=5 M2m also separated" — no M2m h=5 test exists in `e3_audit_uncertainty.json` (only naive_mean vs persist h=5 excludes zero).
2. "MAE tie with values 10.72 vs 10.73" — recorded here but the values were never inserted into v13.
3. `sim_retention_power.csv` `retained` counts do not reproduce the §4.3 power cells — provenance must be pinned before any Monte-Carlo-error work.

Implementation proceeds phase-by-phase (single coordinated pass per phase, no sequential overwrites), per that register.

---

## 5. Points recovered from previously unscanned locations (2026-09-13)

Minimum audit-check files were brought into the workspace (specifications v1–v4, both wave src/ trees, rerun campaigns, source transcripts, `uploads/audit of framework v12.txt`, `uploads/framework audits.txt`, `uploads/last message.txt`, data inputs). Every location not on the v12 scan list was then checked for valid, surviving audit points. Results:

### New points (valid, surviving, now incorporated)

**N1 — Simulation provenance pinned (D3 sharpened).** `uploads/last message.txt` is the verification transcript behind §4.3 and records the exact computation: power = mean of `retained` where `module == truth`; D5 specificity = 1 − any-retained per replicate; D6/D7 = any-retained per replicate; all checks printed OK at the time. Replayed against the release-zip CSVs, the same computation **does not reproduce the published table**: `sim_retention_power.csv` gives D1 0.970/0.995, D2 0.720/0.140, D3 0.520/0.615, D4 0.385/0.070 (published 0.965/0.985/0.710/0.130/0.090/0.110/0.005/0.015); `sim_misspecified_D6D7.csv` gives 0.137/0.153/0.199/0.186 (published 0.680/0.760/0.975/0.925). Adjudication: the transcript lineage is self-consistent with the published table; the archived CSVs in the release are a different (later) serialization and must be replaced with the reproducing ones or the mismatch disclosed. No Wilson/MC-error work may use the zip CSVs as-is.

**N2 — K-bound wording (strengthens the earlier data-backed adjudication).** E1 v20 changelog item A6: "K-bound wording now states the maximum is over **predictor states of the training transitions, excluding the terminal state** — the ambiguity that produced the 81.1-vs-40.8 error earlier in this project." The framework v13's "lower bound 50.8 kt general" never imported that qualification. Data: 50.83 = 40.83 (2006, predictor-state max of late-recovery transitions) + 10; 91.1 = 81.1 (2007) + 10. The E1-v20-A6 wording is the fix template for v14.

**N3 — Frozen spec vs paper (D6/D7 cell count).** `SPECIFICATION_v4.md` Amendment 1 line: "eight cells, 1,600 rolling-ladder passes". The archive holds 4,000 rows (4 cells × 200 reps × 5 modules) and v13 correctly states "4 cells 800 passes (4000 rows)". The frozen sheet's "eight cells" line was itself miscounted; the manuscript corrects it but must record a dated note that the correction diverges from the frozen sheet (or the sheet is amended), otherwise the pre-registration record and the paper contradict each other.

**N4 — DM row count verified; E1-companion divergence noted.** From `batch 7 …/results/e1_dm_uncertainty.csv` (32 rows): 15 CI exclude zero / 17 include (zero exceptions), and exactly **5 rows** have CI-excluding-zero with |z| < 1.96 — matching v13's "5 of 32 rows disagree" and its named examples. E1 v20 uses "four of the twenty-eight" — the framework §5.1 and the E1 companion now report different counts; align or state the exclusion criterion.

**N5 — Data Availability path.** v13 cites `wave_e_cod/results/e1_dm_uncertainty.csv`; the file's archive path is `batch 7 (audits of agent arena 1 paper rewrites)/results/e1_dm_uncertainty.csv`. One-line path fix.

**N6 — Exhaustiveness caveat.** The E1 Tier-3 plan records that its own round-2 joint evaluation was later found non-exhaustive (a live Tier-1 contradiction at v21 L839/L971 never entered that evaluation). The same caveat attaches here: "all surviving points implemented" is as-of-verified, not exhaustive.

### Locations checked and cleared (no framework-relevant surviving points)

- `_scan_work/E1_E3_classified/noncovered/removed.txt` — content-survival classifications between E-paper versions, not audit points.
- `joint_assessment_wave2/3/grok_reviews` — E1-paper findings; the framework-relevant ones (pre-registration, tie band, M4 information set) were processed into the round evaluations already implemented in v12/v13 (verified present in v13 text).
- `source_audits/` transcripts (human reviewer, joint, orchard, empirical, top-down ecol, turnover, profound upgrades, gpt consolidation, caveats and residual, deepseek e3/e4, 4 audits_v33) — framework-relevant content was already captured via the processed rounds; the remainder belongs to the biocapacity/ECOMOD and E3/E4-generalization papers.
- `agent 2 productivity illusion/analysis/JOINT_*`, `TRANSFER_AUDIT_RESPONSE.md` — no framework hits.
- `wave6/paperE3` — sentence-level diff scan, not audit points.
- Capelin module numbers re-verified against `capelin_index_summary.csv`: 150.02/262.34 vs 98.05/264.72 (Spec A) and 132.02/491.74 vs 87.65/317.71 (Spec B) — exact match.
