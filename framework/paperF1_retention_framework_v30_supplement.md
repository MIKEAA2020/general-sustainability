# Supplement S1 — verification and sensitivity detail

*Aligned to carrier of record `paperF1_retention_framework_v30_humanized` (stamp `se30-c1375ed`); content rows below cite the frozen campaign layers and are unchanged for v30.*

*Companion to the main text; section numbering follows the final section order.*
...
## S1.4 Diebold–Mariano mechanics
...
## S1.4 Reproducibility package

The working-example margins, the operating-characteristic cells (power, specificity, mechanism-misattribution rows), the likelihood-ratio derivations, and the calibrated replacement band recompute from the registered campaigns below, archived at https://github.com/MIKEAA2020/general-sustainability; checksums (md5, first 8 hex) pin the release state. The pre-registered 10,000-replicate archives are the dataset of record: alternative decision rules (Section 6.5) and all sensitivity variants (S1.1) are evaluated on the same archived replicates without refitting, so the reported cells are deterministic functions of registered files. Re-executing the campaigns in a fresh environment regenerates every archived result file byte for byte.

| Script (repo path) | Role in the paper | md5 | |
|---|---|---|---|
| `framework/campaign_cod_t71_band_calibration.py` | Primary simulation-calibrated band campaign on the cod T = 71 object (registered prospective replacement band) | `b261aef5` | calibration outputs |
| `framework/campaign_cod_t71_band_calibration_checkpointed_20260917.py` | Checkpointed re-run of the same campaign (byte-identical checkpoints) | `d1336ea2` | checkpoint files |
| `framework/campaign_edwards_band_calibration.py` | Simulation-calibrated band campaign on the Edwards J-17 object | `26bb9780` | calibration outputs |
| `framework/campaign_edwards_band_calibration_audit.py` | Independent audit of the Edwards calibration (recomputes rows) | `b261a0ca` | audit output |
| `framework/certificate_schema_S3.py` | S3 certificate schema validator for the audit layer | `19a22c18` | schema/validation records |
| `phase_c/campaign_power.py` | Core power campaign (D1–D5 replicates) | `352d4e84` | replicate archives |
| `phase_c/campaign_misspecified.py` | Amendment-1 campaign (D6–D7 out-of-class truth) | `ea603471` | replicate archives |
| `phase_c/campaign_origins.py` | Origin-set controls (mixed versus origin-matched) | `0d3b2cf0` | origin tables |
| `phase_c/campaign_snr.py` | Signal-to-noise grid campaign | `2c4a496e` | SNR tables |
| `phase_c/campaign_smoother.py` | Smoothed-series control campaign | `ac2331ea` | smoother tables |
| `phase_c/o6_cod_band_calibration_20260913.py` | Earlier cod band-calibration layer (registered 2026-09-13) | `a5e21be2` | o6 outputs |
| `phase_c/o9_ic_coprimary_20260913.py` | IC co-primary comparison layer | `e327f256` | o9 outputs |
| `phase_c/analysis_d67_gain.py` | D6/D7 retention-gain analysis (mean gains +3.1/+9.2 kt) | `fa367856` | gain tables |
| `phase_c/analysis_gate_mcs.py` | Gate-attribution and model-confidence-set analyses | `6cbdcf7c` | gate/MCS tables |
| `phase_c/analysis_regime_ident.py` | Regime-identification analysis | `a5186de2` | identification tables |
| `batch 7 (audits of agent arena 1 paper rewrites)/campaign_e1_dm_uncertainty.py` | Cod DM/bootstrap post-freeze layer (Spec A/B) | `81c559b8` | results/e1_dm_uncertainty.csv |
| `batch 7 (audits of agent arena 1 paper rewrites)/campaign_e1_dm_uncertainty_regime.py` | Cod regime-stratified DM layer | `6ced3555` | results/e1_dm_uncertainty_regime.csv |
| `batch 7 (audits of agent arena 1 paper rewrites)/campaign_e3_dm_uncertainty.py` | Edwards DM/bootstrap independent replication | `30963493` | results/e3_dm_uncertainty.csv |

*Design-time exclusions.* Candidates rejected before scoring — including the naive regime-switch generator (900 → 888 → … → 0 in four steps, rejected at design time) — are excluded from the registered package by construction and are recorded in Section 6.2 of the carrier.
