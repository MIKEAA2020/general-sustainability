...
**Table 1.** Three objects — two domains, no pooling. Safe set and LRP type the reference frame; primary score never uses them (Brier secondary).
...
Fail any → not retained. Retention per specification. Tie band 5% → improvements inside band ties, not retain, exactly 5% fails. Deciding score is frozen specification's rolling-origin RMSE pair at h=1 and h=5 (H3).
...
| Modules beating baseline (points) | none | three module–horizon cells (M1 h=1 −2.96%, M2m h=1 −7.16% only separated, M2m h=5 −17.34%) + oracle −42.96%/−48.65% under fitted map |
...
| Decided by | ranking alone (17.09% smallest deficit, no Spec A interval against persistence excluding zero) | tie band and comparator gate (4.33% H1 margin, AR(1) interval covering zero) |
| Oracle bound | no | yes 7.547 versus 13.230 (−42.96% h=1, −48.65% h=5) — nowcast bound under fitted map |
...
- `wave_e_edwards/results/e3_audit_uncertainty.json` (uncertainty layer, DM HAC + block bootstrap per Künsch 1989, p percentile-tail fraction p_perc)
- `batch 7 (audits of agent arena 1 paper rewrites)/results/e1_dm_uncertainty.csv` (independent replication, 32 rows, 15 CI exclude zero 1 Spec A +6 Spec B h1 +8 Spec B h5 and 17 include 7+8 Spec A +2 Spec B h1, DM descriptive loss-differential diagnostics not calibrated)
