...
**Framework artefact v3 — joint synthesis of v1 (user draft, information-set audit + operating characteristics) and v2 (verified margins + gate decomposition). v1 preserved as `paperF1_retention_framework_v1.md`, v2 as `paperF1_retention_framework_v2.md`. No overwrite.**

---

## Highlights

- Three scored objects (COD Spec A, COD Spec B, Edwards J-17) run under one retention rule: H1 comparator + H2 persistence + H3 both horizons + 5% tie band
- Empty retained set on all three objects at both h=1 and h=5 — same verdict by different routes
...
- Three Edwards module–horizon margins beat persistence outright (−3.0%, −7.2%, −17.3%) and retention withheld by tie band and comparator gate (4.3% M2m-vs-M1 margin)
- Simulation: rule specific in-class (0.985/0.970 under persistence-true null), powerful where signal strong (0.965/0.985 for D1 collapse-window autonomous), power <15% for stock-flow and depensation, over-retains out-of-class (0.68–0.98) — specificity conditional on in-class data
...
**Keywords:** model selection; out-of-sample forecasting; retention rule; operating characteristics; identifiability; conditional hindcast; stock assessment; groundwater; negative certificate
...
**Cod ladder (Spec A and B, same modules):** M1_autonomous_Schaefer, M1b_autonomous_Allee (s in [0, max_train S], r at bound 2.0), M2_stockflow_regimeC, M3_AR_residual (phi in [-0.95,0.95]), M4_delayed_info (stale-start experiment), baselines naive_persist, naive_train_mean.

**Edwards ladder:** M1 one-pool affine, M2 two-parameter, M2m modified training-mean balance (closest to persistence), M3, M4, M2_oracle (oracle with realised drivers, declared unable to be retained), baselines naive_persist, naive_mean.
...
Full detail for each application is in its companion paper. Reported here are the scored verdicts, the margins, and the route by which each verdict was reached. Every number below verified against source CSVs (see Appendix).
...
| 1 | 13.23 ft | M2m training-mean balance | 12.28 | -7.2% |
| 5 | 21.11 ft | M2m training-mean balance | 17.44 | -17.3% |
...
M1 fails H2 at both horizons (band 12.57 ft). M2m passes H2 at both horizons but fails H1 at h=1: 12.28 vs M1 12.84 = 4.33% <5%. Since H1 fails at h=1, M2m not retained. The companion additionally declines M2m on class grounds, since it collapses to an autoregression under constant fluxes.
...
| Modules beating baseline (points) | none | three module–horizon cells (M1 h1 -3.0%, M2m h1 -7.2%, M2m h5 -17.3%) |
...
## Appendix — Verification (machine-checked at time of writing, from `results/`)
...
All 15 primary scores, 8 derived percentages, and 12 simulation figures verified against committed CSVs.
