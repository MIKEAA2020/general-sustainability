...
\paragraph{Validated discrete-level certificates (independent rerun 2026-08-26).}
The computational supplement now carries three discrete-level artifacts for this orbit, independently rerun from committed code on a second agent and toolchain (\texttt{batch 4/VALIDATED\_COMPUTATIONS\_RERUN.md}):
\begin{itemize}
\item $K=80$ Fourier-collocation Krawczyk: a unique zero of the $645$-dimensional collocation map in a componentwise $10^{-8}$ box about period $P=370.9311778394$~yr (committed certificate hash \texttt{5e8df633\ldots65ab133}). The independent rerun certified the same claim at a Newton centre with $|\Delta P|=4.5\times 10^{-12}$, inside the committed box.
\item Off-grid interpolant residual on a $256$-point grid, genuine mpmath interval arithmetic: committed bounds $N\le 6.57\times 10^{-8}$, $A\le 1.04\times 10^{-9}$, $Z\le 8.29\times 10^{-7}$, $E\le 2.85\times 10^{-6}$. The independent rerun reproduced $N,Z,E$ to four digits and gave $A\le 1.11\times 10^{-9}$; both support the register's citation form ``continuum residual $\le 3\times 10^{-6}$''.
\item Method-of-steps monodromy enclosure at $\Delta t=0.25$ (history dimension $76$, $1484$ period steps): phase multiplier $1.00480$ certified simple and neutral, dominant nontrivial $0.68764$ with disc $0.069<1$, all nontrivial multipliers strictly inside the unit disc. This enclosure is \emph{not} the shooting table above (phase $0.98688$): the two computations use different orbits and different error models. The enclosure hash \texttt{01d8c253\ldots dbaef76} is byte-identical under independent rerun.
\end{itemize}
None of these three is a continuum orbit existence proof, a continuum Floquet enclosure, or a bunching proof. They discharge the independent-rerun gate for the discrete certificates; they do not close Conjecture~\ref{conj:graph} or Proposition~\ref{prop:conditional}.
