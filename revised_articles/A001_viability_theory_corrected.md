...
**Remark 8.3 (Numerical instance of Corollary 8.1).** With $\sigma = 2$, $\alpha = 0.5$, $Y_0 = 10$, and $A_0 = 4$ (any $R_0 > 0$), the essentiality formula of Corollary 8.1(1) gives
$$F(A, 0) = Y_0\,\alpha^{\sigma/(\sigma-1)}\,\frac{A}{A_0} = 10 \cdot (0.5)^2 \cdot \frac{A}{4} = 0.625\,A,$$
so $F(5, 0) = 3.125$ and $F(10, 0) = 6.25$: at $\sigma > 1$ capital alone yields positive output, linear in $A$ at zero resource flow, while at $\sigma \le 1$ the same instance gives $F(A, 0) = 0$ for every $A$ — the two sides of the essentiality threshold on one datum.
