...
\textbf{Problem.} Index-well head forecasting is a recurring operational
groundwater management need, yet deliberately simple process-based
water-balance models are rarely tested against naive benchmarks under a
locked retention rule.

\textbf{Approach.} Using the 1934--2023 annual-mean J-17 Edwards Aquifer
head record, a fixed model ladder was scored by rolling and fixed-window
out-of-sample RMSE: last-value persistence, training-mean climatology,
univariate AR(1), a one-pool stock-flow water balance, residual/delay
variants, and climate-informed recharge modules. A causal module was
retained only if it beat both persistence and the next-simpler causal
model under a scoring protocol frozen and dated before any score was
computed.

\textbf{Results.} The causal stock-flow map that persists last year's
recharge loses to persistence at the one-year horizon (14.70 versus
13.23 ft), because annual recharge is near-white: corr(R\_t, R\_\{t−1\})
= 0.17 against corr(ΔH\_t, R\_t) = 0.74. The univariate AR(1) improves
one-year RMSE by 0.39 ft (12.84 ft; mean absolute error a tie; loses at
five years) --- a margin whose bootstrap interval covers zero: a
provisional retention recorded by the point-RMSE rule, not a skill
claim. The water-balance-identified affine map with climatological
fluxes (M2m) is the best one-step forecaster tested (12.28 ft, the only
margin that separates from noise against persistence); it is listed and
then declined by a protocol class clause outside the frozen retention
rule. Given realized future recharge and pumpage, the same map reaches
7.55 ft --- a nowcast, not a forecast; no signal at the annual forecast
origin recovers that gap. Climate modules beat persistence and the AR(1)
by at most 0.13 ft and lose to climatological fluxes; none is retained.
At five years, the training mean (16.80 ft) beats persistence (21.11 ft)
with an interval excluding zero.

\textbf{Implications.} For annual J-17 head forecasts, simple
persistence and a univariate AR(1) are difficult to beat; the one-pool
balance, given the year's fluxes, nowcasts rather than forecasts.
Multi-year planning should prefer climatological baselines to persisted
recharge.
...
\textbf{Article Impact Statement.} Annual J-17 head forecasts:
persistence and AR(1) beat the one-pool water balance because recharge
is not persistent at the annual origin; with realized fluxes the map
nowcasts (7.6 ft), it does not forecast; five-year planning should use
climatology.
...
linear drain, with the spring-discharge series stored but excluded by
design from the forecasting equation, and a {[}610, 710{]} clip standing
in for a physical storage floor. The map is not a closed mass balance.
...
the Edwards Aquifer Authority) and its physical threshold (the ≈618-ft
...
map is a live hydrogeologic question.
...
enter because El Niño/Southern Oscillation (ENSO) phases shift North
American precipitation patterns (Ropelewski and Halpert 1986):
September--November Niño 3.4 and lagged climate-division precipitation
...
(≈618 ft; 1956 daily minimum 612.51 ft) & N / E \\
...
Recharge R is USGS total San Antonio-area recharge (10³ acre-ft yr⁻¹),
...
are September--November Niño 3.4 (HadISST, anomaly relative to
...
Niño 3.4, lagged climate-division precipitation, or all three. A
...
prospective clinical-style registration; the term ``pre-registered'' is
therefore not used in this article. The frozen retention rule is
Definition 4.2, verbatim above. Three protocol elements fall outside
that rule and are recorded here as deviations:
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Figure 1}]{figs/fig1_series.png}}
\caption{Figure 1}
\end{figure}

\textbf{Figure 1.} J-17 annual mean and daily-high range. 1956 mean
623.15 ft, daily minimum 612.51 ft. 1992 mean 691.96 ft, daily maximum
703.31 ft. 2023 mean 635.68 ft. The annual mean is below 660 ft in 31 of
90 years; the daily minimum is below 618 ft in one year (1956).

\subsubsection{5.2 Fixed windows}\label{fixed-windows}

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Figure 2}]{figs/fig2_windows.png}}
\caption{Figure 2}
\end{figure}

\textbf{Figure 2.} Multi-step forecasts from the end of each training
window. The oracle (dashed) is diagnostic and uses realized future R, P.
...
the wrong sign on pumpage (γ = +0.021) --- pumping rose as the drought
...
persisted (R, P) = (43.7, 321.1) runs 617.1 → 611.6 → 606.8 → 605.3 →
...
on these persisted-drought forecast paths, and it is operative precisely
there.

The 1957--61 recovery is a recharge pulse (R₁₉₅₆ = 43.7, R₁₉₅₇ = 1142.6
× 10³ acre-ft). Persistence stays at the 1956 floor (RMSE 44 ft). Causal
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Figure 3}]{figs/fig3_rmse.png}}
\caption{Figure 3}
\end{figure}

\textbf{Figure 3.} Rolling-origin RMSE. The oracle is excluded from
retention.
...
\bottomrule\noalign{}
\endlastfoot
M1 & 12.84 \textless{} 13.23 (−0.39) & --- & output only & retained
(point rule; margin within noise) \\
M2 & 14.70 \textgreater{} 13.23 (+1.47) & +1.86 & causal fluxes &
reject \\
M2m & 12.28 \textless{} 13.23 (−0.95) & −0.56 & no (affine AR(1)
...
RMSE 702 versus climatological 556 × 10³ acre-ft on the recharge
target).
...
provisional, recorded by the point rule on a margin within noise --- and
it records a slightly mean-reverting head series, not a confirmation of
stock-flow structure. At the decision scale of annual drought-stage
declarations the difference is operationally nil. The M2m decline and
its 0.56-ft advantage over M1 are likewise within noise (Section 5.3.1):
the estimator-level advantage of water-balance identification is visible
in the point scores but not separated from zero at this sample size.
...
and Mariano 1995; Newey--West HAC, lag h − 1) and moving-block bootstrap
intervals (Künsch 1989; block length 8, 10,000 replications, seeded) to
...
layer; Diebold--Mariano with Newey--West HAC, lag h − 1; moving-block
...
M2m − persist & −0.95 & −3.07 & 0.003 & {[}−1.45, −0.68{]} &
\textbf{yes} \\
M2m − M1 & −0.56 & −1.61 & 0.11 & {[}−1.53, +0.13{]} & no \\
M2 − persist & +1.47 & +1.27 & 0.21 & {[}−0.02, +3.40{]} & no
(borderline) \\
mean − persist (h = 5) & −4.30 & −1.65 & 0.10 & {[}−9.72, −2.09{]} &
\textbf{yes} \\
M1 − persist (h = 5) & +0.15 & +0.06 & 0.95 & {[}−5.57, +4.94{]} & no \\
M2\_combo − M1 (climate) & −0.13 & −0.21 & 0.83 & {[}−1.37, +1.31{]} &
no \\
M2\_combo − M2m (nested baseline) & +0.43 & +0.74 & 0.46 & {[}−0.56,
+1.57{]} & no \\
\end{longtable}
}

Three readings. First, the retention margin itself (M1 − persist, 0.39
...
over persistence (−0.95 ft, p = 0.003). Second, the five-year
climatology win (−4.30 ft) is robust: its bootstrap interval excludes
...
primary evidence. Third, the nested-baseline margin (combo − M2m, +0.43
ft) is itself within noise: the climate rejection is a point-RMSE rule
outcome, not a significance finding.

An independent implementation of the Diebold--Mariano and bootstrap
layer under different seeds, block lengths, HAC weighting, and variance
conventions reproduces every load-bearing conclusion of Table 6
...
September--November Niño 3.4 anomaly of year t, and calendar-year
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Figure 4}]{figs/fig4_pass2.png}}
\caption{Figure 4}
\end{figure}

\textbf{Figure 4.} Rolling RMSE on J-17 for climate-informed recharge.
The precipitation oracle uses year t+h precipitation and cannot be
retained.
...
Lagged precipitation and September--November Niño 3.4 have modest skill
on R relative to climatology (528--545 versus 556 × 10³ acre-ft), and
...
Section 5.3.1, also within noise). Both readings accompany the
rejection: the climate modules beat persistence and the AR(1) by at most
0.13 ft and lose to climatological fluxes; none is retained, and nothing
in that verdict is a significance finding. M2\_Rar loses at h = 1 (13.25
ft; 0.41 ft worse than M1) --- autoregression on nearly white recharge
...
precipitation oracle 33.7 ft). September--November 1956 is La Niña
(−0.92) and does not announce R₁₉₅₇ = 1143.

The remaining fixed windows complete the same record; on the 2015--23
critical-period window the climate-informed modules M2\_Rprecip and
M2\_Rar edge past M1 by about one foot on a window-specific basis that
does not constitute a recharge forecast (Supplement, Section S2).
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Figure 5}]{figs/fig5_fibre.png}}
\caption{Figure 5}
\end{figure}

\textbf{Figure 5.} Comal annual mean versus J-17. Contemporaneous r =
0.986. The service series is a measured rating curve of the same state
--- a measured service, not an independent information source.

\textbf{The Comal channel (rating-curve reading).} The map Q = c₀ + c₁H
fitted on 1934--1950 (c₀ = −2876, c₁ = 4.77) is a linear channel of the
...
1956 annual mean (623.15 ft) it reads ≈97 cfs against an observed ≈32
cfs annual mean --- it predicts non-cessation --- and its zero-discharge
level (2876/4.77 ≈ 602.9 ft) lies below every observed head (daily
...
20\% cut raises the simulated 2023 head by 5.2 ft (646.8 − 641.6)
relative to the actual-pumpage path. Fourth, the recharge coefficient
dominates the map through its range, not its magnitude: a 1000 × 10³
acre-ft recharge difference moves the head by roughly 17.5 ft at the
pre-permit coefficient (β̂ ≈ 0.0175 on the 1980--1990 train; the pumpage
coefficient on that train is −0.031 ft per 10³ acre-ft, larger per unit
but acting on a range of \textasciitilde300--540 against recharge's
44--2,486), and γ is an ordinary-least-squares association the paper
itself diagnoses as simultaneity-contaminated (Section 5.2), not a
policy lever. The full-sample coefficients (β̂, γ̂) = (0.017, −0.026)
differ from the pre-permit window's because the windows differ.

\subsection{6. Discussion}\label{discussion}

The one-pool increment ΔH tracks recharge (r = 0.74), and the oracle's
...
benchmark of Zhu et al.~(2026) ranks nine architectures by RMSE and R²
...
Two specification points bear on interpretation. First, next-year
pumpage is partially an institutional scenario --- permits and
...
the year's head (7.55 ft RMSE), and the contemporaneous increment ΔH\_t
against R\_t (r = 0.74) closes the argument. The retained AR(1) admits a
...
H\_\{t−1\}) = 0.64 is a different quantity) is consistent with a
drainage-decay coefficient \(\hat k \approx 0.34\) yr⁻¹. The module is
...
The leading-indicator question separates institutional from physical
references. The 1950s decline is not a Stage I event: the 660-ft line is
a 2007 rule, and slack to 660 ft is the wrong leading indicator for
1951--56, just as the 2016 Northern cod limit reference point was the
wrong leading indicator for 1983--90 in the companion study. 1956 is a
physical near-cessation at Comal (annual mean springflow 32 cfs; daily
J-17 minimum 612.51 ft).
...
model by the frozen point rule; the margin lies within noise (MAE tie,
...
Nölscher, M., and Broda, S. 2026. GEMS-GER: A machine learning benchmark
