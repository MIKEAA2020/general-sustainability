...
*Second edition (batch-5 corrections, 2026-08-29): the abstract's climate-margin clause excludes the AR(1)-on-recharge variant (which loses at the one-year horizon by 0.41 ft); the model identifiers of §5.4 are standardised ($M2\_Renso$, $M2\_Rprecip$ throughout); the figure embeds of §5.4–§5.5 point at files whose indices match the displayed figure numbers (`fig4_pass2.png`, `fig5_fibre.png` — byte-identical copies of the committed `fig5_pass2.png` and `fig4_fibre.png`, whose filenames carried the pre-renumbering indices). The adjudication record is `BATCH5_JOINT_AUDIT_EVALUATION.md`; no score, table value, or retention decision changes.*
