...
*Second edition (batch-5 corrections, 2026-08-29): an Abstract and a References
section are supplied; BAU's nominal physical-threshold kernel empties beyond
$T\approx13$ years (the continuous crossover is 12.7; the first edition's
$\approx14$ was inconsistent with the paper's own continuous-crossover
convention); the certified-horizon statement records zero pumping's $T=4$
UC-min kernel (analytic; not sampled by the committed horizon grid); the CPM
trigger terminology, the oracle-gap baselines, the floor wording, and the BAU
mean's displayed value are corrected; the retention comparator's code-level
correction is recorded in the availability section. The adjudication record is
`BATCH5_JOINT_AUDIT_EVALUATION.md`; every attractor, boundary, supply, replay,
and retention number is unchanged except as listed.*
...
causal ladder on J-17, and the oracle water-balance gap (7.55 ft against
persistence's 13.23 ft; 7.55 against the retained AR(1)'s 12.84 ft)
...
These floors are certification geometry — the persistent floor regimes are
harsher than any single recorded year (UC-min is the recorded 1956 minimum,
held perpetually) — not recharge forecasts.
...
Governance family: BAU (\(P \equiv \bar P = 282.16\), training mean); flat
...
beyond \(T \approx 13\) years — the \(T=12\) boundary is 692.6 ft and the
...
above the first-stage CPM trigger (660 ft — the highest of the four declared
trigger levels, and therefore the easiest to reach), so no declared
demand-management rule activates in the viable region and the kernel is
policy-invariant.
...
With the erosion of §3 applied, every demand-management policy in the family
has an empty certified kernel beyond \(T = 3\) years at the physical threshold
and beyond \(T = 1\) year at the institutional threshold. Zero pumping is the
exception: its certified physical-threshold kernel is nonempty through
\(T = 4\) under UC-min (lower boundary \(\approx687.9\) ft from the affine
certified-kernel algebra; the committed horizon grid
\(\{1,2,3,5,8,10,15,20,\infty\}\) does not sample \(T=4\), so the
archived \(T=\infty\) emptiness is consistent with the analytic boundary
...
information layer: the erosion bound absorbs the oracle gap measured against
the retained AR(1) baseline (12.84 versus 7.55 ft).
...
`wave_e_edwards/protocol_intervention.md`. A second edition of the runner
(`src/run_intervention_v2.py`) corrects the retention comparator's treatment
of empty kernels (an empty kernel now counts as worst on both sides,
consistent with the cod-side runner; the first edition's asymmetric
comparator was mechanically verified inert for these artifacts) and writes
`results/intervention_results_v2.json` and
`results/intervention_boundaries_v2.csv`, value-identical to the committed
outputs; the committed first-edition files are retained unchanged as the
audited record.
