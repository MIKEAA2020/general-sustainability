...
{\small Independent Researcher}\\[0.35em]
{\small \href{https://orcid.org/0000-0002-0019-1842}{ORCID: 0000-0002-0019-1842}}\\[0.25em]
{\small \href{mailto:amin_abaee@ut.ac.ir}{amin\_abaee@ut.ac.ir}}}
\date{19 September 2026}
\maketitle

\begin{abstract}
This Supplementary Information formalizes the certification layers of the typed material-flow ledger, specifies the interface between the closed ledger and institutional delay dynamics, reports the application classifications, and defines operational weak- and strong-sustainability regimes. The formalization is conditional on declared types, routes, barriers, demand and horizon; it does not constitute a universal theorem about sustainability ethics or an empirical calibration of any named domain.
\end{abstract}

\section*{Scope}

The Supplementary Information contains four components:
...
\item full tables for the groundwater, phosphate and fisheries records, including status and vintage information; and
\item an operational weak/strong-sustainability formalization that states the admissible closure assumptions.
\end{enumerate}

These components use the definitions and evidence classifications of the article. Procedural details are given in the methods companion (Abaee, 2026a), and the accounting-standards interpretation is given in the accompanying commentary (Abaee, 2026b). The formalization is conditional on the declarations stated here; it does not promote an interpretation to an unrestricted theorem.
...
Table~\ref{tab:inventory} gives the extended inventory. The labels are local to this Supplementary Information and identify the results stated below. Proof sketches are supplied where the argument is short.
...
The closed material ledger comprises the primitive stock--flow equations, typed routing, conservation, positivity, componentwise margins and finite-donor budget. The institutional-delay analysis concerns an open working completion with memory and review delay (Abaee, 2026c). The exact object shared under the single-resource specialization is the decline-pressure identity
\begin{equation}
...
\end{align}
where $z_t(\xi)=z(t+\xi)$ for $\xi\in[-\tau,0]$. This notation identifies the memory input, review delay and effort readout without replacing the equations of the institutional-delay analysis. The interface supplies the typed decline input and its boundary conditions.
...
\end{equation}
which treats a continuing donor support as an imposed completion. The parameterization uses $A^{\rm eq,intrinsic}=50$, $A^{\rm act,*}=397.87$ and $A^{\rm eq,W}=5050$.
...
The permitted relation is a shared diagnostic identity and reconstruction of omitted flows. Hopf crossings, periodic orbits and stability thresholds of the frozen-donor working system do not transfer to the closed ledger. The open working system is therefore treated as a distinct completion with explicit omitted turnover and imposed recharge.
...
The application tables are source-vintage records and bounded arithmetic. The delay section specifies an interface and a non-reduction boundary; results for the open institutional completion are not assigned to the closed ledger.

The principal article record is available at \url{https://doi.org/10.6084/m9.figshare.33942451}.

\section*{References}

Abaee, A., 2026. \emph{Typed Flux Ledgers and Depletion Arithmetic: Conservation, Componentwise Diagnostics, and the Semantics of Depletion Horizons}. Figshare preprint. \url{https://doi.org/10.6084/m9.figshare.33942451}.
...
Abaee, A., 2026c. \emph{Institutional delay dynamics companion analysis}. Zenodo. \url{https://doi.org/10.5281/zenodo.22554217}.
