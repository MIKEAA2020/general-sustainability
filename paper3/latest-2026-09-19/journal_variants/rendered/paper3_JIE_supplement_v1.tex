...
\title{Supplementary material for the Journal of Industrial Ecology version\\
\large Extended theorem inventory, delay-dynamics interface, application tables, and operational sustainability regimes}
\author{Amin Abaee\\[0.25em]\small Independent Researcher\\\small ORCID: 0000-0002-0019-1842}
\date{19 September 2026}
\maketitle

\begin{abstract}
This supplement extends the compressed Journal of Industrial Ecology article without changing its definitions, evidence statuses, or non-claims. It records the theorem inventory that the article delegates, gives the interface contract between the closed typed ledger and the open delay-dynamics completion, reproduces the full application tables behind the compact crosswalk, and formalizes weak and strong sustainability as scoped operational regimes. The formalization is conditional on declared types, routes, barriers, demand and horizon; it is not a universal theorem about sustainability ethics or an empirical calibration of any named domain.
\end{abstract}

\section*{Scope and reading rule}

The main article is the submission-length account. This file is a technical supplement, not a second empirical article and not a supported software package. It uses the full-length v49 source line as its mathematical provenance. Companion A remains the procedural account of the input schema, predicates, programmes, vintage protocol and reproduction bundle. Companion B remains the standards commentary. The present file adds four things that are useful to a material-flow referee:

\begin{enumerate}
\item the extended inventory of accounting, positivity, barrier, depletion and aggregation results;
\item the exact shared object and the non-reduction boundary for institutional delay dynamics;
\item full tables for the groundwater, phosphate and fisheries records, including the status and vintage caveats; and
\item an operational weak/strong-sustainability formalization that states the admissible closure assumptions rather than promoting an interpretation to an unrestricted theorem.
\end{enumerate}

Every application number remains a bounded descriptive record. The groundwater index is record-relative; the phosphate quotient is an economic reserve-class ratio; and the fisheries quantity is a removals-only pressure time. None is presented here as a calibrated forecast.
...
Table~\ref{tab:inventory} gives the extended inventory. The labels are local to this supplement and point to the corresponding results in the full-length line. A proof sketch is supplied where the argument is short; the full line-by-line derivations remain in the full-length source and the deposited supplementary records.
...
\toprule
Result & Statement supplied to the JIE reader & Boundary of the result \\
\midrule
\endfirsthead
\toprule
Result & Statement supplied to the JIE reader & Boundary of the result \\
...
S1.7 Closed finite-donor mass & For the closed natural block, $M=N+A^{\rm act}+A^{\rm geo}+U$ obeys $\dot M=-qEN-C^{A,\rm lim}$. & The identity is for the declared routing; internal transfers do not exhaust a conserved moiety. \\
...
S1.14 Delay-interface non-reduction & The closed ledger and open working delay system share a decline identity under a specialization but do not share a dynamic reduction. & Shared diagnostics do not transfer Hopf, equilibrium or periodic-orbit results. \\
\end{longtable}
...
Summing gives
\begin{equation}
\dot M=-qEN-C^{A,\rm lim},\qquad M=N+A^{\rm act}+A^{\rm geo}+U.\label{eq:massblock}
\end{equation}
...
The closed material ledger owns the primitive stock--flow equations, typed routing, conservation, positivity, componentwise margins and finite-donor budget. The institutional delay companion owns an open working completion with memory and review delay. The exact object shared under the single-resource specialization is the decline-pressure identity
\begin{equation}
...
\qquad z=(N,A^{\rm act},A^{\rm geo},U,Z,E),\\
\dot Z(t)&=\mathcal F\bigl(Z_t,\Lambda(t);\vartheta\bigr),
\qquad E(t)=\mathcal E\bigl(Z_t;\vartheta\bigr),
\end{align}
where $z_t(\xi)=z(t+\xi)$ for $\xi\in[-\tau,0]$. This notation identifies the memory input, review delay and effort readout without silently replacing the companion's calibrated equations by a new model. The delay equations belong to the companion; the ledger contributes the typed decline input and its boundary conditions.
...
e_{GA}=\omega_A A^{\rm eq,intrinsic}\sigma.
\end{equation}
The open working completion uses the derived target
\begin{equation}
A^{\rm eq,W}=A^{\rm eq,intrinsic}+\frac{\kappa_AK}{\omega_A},
\end{equation}
which treats a continuing donor support as an imposed completion. The registered values are $A^{\rm eq,intrinsic}=50$, $A^{\rm act,*}=397.87$ and $A^{\rm eq,W}=5050$; these are imported registered values, not a new calibration in this supplement.
...
=\kappa_AK-\gamma_UU
\end{equation}
under the registered scale separation $\sigma\approx1$. At the working point, the difference is approximately $0.535$ stock units per year; at $U=0$ the scale is $\kappa_AK=5.000$. The difference is structural: the working completion omits the detritus return carried by the closed field and uses a derived target.

\subsection*{S2.3 Non-reduction result}

\textbf{Proposition S2.1 (One-way projection).} Under the institutional-failure specialization, if the right-hand sides for $(N,A^{\rm act},A^{\rm geo},U,Z,E)$ contain no excluded macroeconomic state, that block is an exact projection for every parameter value. The converse projection is not available when excluded variables are driven by the block.
...
\textbf{Theorem S2.2 (Non-reduction of the open working completion).} There is no exact dynamic reduction, regular perturbation or finite-time tracking correspondence from the closed primitive finite-donor ledger to the open working completion under the registered definitions.
...
The permitted relation is analogy for shared mechanism language plus reconstruction of omitted flows. Hopf crossings, periodic orbits and stability thresholds of the frozen-donor working system do not transfer to the closed ledger. Conversely, the open working system can be used as a diagnostic completion whose omitted turnover and imposed recharge are named, not as a theorem-preserving reduction.
...
\end{equation}
The limit $G_0\to\infty$ freezes the donor coordinate but does not restore the working completion's derived target; it is therefore not a regular perturbation of the working vector field. If a sustained extraction flux satisfies $c(t)\geq c_0>0$ over an interval, the closed donor budget gives the order bound $T\leq G_0/c_0$. This is a budget bound, not an asymptotic estimate of delay-induced cycles. Whether a frozen-donor Hopf structure persists as a slowly drifting transient is an open slow-passage question.
...
\bottomrule
\endfoot
Archived selected cohort & 43 & $\approx1.8$ & Headline cohort; eight zero entries at or below reference; not reproduced by either public release \\
Archived positive sub-cohort & 35 & $\approx2.9$ & $F>0$ and $\mathrm{SSB}_{\rm now}>B_{\rm lim}$; still removals-only \\
RAM Legacy v4.44 public release & 415 & $\approx2.57$ & Same protocol; public-release comparison, not the archived cohort \\
RAM Legacy v4.66 public release & 454 & $\approx3.39$ & Same protocol; version-sensitive broad cohort \\
\end{longtable}

The selected 43-stock record is a class-specific diagnostic rather than a statistic of all assessed fisheries. The broad release comparison is a sensitivity record, not an uncertainty interval around a demographic forecast.
...
A universal weak/strong-sustainability theorem would require a complete class of closure maps, admissible substitutions, boundary flows, time horizons, critical-stock definitions and institutional valuation rules. Those objects are not supplied by a generic MFA table. Accordingly, this supplement formalizes only the operational regime readout in Definitions S4.1--S4.2. The safe claim is narrower: the ledger makes the physical substitution and closure assumptions explicit, and it refuses cross-type compensation as a certification shortcut.
...
\section*{S5. Reproducibility and non-claims}

The supplement does not add a calibrated case study or a supported software implementation. Companion A supplies the exact procedure and negative-output semantics. The current Route C prototype remains a dependency-free reference core with four tests, not an EMS-ready package. The application tables above are source-vintage records and bounded arithmetic. The delay section records an interface and a non-reduction boundary; it does not import the companion's Hopf or periodic-orbit results into the closed ledger.

The public full-length preprint record supplied by the author is available at \url{https://doi.org/10.6084/m9.figshare.33942451}. The expanded 49-record deposit package is maintained separately and must be version-matched before the DOI record is described as the same file set.
...
Abaee, A., 2026. \emph{Institutional delay dynamics companion analysis}. Zenodo record, \url{https://doi.org/10.5281/zenodo.22554217}.
