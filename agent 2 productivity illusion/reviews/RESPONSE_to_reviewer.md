...
We believe the revision addresses the clarity and rigour concerns without over-claiming empirical support. The empirical calibration the reviewer requests is a substantial programme in its own right, and we have specified how it would be carried out (§11) rather than presenting its results prematurely. If the reviewer would find it useful, we would be glad to add a clearly labelled calibration of τ_g (and possibly b) from a small set of case studies, or to expand the robustness analysis in §S5.
