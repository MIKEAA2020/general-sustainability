...
> constant-parameter subsystem, the maximum sustainable yield is an interior stock `A*`, and the
> non-trivial equilibrium is locally stable for any single delay; the delay-perturbed eigenvalues
> remain near-neutral across the two-delay plane, and the onset of linear instability is strongly
> dependent on the *ratio* of the two lags and controlled by a single dimensionless group `χ`. We
...
|---|---|
| "instability requires both delays, threshold ≈ 80 yr" | "the two-delay stability boundary is controlled by the single group χ (χ=1 ⇔ ρ=3q); the onset is ratio-dependent, marginal (Re λ ≲ 0.01 yr⁻¹), and closed-form: s=π/ω at χ=1. The number 78.5 yr = π/(2r) arises because both loops contribute equal delayed gain r at χ=1 — **this is a two-loop coincidence of the model, NOT the Hutchinson π/(2r) demographic-threshold** (Hutchinson's is the single-delay logistic; the value here comes from the round-trip gain, not the demographic lag)." |
...
1. **Which lag destabilises** is set by the sign of `Λ`/`χ` (not set by the equations).
2. The **oscillation period near onset ≈ 4× the dominant lag**.
...
- **A genuinely-remaining analytic step (master 12B; Claude audit §3.2).** The two-delay threshold is
  still a single-branch estimate; it should be **re-owned with stability-crossing curves** (Gu,
  Niculescu & Chen 2005) and verified by **full-spectrum computation** (DDE-BIFTOOL, or
  Julia `DelayDiffEq` + a spectral solver), replacing the single-branch root tracker. This is filed as
  required work (R2) — not silently assumed done.
- **Add the "balanced-with-lags" scenario row (master B12 / receipt).** The one gap in the scenario
  ...
  oscillation once lags are present" — this is what the receipt currently only references.
...
## 14. Process & attribution (RC4 / RC6)
