...
Carrying capacity is one of ecology's most enduring yet elusive concepts. In classical population biology it
appears as a fixed number — a ceiling that caps logistic growth. Imported into global human ecology it is
usually treated as a static boundary: the maximum number of people the Earth can support (Cohen, 1995). Such
treatments overlook a fundamental feedback. If a population overshoots its resource base, it degrades the
environment that sustains it, thereby lowering the future ceiling. The ceiling itself becomes a moving
target.

**The orchard.** Imagine an orchard. Each year the trees bear fruit — a flow that can be taken without
reducing the trees themselves. This is nature's regenerative yield, which ecologists call biocapacity. The
same orchard is also a standing stock: its timber can only be obtained by cutting the trees. Now suppose the
harvesters take more than the orchard can regrow. At first nothing seems wrong: the trees still stand, the
harvest still comes. But the surplus is not free — it is paid for from the branches, the soil, and
eventually the trees. This is the *productivity illusion*: the mistaken belief that a stable or rising
harvest signals a healthy orchard. It lies at the heart of a long-standing debate in sustainability science
between those who hold that technology can always substitute for natural capital and those who argue that
substitution has hard limits.

The Global Footprint Network (GFN) makes this bookkeeping quantitative through two accounting variables
(Borucke et al., 2013). Biocapacity is the annual regenerative flow of biologically productive land and
water — the fruit of the orchard. The ecological footprint is humanity's annual demand on that flow — the
apples harvested. When the footprint exceeds biocapacity, an ecological deficit occurs, analogous to drawing
down a bank account: the annual balance is a flow, but its cumulative effect erodes the underlying stock of
natural capital, which in turn determines future biocapacity. The very quantity that defines the ceiling is
eroded by sustained overshoot.

A further complication is that neither the environment nor human populations respond instantaneously. Forest
loss, soil degradation, and climate change operate over decades; demographic patterns and social norms
exhibit considerable inertia. These features make the system a delay differential equation (DDE), a class
widely used to capture feedback delays in ecology (Kuang, 1993). They also explain why civilisations can
thrive for extended periods while silently undermining their productive base, only to experience abrupt
declines (Tainter, 1988; Diamond, 2005).

*Relation to existing literature.* The most prominent global human–environment model is the World3 system of
*The Limits to Growth* (Meadows et al., 1972, 2004) and *Dynamics of Growth in a Finite World* (Meadows et
al., 1974), which produces overshoot scenarios through complex feedbacks among numerous interacting stocks.
Since then a rich literature on coupled human–environment and resource-harvest dynamics has developed
(Brander & Taylor, 1998; Anderies, 2000). Estimates of Earth's carrying capacity span an enormous range,
from well under one billion to over one trillion people (Cohen, 1995; van den Bergh & Rietveld, 2004), and
some scholars argue the very notion of a fixed carrying capacity is inapplicable to humans (Simon, 1996).
Meyer (1994) and Meyer & Ausubel (1999) model carrying capacity itself as logistically varying, laying the
groundwork for an *emergent*-carrying-capacity framework; Dasgupta (2021) formalises natural capital as a
depreciating asset. These diverging perspectives highlight the need for frameworks in which carrying capacity
is not an exogenous input but an emergent property of the system.

**The overload. The same idea has an everyday form.** A system can bear overload for a time — an elevator
rated for 10 people will still manage 14, showing only subtle wear and no visible strain before the cable
suddenly gives way — while the underlying integrity is quietly eroding. Judged only by delivered function
(biocapacity, the harvest), the elevator still runs; judged by its structural reserve (the stock, the cable),
it is already failing. The productivity illusion is precisely this gap: apparent health on the outside
concealing depletion on the inside. The question this paper addresses is not whether such masking can occur,
but under what conditions it can be sustained and when monitoring fails to reveal it.

**The model.** We formalise this distinction in a minimal deterministic model. Biocapacity is the sum of a
*flow yield* — separable from the stock, taken without reducing it — and a *stock increment* — removable only
by liquidating the base itself. The share of these two components, the flow share `ψ`, governs how the
system responds to demand and determines whether a rise in measured biocapacity is a genuine recovery or a
mask over a declining stock. Two results are central. First, the constant-parameter subsystem possesses not
a unique interior attractor but a one-parameter family of equilibria, every interior point of which is
monotonically unstable: the onset of collapse is a structural vicious cycle, with a positive real eigenvalue
for every delay and no imaginary-axis crossing (hence no classical delay-induced Hopf). Second, the balance
point and the basin separator coincide in the simple scalar condition `R_B = 1` (footprint = total
biocapacity), while the more commonly monitored flow-yield ratio `R_A = 1` is only a leading, non-causal
indicator — and it fails entirely as a collapse forecaster once the regeneration lag is too long.
