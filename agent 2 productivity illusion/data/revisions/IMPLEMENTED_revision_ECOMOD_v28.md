...
Coupled human–environment models often treat carrying capacity as an imposed ceiling. We
present a minimal deterministic model in which it is instead an emergent
quantity—generated by the interaction of environmental stock, technological productivity
and per-capita demand—and show why treating it as fixed creates a false sense of
environmental health. Environmental assets—forests, soils, water—are natural capital: a
standing stock that yields a regenerative flow, and this flow is biocapacity.
Biocapacity is the sum of a separable flow yield and a capital-growth term, recovering
the orchard metaphor in one limit and the standard harvest model in the other.
Harvesting within regeneration rates leaves the capital base intact; extracting faster
liquidates the stock, reducing rather than contributing to future biocapacity. This
shortfall becomes ecological debt that erodes productivity, while environmental recovery
and demographic response act through two distinct delays.

Three results follow. First, the constant-parameter subsystem admits a one-parameter
family of equilibria (P = B(A)/e) rather than an isolated attractor, and every interior
point is monotonically unstable—a positive real eigenvalue for every delay, with no
imaginary-axis crossing. Collapse is therefore a structural vicious cycle, not a delay-
ratio Hopf bifurcation. Second, the operating boundary is the biocapacity ratio R_B = 1
(footprint equals total biocapacity); the flow-yield ratio R_A = 1 is a leading but non-
causal signal, and once the regeneration lag is long neither warns. Third, an early
technology wave can raise measured biocapacity while the stock falls—the productivity
illusion—but only within about five years, only for small initial deficits, vanishing
beyond modest overshoot. Yield gains saturate while debt compounds, and the same
technology raises biocapacity, sustainable population and aggregate demand, increasing
debt under overshoot. The model is deliberately stylised, its parameters representative,
its eight predictions falsifiable hypotheses. The decisive lever is the regeneration
delay; neither a rising harvest nor an early-sounding ratio signals collapse.
