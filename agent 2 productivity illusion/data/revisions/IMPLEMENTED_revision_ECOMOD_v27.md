...
Human–environment models usually treat carrying capacity as a fixed ceiling. We treat it
as something the system produces, and show why treating it as fixed creates a false
sense of environmental health. In ecological accounting, environmental assets—forests,
soils, water—are natural capital: a standing stock that yields a regenerative flow, and
this flow is biocapacity. It may be harvested as fruit is gathered, leaving the base
intact while offtake stays within regeneration rates. Drawing on it faster than it
renews cuts into the capital base itself; this liquidation depletes the standing stock,
reducing rather than contributing to future biocapacity. When demand exceeds the flow,
the shortfall is paid from the capital, and cumulative overshoot becomes an ecological
debt that erodes productivity. Recovery and population response each act with a delay,
so a society can appear thriving while its base declines.

Three results follow. First, a sustainable balance is not one fixed point but a family,
and each tends to decline: once demand presses past the renewable flow the system enters
a self-reinforcing loss needing no oscillatory mechanism—the decline is structural, the
constant-parameter subsystem monotonically unstable (a one-parameter family P = B(A)/e).
Second, the boundary separating recovery from collapse is when total demand equals total
biocapacity (`R_B = 1`); the flow-only ratio (`R_A = 1`) is a leading, non-causal
signal, not the trigger, and when the regeneration delay is long neither warns. Third,
an early technology wave can raise measured biocapacity while the base falls—within
about five years, and only while the initial deficit is small; beyond a modest overshoot
the window closes. Rising biocapacity need not signal improving health: the same
technology lifts biocapacity, sustainable population, and demand, raising debt. The
model is deliberately simple; its eight predictions are falsifiable hypotheses. The
decisive lever is the regeneration delay; neither a rising harvest nor an early-sounding
ratio warns of collapse.
...
So the sustainable equilibrium is interior (`A* < A_max`), not `A_max` — a steady harvest of the capital growth
