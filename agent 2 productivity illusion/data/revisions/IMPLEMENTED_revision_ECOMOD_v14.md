...
**Empirical grounding of `τ_g` and the delay-response robustness sweep (v14).**
...
- **`τ_p` handling.** `τ_p` was held fixed at `25 yr` (baseline). A coarse `(τ_g, τ_p)` grid
  (`reports/empirical_tau_g_tau_p_grid.json`, `…_grid.png`) shows the cliff is **governed by `τ_g`**: the
  recover fraction is **0.399 at `τ_g`∈{10,15} and 0.0529 at `τ_g`∈{20,30} for every `τ_p`∈{20,25,30}** —
  i.e. `τ_p` does **not** move the boundary. The qualitative results are expected to be similar for `τ_p`
  in the 20–30 yr range, but a **full two-delay sweep is left to future work**.
...
not measured. `τ_g` is the partial exception: it is now **field-banded** (§8, v13/v14 empirical-grounding
note) as a **regeneration/recruitment-response lag** (≈time to ~50 % of pre-disturbance productive
capacity), and the R1/R2 delay-response sweep shows the **collapse basin and the monotone no-Hopf
instability are robust across that band for `τ_g ≳ 20 yr`**, with `τ_p` (coarse grid) **not** moving the
cliff; the fast-regeneration regime `τ_g ≲ 18` is instead where the model returns the recovery outcome. **Fit-defect disclosure (frame from E2):** any parameter that sits **pinned at an
