...
> `scans/eco_recovery_insight.png` — density-dependent recovery + the rate/lag decoupling, §13(8)(vi)/(vii)),
...
> `SCAN_risk_register.md`, `SCAN_process.md`). **v20 top-down analysis:** the verified macro-ratio and
> bookkeeping results of §4.5/§13(9) are reproducible from the module `model_sims/topdown.py`
> (driver `model_sims/_run_topdown.py`, results `data/topdown_results.json`) and rendered as
> `scans/topdown_macro_ratios.png`, `scans/topdown_ratio_separation.png`, and
> `scans/topdown_delay_boundary.png`; the analysis and its reconciliation are in
> `reports/v20_topdown_macro_ratios.md`, `reports/v20_corrected_topdown_framework.md`,
> `reports/v20_review_adjoint_code.md`, and the merged plan `reports/v20_merged_implementation_plan.md`.
...
> model: parameters are representative, and the eight predictions it makes are stated as falsifiable
...
increment keeps the stock below its unmanaged maximum (the Schaefer/forestry picture; the
surplus-production / MSY logic here is **Schaefer (1954)**, *Bull. Inter-Am. Trop. Tuna Comm.*
**1**(2):25–56 — the same symmetric intrinsic-growth curve that gives `MSY = rK/4`). Only in the
...
located at `A_c(E)`, never at `A_max/2`. When a fold is *realized*, this is a **saddle-node (fold)
bifurcation** — the safe basin *vanishes* as `E` crosses `B_max`, the **catastrophic-shift /
critical-transition** structure of alternative-stable-state ecology (Scheffer, Carpenter, Foley, Folke &
Walker 2001, *Nature* **413**:591–596): a loss of resilience precedes the switch, the switch is abrupt
and hard to reverse — not because of an imposed Allee minimum, but because the *stable and unstable
equilibria first meet* (the fold) then destroy each other. **Scope it correctly:** this fold is realized
**only in the increment-dominated regime (`b_G ρ > b`)**, where `B(A)` has an interior maximum. There,
`dB/dA = 0` **exactly** at the interior MSY (`A* = A_max(b+b_Gρ)/(2b_Gρ)`), and for `E` just below `B_max`
there are **two** fixed points — one **unstable** (`B'(A)>0`) and one **stable** (`B'(A)<0`) — that merge
at the saddle-node (verified numerically: `B'(A*) = 0`; two roots with opposite-sign `B'` for `E<B_max`).
At **baseline** (flow-dominated, `b_G ρ = 0.04 < b = 0.5`) `B(A)` is **monotone increasing** on
`[0, A_max]` and `B'(A) > 0` for every `A` in the domain, so **no interior fold is realized**; the
fixed-liability equilibria are single-valued and unstable, and the stable object is the boundary `A_max`
(consistent with §4.1's regime table and R1). **Do NOT conflate this with the `τ_g` cliff (§8, §13).**
The `τ_g` transition is a **basin-boundary crisis, not a fold**: R2 finds a **constant positive-real
eigenvalue (`Re λ ≈ +0.62`) for every `τ_g`** — no eigenvalue crosses zero as `τ_g → 20`, so there is **no
critical-slowing-down / early-warning-signal precursor** (verified: the `P`-relaxation return time is
flat ≈ 150 yr for `τ_g=0–18`, then the attractor vanishes abruptly at `τ_g≈19–20` rather than slowing).
So Scheffer's "loss of resilience precedes the switch" describes **only** an *increment-dominated*
`E`-fold; it is the wrong description of the *delay* transition. The two must be told apart (§13(9)).
...
```
because `[E − bA]₊ = E − bA` there, and the lag-adjusted biocapacity is **`B̃ = bA + b_G G(A(t−τ_g))`**
(so `B̃ = B` only when the regeneration is evaluated at the *delayed* state). This is the algebraic
identity that makes "deficit-driven" rigorous — the *shortfall* between biocapacity and footprint, not the
...
In addition to the dimensionless groups, the bookkeeping can be reported as **two observable ratios**
that summarise the whole safe-operating picture. Define the **biocapacity ratio** and the **flow-yield
ratio**:
...
`P = B(A)/e`.** From the deficit identity `dA/dt = (B̃ − E)/b_G` (§4.3), `dA/dt < 0 ⟺ E > B̃`, i.e. the
stock declines **iff footprint exceeds total biocapacity**. At equilibrium `E = B̃` so `R_B = 1`; this is
simultaneously the **bookkeeping trigger**, the **macro-ratio safe-operating boundary**, and (as the family
`P = B(A)/e`) the **basin separator** of R2. Verified onset (baseline S0, documented `A₀` grid): the
recover/collapse boundary is crossed at **`R_B = 1.000`** for every `A₀ ∈ {0.30,…,1.05}`, while the
flow-yield ratio there is **`R_A = 1.01–1.06`**.

**`R_A = 1` is necessary but not sufficient.** `R_A = 1` means `E = bA`, i.e. demand equals the *flow*
only. A system sitting between `R_A = 1` and `R_B = 1` still regenerates (`dA/dt > 0`), because the
increment `b_G G(A)` is untouched. `R_A > 1` is therefore **not** the collapse trigger, and the two ratios
are **not** interchangeable: `R_A` is a *leading, non-causal* signal, `R_B` is the trigger.

**The gap between them is the flow share `ψ`; the flow share is the master parameter.** `R_A = R_B/ψ`, so
`R_A` and `R_B` coincide in the flow-dominated limit (`ψ → 1`, baseline) and separate as `ψ → 0`
(increment-dominated). **Regime-scoped closed form** (at a sustainable interior MSY): `ψ* = 2/(1 + b_Gρ/b)`,
`R_A^eq = (1 + b_Gρ/b)/2`. Verified across regimes: `b_Gρ/b = 0.05` (flow-dominated, no interior MSY,
`ψ → 1`, `R_A = 1`); `2.0 → ψ* = 0.667, R_A^eq = 1.50`; `37.5 → ψ* = 0.052, R_A^eq = 19.25`; `150 → ψ* = 0.013,
R_A^eq = 75.5`. So the single regime index `b_Gρ/b` governs the gap between `R_A` and `R_B`
(`ψ* = 1/R_A^eq = 2/(1 + b_Gρ/b)`).

**Consequence — a sustainable system can run at `R_A > 1` (and this is normal).** In the
increment-dominated regime the balance point is `R_B = 1` with `R_A > 1` (footprint > flow yield, the
buffer intact). Do **not** read `R_A > 1` as a warning; the balance point is `R_B = 1` in *every* regime.

**Temporal-lead nuance.** Because `R_A = R_B/ψ`, `R_A` crosses 1 *earlier* than `R_B` by the factor `1/ψ` —
but its lead is **≈ 0 in flow-dominated systems** and grows only with increment-dominance. A large lead is
therefore a *signature* of the increment-dominated regime, not a general warning property.
...
(no-delay) → **81.2 %** (`τ_g = 30`); a **linear functional** is **chance** at `τ_g = 30` (balanced 50.0 %,
raw 94.7 % = the majority-collapse class = a class-imbalance artefact). So `R_B = 1` is **necessary** for
...
7. **Regeneration *rate* vs *lag* are two decoupled levers** (§13(8)(vi)): time-to-50 % scales ≈ inversely
   with the regeneration rate `ρ`, but whether the stock recovers *at all* is set by the regeneration lag
   `τ_g`. So a system with a long `τ_g` cannot be rescued by raising `ρ` alone — the lever is which one is
   changed, and the model predicts a **different recovery time for the same `ρ` depending on `τ_g`**, with
   the outcome flipping at `τ_g ≈ 20` yr for *every* `ρ`.
8. **The *operating* boundary is `R_B = 1` (footprint = total biocapacity), not `R_A = 1` (§4.5).** The
   flow-yield ratio `R_A` crosses 1 *earlier* than `R_B` (by `1/ψ`) but is **not** the collapse trigger, and
   its lead is ≈ 0 in flow-dominated systems (§4.5). Consequently a system can be on the collapse side while
   `R_A > 1` (increment-dominated) — and, in the long-`τ_g` regime, **even while `R_B < 1`** (silent
   collapse, §13(9)). Falsifiable: monitor the two ratios on the basin/field data and confirm the trigger is
   `R_B = 1`, not the earlier-crossing `R_A`.
...
- **Literature (priority statement — be precise).** The delayed-logistic *stability* result is
  ...
  *aggregate and data-limited* — they rely on measured yields and conversion factors, do not capture
  ...
  (see §11). Cite or remove the orphan May (1973). **Ecological-theory grounding (add where each is
  used):** the surplus-production/MSY curve in §4.1 is **Schaefer (1954)** (*Bull. Inter-Am. Trop.
  Tuna Comm.* **1**(2):25–56); the fold/saddle-node with a vanishing safe basin in §4.2/§13 is the
  **catastrophic-shift / critical-transition** structure of **Scheffer, Carpenter, Foley, Folke &
  Walker (2001)** (*Nature* **413**:591–596); and the empirical recovery-band sources (Poorter et al.
  2016; Poeplau et al. 2011 + IPCC 2006; Hutchings & Reynolds 2004; Neubauer et al. 2013) already
  ground the `τ_g` band.
  *aggregate and data-limited* — they rely on measured yields and conversion factors, do not capture
  soil erosion, deforestation or groundwater depletion, and GFN notes likely **overstate** biocapacity
  and **understate** the Footprint, so the 1961–2022 series is a **lower bound** on genuine overshoot
...
- **Provenance — these are ORIGINAL-model numbers (important, do not gloss).** The `0.506 → 0.042`
  ...
  - the corrected analogue is the **recover (`A→A_max`)/ collapse (`A→A_ext`) boundary** (with the
    ...
    `A→1.36`, `τ_g=30`) — the mechanism, in trajectories rather than only a basin fraction. A companion
    recovery-side figure (`scans/eco_recovery_insight.png`) shows the **density-dependent, sigmoidal
    recovery trajectory** (slow near the floor, `G≈ρA`) and the **decoupling of regeneration rate (sets
    recovery time) from regeneration lag (sets recovery outcome)** (§13(8)(vi)/(vii)).
...
- **Delay-response sweep (R1/R2, baseline `τ_p=25`; `reports/empirical_tau_g_sweep.json`, `…_sweep.png`).**
  ...
  **0.399 (`τ_g ≲ 17`), 0.394 (18), 0.240 (19), 0.053 (`τ_g ≳ 20`)** — so the "cliff" is a **steep
  ...
  returns the weak-sustainability/recovery outcome. And it is **not merely a one-time basin loss**: §13(8)(v)
  shows the recovery is *not self-sustaining* either — starting from the floor the stock regrows, overshoots
  `A_max`, and re-collapses, so the collapse is effectively **absorbing** under the regeneration lag. This is a
  **basin-boundary crisis, not a fold** — it carries **no critical-slowing-down / early-warning precursor**
  (§13(9)); the reader should not await a smooth warning signal. State
  the interval **scoped**, never as a flat
...
- **The cliff is `τ_p`-independent across the whole tested set, and there is **no Hopf** anywhere in the
  delay plane — `topdown.py` (A1, A5).** Extending the sweep to `τ_p ∈ {0,5,10,15,20,25,40,60}` with
  `τ_g` stepped at 1-yr resolution: the last recovering `τ_g` is **`τ_g = 18` for *every* `τ_p`** (no
  dimensionless `τ_g/τ_p` tuning rescues the cliff), and a full `(τ_g, τ_p)`-plane scan of the
...
- **Reproducibility (v20).** Every number in §4.5 and §13(9) that is labelled "verified" is generated by
  ...
  AI‑C6 fixed points, and the full-plane no-Hopf). These are **corrected-model** results with the same
  model-provenance rule as R1/R2; nothing here is an original-model number.
...
**typed** floor — the stock `A` — is violated. **Quantitatively it is the substitution buffer:** because
`B = bA + b_G G(A)`, the aggregate floor on `B` can be met while the typed floor on `A` is violated by
exactly the increment `b_G G(A)` — the same separation that shows up in §4.5 as the gap between `R_B` and
`R_A`. (Frame from P1/R1 — leverage of the existing typed-ledger point, not a new computation.) To say it
in this paper's notation: the aggregate that an
...
| macro-ratio safe operating space (`R_B` vs `R_A`), `R_B=1` = trigger & separator (v20) | **new §4.5** (`topdown.py`) |
...
curve as a function of `(τ_g,τ_p)` is the natural follow-up to the §8 R1 figure. (v) **Recovery from the
extinction floor is NOT self-sustaining at realistic lags (computed R1, one-sided integration from
`A₀=A_ext`, `P₀≈0`).** Starting from the post-collapse floor at `τ_g ≥ 20`, the stock regrows but
**overshoots `A_max` and re-collapses to `A_ext`**, rather than settling at the sustainable boundary:
`A_peak` = 1.31 (`τ_g=20`), 1.45 (30), 1.60 (40), 1.88 (60), and the fraction of the run spent above
`0.5·A_max` falls monotonically (0.16 → 0.045 → 0.033 → 0.020) — so the collapse is effectively
**absorbing**, not a transient excursion. This is the ecological content of the rise in the §8
"recover vs collapse" claim and matches the field picture of **slow/incomplete, often failed,
recovery** (Hutchings & Reynolds 2004; Neubauer et al. 2013). State it as such: **the collapse is not
merely hard to reverse, it is *self-sustaining under the regeneration lag*.** (Note this is separate
from the §(5) masking window and from the Allee-free fixed-liability threshold — it is the *same*
delay-driven liquidation, but read as a recovery-blocking mechanism rather than a collapse mechanism.)

**(vi) The regeneration *rate* sets HOW LONG recovery takes; the regeneration *lag* sets WHETHER it
happens at all — two decoupled levers (computed R1).** Sweeping `ρ` (regeneration rate) and `τ_g`
(regeneration lag) independently from the extinction floor, the time to 50 % of `A_max` scales
**roughly inversely with `ρ`** (t50 ≈ 145/ρ yr: 203 at `ρ=0.03`, 142 at 0.05, 105 at 0.08, 82 at 0.12,
at `τ_g=30`), while **whether it recovers at all is decided by `τ_g`** (recovered at `τ_g ≤ 18`,
collapsed at `τ_g ≥ 20`, **for every `ρ`**):
...
So a manager has **two genuinely different levers**: accelerate the regeneration *rate* (e.g. restore
stock-by-stock productivity) to shorten the time to recovery, versus *shorten the regeneration lag* (e.g.
recruit/protect the seed source) to change whether recovery happens. The two do **not** substitute:
raising `ρ` makes a doomed trajectory recover *faster to 50 %* but does **not** keep it from collapsing
under a large `τ_g`. This is a testable, policy-relevant ranking (see §6, prediction 7).
**(vii) Recovery *overshoots* the old `A_max` before settling (a catch-up rebound pulse).** From the
floor, a system that does recover rises **past** `A_max` before relaxing: `A_peak` = 1.21 (`τ_g=10`),
1.25 (15), 1.28 (18) — i.e. up to `+0.08` beyond `A_max` (fig. `scans/eco_recovery_insight.png`). This is
the Hutchinson-style overshoot but on the *recovery* side, and it is why a recovering system can appear
to "overshoot" briefly — a transient that must not be read as a new stable overshoot. (It is distinct
from the §10 collapse-regime overshoot, which ends in liquidation.)

**(9) Structural properties and negative results (what the corrected model does NOT do, and why it
matters).** Several phenomena common in ecological models are **absent** here, and the absence is
*a structural property*, not an oversight. Each is verified and stated so the reader does not expect it:

| Phenomenon looked for | Present? (verified) | Why absent / what it *is* | Implication |
|---|---|---|---|
| **Critical slowing down (CSD) / early warning** before the `τ_g` cliff | **No** | the `τ_g` transition is a first-order **basin-boundary crisis**: `Re λ ≈ +0.62` is constant (no eigenvalue crossing zero), and the `P`-relaxation return time is flat ≈ 150 yr for `τ_g=0–18` then vanishes abruptly at `τ_g≈19–20` | **Do not** expect a smooth warning signal before a long-`τ_g` collapse; monitoring a single trajectory's relaxation time will **not** warn you |
...
| **Endogenous limit cycle** (sustained boom–bust) | **No** | the regrow-overshoot-recollapse is a single **recovery-overshoot pulse** (2 crossings of `0.5·A_max`), consistent with R2's monotone (no-Hopf) instability | the overshoot is **not** a self-oscillation; sustained boom–bust would need an external mechanism |
...
**So the corrected model's collapse is *robust but not predictable from a single scalar*: it is driven by
the regeneration lag (a finite-amplitude basin erosion), not by a fold, not by Allee low-density dynamics,
and it carries no generic CSD precursor.** This is *more* informative than a set of positive results: it
tells a manager which levers don't exist (no rescue-by-stock, no early-warning signal) and which do
(shorten the lag; and, only for the increment-dominated `E`-fold, watch for slowing). Model **extensions**
suggested by the negative results — stochasticity, seasonal forcing (to test for induced cycles), or an
explicit Allee term (to test for a rescue route) — are noted as **future-work directions**, not changes to
the current (deliberately minimal, deterministic, Allee-free) model.

**The macro-ratio monitor fails silently in the long-`τ_g` regime (verified — the honest negative).** The
`R_B`/`R_A` safe-operating ratios of §4.5 are **necessary-but-not-sufficient** once the lag is too long:
on the documented `A₀×P₀` grid at `τ_g = 30`, **36.5 % (72 of 197) of collapses begin with `R_B < 1` AND
`R_A < 1`** — i.e. *neither* ratio warns; only shortening the regeneration lag helps. (At `τ_g = 10`,
only 1.6 % of collapses are silent.) So ratio-based monitoring **cannot** replace the lag: it is a
leading/necessary check, not a collapse forecaster. This directly targets prediction #8 (§6, §4.5).

**The rescue set collapses to a measure-zero strip (verified — there is no "start higher" fix).** The
rescue set (initial conditions that recover) is a **39.9/39.9/39.4 %** A-span of **0.100–1.300 (13 distinct
`A₀`)** at `τ_g = 0/10/18`, then collapses to **5.3 %** with a **1.200–1.200 (1 distinct `A₀`)** strip at
`τ_g ≥ 20` — i.e. at `A_max` only (`data/topdown_results.json`, `topdown.py` AI‑C4). Combined with the
absorbing collapse (§13(8)(v)), this is the quantitative form of the "cannot rescue by starting higher"
statement in the table above.

**The transition is a nonlocal, not a map, bifurcation (verified — AI‑C6).** The asymptotic one-step map
(when the delays are restored to their large-time limits) has fixed points exactly on the equilibrium
family (`E = 0.15 → A = 0.283`; `0.30 → 0.576`; `0.50 → 0.986`; boundary `A_max = 1.2`) and its local
`Re λ_max` is **`+0.5917 → +0.6249 → +0.6250`** — i.e. **constant, no flip** across `τ_g ≈ 19`. The
asymptotic map therefore reproduces the fixed points but **not** the `τ_g` cliff, confirming the transition
is a **nonlocal basin-boundary crisis**, not a map bifurcation, and that the discrete and continuous
pictures do **not** agree as `τ_g` is increased. This corroborates the no-CSD reading of the *delay*
transition (§8), which we distinguish from the `E`-fold of §4.2.

**Separator degradation is a class-imbalance artefact unless reported by balanced accuracy (verified).**
The neutral-family separator `P = B(A)/e` (i.e. `R_B = 1`) classifies the basin at **balanced 99 %**
(no-delay) → **81 %** (`τ_g = 30`); a linear functional shows 96 % (no-delay) → **50 %** (`τ_g = 30`,
chance) while its *raw* accuracy is **94.7 %** — equal to the majority-collapse class (197/208). Report
separator/ratio accuracy by **balanced accuracy**, never raw accuracy on the long-`τ_g` basin.

**Recovery-overshoot is real, initial-condition-dependent, and steep (qualifier to §13(8)(vii)).** The
recovery overshoot scales steeply with the lag (a fit to the computed pulse gives `ov ≈ 0.0047·(τ_g/10)^4.84`,
