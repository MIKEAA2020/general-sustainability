...
Human–environment models usually treat carrying capacity as a fixed ceiling. We treat it instead as something
the system itself produces, and we show why treating it as fixed can create a false sense of environmental
health. The environment is represented as a base of natural capital — the standing stock of trees, soils, and
water — that yields a regenerative flow each year. That flow is biocapacity: it can be harvested, as fruit is
gathered, without reducing the base, provided it is not taken faster than it regenerates. The standing stock
is not itself biocapacity but the capital that generates it; cutting into that stock faster than it
regenerates is liquidation of capital, and it reduces future biocapacity rather than adding to it. When
demand exceeds what the flow can supply, the shortfall is paid from the capital, and cumulative overshoot
accumulates as an ecological debt that steadily erodes the base's productivity. Environmental recovery and
population response each act with a delay, so a society can appear to be thriving while its resource base
quietly declines.

Three results follow. First, a sustainable balance is not a single fixed point but a whole family of possible
balances, and every one of them is inherently inclined to decline. Once demand presses past the renewable
flow, the system enters a self-reinforcing loss that requires no oscillatory, delay-triggered mechanism to
begin: the decline is structural rather than a delicately tuned wobble. Second, the boundary that actually
separates recovery from collapse is when total demand equals total biocapacity. The more commonly monitored
flow-only ratio is only an early warning signal, not the trigger, and when the regeneration delay is long
neither ratio gives warning. Third, a sufficiently early technology wave can make measured biocapacity rise
while the underlying base falls — but only within a narrow window of about five years, and only while the
initial deficit is small; beyond a modest overshoot the window closes. Rising biocapacity therefore need not
signal improving environmental health: the same technology that lifts biocapacity also lifts the sustainable
population and aggregate demand, tending to increase cumulative debt under overshoot.

The model is deliberately simple and its parameters illustrative; its eight predictions are stated as
falsifiable hypotheses rather than calibrated forecasts. The practical message is that the decisive lever is
the regeneration delay, and that neither a rising harvest nor an early-sounding ratio can be trusted to warn
of collapse.
...
stock, taken without reducing it — and a stock increment — removable only by liquidating the base. The share
