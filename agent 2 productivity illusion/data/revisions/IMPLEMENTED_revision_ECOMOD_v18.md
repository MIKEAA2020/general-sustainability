...
located at `A_c(E)`, never at `A_max/2`. This is a **saddle-node (fold) bifurcation** — the safe basin
*vanishes* as `E` crosses `B_max`, which is precisely the **catastrophic-shift / critical-transition**
structure of alternative-stable-state ecology (Scheffer, Carpenter, Foley, Folke & Walker 2001,
*Nature* **413**:591–596): the loss of resilience precedes the switch, and the switch is abrupt and hard
to reverse — not because of an imposed Allee minimum, but because the *stable and unstable equilibria
first meet* (the fold) and then destroy each other. Cite this as a *framing*, not as a claim that this
model reproduces Scheffer's cases. **Do NOT conflate this with the `τ_g` cliff (§8, §13).** The `τ_g`
transition is a **basin-boundary crisis, not a fold**: R2 finds a **constant positive-real eigenvalue
(`Re λ ≈ +0.62`) for every `τ_g`** — there is no eigenvalue crossing zero as `τ_g → 20`, so there is **no
...
So Scheffer's "loss of resilience precedes the switch" is the wrong description of the *delay* transition;
it applies to the *fixed-liability* `E`-fold of this subsection. Only the latter admits a generic CSD
precursor; the former must be read as a **finite-amplitude basin erosion**.
...
- **Delay-response sweep (R1/R2, baseline `τ_p=25`; `reports/empirical_tau_g_sweep.json`, `…_sweep.png`).**
  ...
  **0.399 (`τ_g ≲ 17`), 0.394 (18), 0.240 (19), 0.053 (`τ_g ≳ 20`)** — so the "cliff" is a **steep
  ...
  `A_max`, and re-collapses, so the collapse is effectively **absorbing** under the regeneration lag. State
