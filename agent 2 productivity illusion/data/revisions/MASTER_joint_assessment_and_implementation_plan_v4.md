...
rather than contradicted; the characteristic equation and figures are recomputed and match; an
equilibrium exists (η); the two-delay classification is correct (χ sign); **the corrected-model basin
recompute (R1) and characteristic-equation/full-spectrum analysis (R2) are computed and reported**; and
no claim is asserted without a protocol. (Residual: the corrected scenario *table* in the paper's own
format and the corrected figure *set* remain an editorial/rendering step — the computations behind them
are done.)
...
**Analytics:** [x] recompute characteristic equation · [x] full-spectrum + exact crossing-curve
methods (R2: neutral zero eigenvalue + positive real leading mode ⇒ monotone, NOT a Hopf —
`χ` classification does not transfer to the corrected S0) · [ ] recompute table + all figures ·
...
**Measured basin (12G.2/12G.5):** [x] report stable-fraction 0.506 → 0.042 (std IC flips) · [x] add basin_shrinkage.png · [ ] state the (20,20) recovers / (30,25) collapses boundary · [ ] state B/C = “environment recovers, humans collapse” explicitly.
**R1 (corrected-model basin recompute):** [x] compute the corrected `(1‴)` recover/collapse basin on a documented `A0×P0` grid (recover 39.9% no-delay → 5.3% at (30,25); delay-triggered `A_max`-overshoot → `E>bA` → liquidation; recover basin vanishes as `τ_g` crosses ~20 yr) · [x] add `scans/r1_basin_baseline.png` + `scans/r1_basin_delay_response.png`.
...
- Recompute the characteristic equation, table, and figures for `(1‴)` — not cosmetic. **R2 (done):**
  the corrected-S0 characteristic equation `D(s;τ_g,τ_p)=(s−a₁e^{−sτ_g}−a₃)(s+r)−a_E a₄e^{−sτ_p}=0` has a
  neutral zero eigenvalue on the whole equilibrium family `P=B(A)/e` and a **positive real** leading
  mode (`Re λ≈+0.59`) for all delays — a **monotone** instability, **no Hopf crossing**, and the
  zero-delay `a₁₁<r` condition is violated everywhere. The `χ` Hopf classification does **not**
  transfer to the corrected S0 (see `model_sims/char_eq.py`, `SCAN_risk_register_r1_r2.md`).
- R1 (done): the corrected-model recover basin is computed — recover 39.9% (no delay) → 5.3% at
  `(30,25)`, driven by `τ_g`-triggered `A_max`-overshoot → `E>bA` → liquidation (`scans/r1_basin_*.png`).
- **R1/R2 sensitivity & validity now quantified** (revision §13, points (2)–(6)): the full-`D(s)=0`
  leading eigenvalue is ≈ +0.62 and **independent of `ρ`** (`ρ/r = 1–75`); at realistic
  `ρ ∈ [0.02,0.1]` (`ρ/r = 1–5`) the fast–slow `χ` reduction is **out of its regime** so the full
  transcendental equation is used; the R1 recover-collapse is **robust to `b_G`** (no-delay 0.394→0.404,
  baseline pinned at 0.053 over `b_G = 0.4–1.2`); the delay-response re-entry at `τ_g ≈ 40–60` is **real**
  (fine-grid min at `τ_g=30` ≈0.03 → re-opens to ≈0.21/0.30/0.32 at 40/50/60). Set **`η = 0.05 yr⁻¹`**
  (minimal regeneration; model insensitive to `η` provided `η > 0`); R1/R2 operate on the constant-S0
  where `η` is absent, so the `η → 0` singularity concerns the full `(6′)`.
