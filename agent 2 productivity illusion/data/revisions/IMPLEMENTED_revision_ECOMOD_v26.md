...
Human–environment models usually treat carrying capacity as a fixed ceiling. We treat it as something the
system itself produces, and show why that framing creates a false sense of environmental health. The
environment is natural capital — standing trees, soils, water — yielding a regenerative flow. That flow is
biocapacity: harvested as fruit is gathered, it leaves the base intact if not taken faster than it regrows.
The standing stock is not biocapacity but the capital that generates it; cutting into it faster than it
regrows is liquidation, reducing future biocapacity rather than adding to it. When demand exceeds the flow,
the shortfall is paid from the capital, and cumulative overshoot becomes an ecological debt that erodes
productivity. Recovery and population response each act with a delay, so a society can appear thriving while
its base quietly declines.

Three results follow. First, a sustainable balance is not one fixed point but a family, and each tends to
decline: once demand presses past the renewable flow the system enters a self-reinforcing loss needing no
oscillatory mechanism — the decline is structural, the constant-parameter subsystem monotonically unstable (a
one-parameter family P = B(A)/e). Second, the boundary separating recovery from collapse is when total demand
equals total biocapacity (`R_B = 1`); the flow-only ratio (`R_A = 1`) is a leading, non-causal signal, not the
trigger, and when the regeneration delay is long neither warns. Third, an early technology wave can raise
measured biocapacity while the base falls — only within about five years, and only while the initial deficit
is small; beyond a modest overshoot the window closes. Rising biocapacity need not signal improving health:
the same technology lifts biocapacity, sustainable population, and demand, tending to increase cumulative
debt. The model is deliberately simple and illustrative, its eight predictions falsifiable hypotheses. The
decisive lever is the regeneration delay; neither a rising harvest nor an early-sounding ratio warns of
collapse.
