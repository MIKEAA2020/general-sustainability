...
|---|---|
| "instability requires both delays, threshold ≈ 80 yr" | "the two-delay stability boundary is controlled by the single group χ (χ=1 ⇔ ρ=3q); the onset is ratio-dependent, marginal (Re λ ≲ 0.01 yr⁻¹), and closed-form: s=π/ω at χ=1. The number 78.5 yr = π/(2r) arises because both loops contribute equal delayed gain r at χ=1." |
...
- **Why this matters for the claim.** The master (Parts 12A.1, 12G.7) offered headline masking sets
  ...
  debt grows and closes the window).
...
it. The empirical programme the Discussion promises is the **`d ln B = d ln b + d ln A`** decomposition
with independent `A`-proxies (land cover, soil carbon, NPP).
