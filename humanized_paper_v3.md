...
#### Editorial additions — related work not cited in the original (v3)

*The following items are **not** in the original paper; they are added here on request, and each is tied to a specific claim the paper already makes. Items marked "empirical" are cited strictly as illustrations of a phenomenon the paper formalizes — the paper itself makes no empirical claims and none are imported here.*

1. **The algebraic home of the backward recursion.** The paper's §5.2 traces its recursion to the viability/reachability lineage via books and a general SIAM survey. The *precise* journal reference for the fixed-point character the recursion relies on is **Aubin & Catté (2002)**, which proves that viability/invariance kernels and capture/absorption basins are largest/smallest fixed points (openings/closings) of maps on pairs of sets, and that the **Saint-Pierre algorithm** — the iteration Kₙ₊₁ = C ∪ (Kₙ ∩ Φ⁻¹(Kₙ)) — is exactly this fixed-point iteration. That is the right citation for the statement "the accepted-state recursion converges to the largest fixed point," which the paper uses implicitly. It also supplies the vocabulary for reading the acceptance gap as a fixed-point failure: per-weight acceptance is an intersection of fixed points of the maps E_w, while common-plan acceptance is a fixed point of the intersection ⋂_w E_w.

2. **The game-theoretic reading's viability home.** The paper's §4.10 remark frames the assessor–planner gap as pure-vs-mixed strategies (von Neumann; Sion). The viability-theoretic home of that reading is the **discriminating kernel** — the "viability kernel against an adversary" — of **Cardaliaguet (1996)** and its numerical treatment in **Cardaliaguet, Quincampoix & Saint-Pierre (1999)**. This is the standard machinery for two-player reach/avoid games, and it is where a "single plan that defeats every assessor weight" would be formalized as a discriminating domain; Theorem 8's convexification is then the mixed-strategy closure of a discriminating-kernel problem.

3. **The empirical instantiation of separately-checked floors (illustrative).** The "doughnut" of social floors and planetary ceilings is the canonical empirical program that *refuses* to aggregate exactly because aggregates mask per-floor shortfalls: **O'Neill, Fanning, Lamb & Steinberger (2018)** check 11 social thresholds and 7 biophysical ceilings **separately, with no compensation across them**, and find no country inside the safe space. This is the paper's Third policy implication ("per-floor reporting is a detection requirement") in operation. Its dynamic follow-up, **Fanning, O'Neill, Hickel & Roux (2022)**, tracks the same floors/ceilings as *time series* (1992–2015, 148 countries) and finds countries leave the safe space **along the trajectory**, not just at the endpoint — an empirical counterpart of "the endpoints are the photograph; the tube is the trajectory." And **Lade et al. (2020)** documents that planetary-boundary processes **interact and amplify** one another — the empirical anchor for the paper's §5.3 caveat that its separation is scoped to an *action-indexed* (uncoupled) disturbance class, and that coupled all-floor shocks are a genuinely different regime.
...
**Added in v3 (editorial — not in the original reference list):**
...
- Cardaliaguet, P., Quincampoix, M., and Saint-Pierre, P. (1999). Set-valued numerical analysis for optimal control and differential games. In: *Stochastic and Differential Games: Theory and Numerical Methods*, Annals of the International Society of Dynamic Games, vol. 4, Birkhäuser, 177–247.
