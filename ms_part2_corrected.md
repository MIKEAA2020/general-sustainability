...
*Corrected edition (batch-5, 2026-08-29): the hybrid-trajectory formula of §8.1 is repaired — the committed file carried two literal carriage-return bytes where the `
` of `
ightarrow` belonged, rendering as `ightarrow`; the post-arrival maintenance horizon \(T'\) of §8.2 is defined and the post-reset state reading of \(z(T^*)\) is stated. Adjudication record: `BATCH5_JOINT_AUDIT_EVALUATION.md`.*
...
\[
\tau=
(q_0,z_0)\rightarrow(q_1,z_1)\rightarrow\cdots .
\]
...
Formally, a candidate meta-policy \(\Pi\) seeks some \(q'\) and time \(T^*\) (the arrival deadline) such that
...
while all transition constraints hold before \(T^*\). Here \(z(T^*)\) is read in the destination architecture's phase space — reached from the pre-arrival state through the architecture-transition reset declared with the transition action (a reset or translation map between the two phase spaces; the successor monograph writes it \(R_{qq'}\)) — and \(T'\) is the post-arrival maintenance horizon: the horizon over which the destination architecture's viability must hold after arrival, declared per candidate.
