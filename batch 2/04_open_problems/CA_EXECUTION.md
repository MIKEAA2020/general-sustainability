...
With `Succ` a table, `Pre(W)` is computed by scanning all `G·|U|·|D|` table entries and testing subsethood against `W` (bitwise on the `G`-bit characteristic vector): `O(G·|U|·|D|)` bit operations. `Pre(W)` is monotone in `W` (`W ⊆ W' ⇒ Pre(W) ⊆ Pre(W')`).
...
**Lower bound (arbitrariness).** Let `S ⊆ 𝕄` be any subset of the finite model lattice. For each `M ∈ S` and each `M' ∉ S`, the pair differs in some atomic datum; the sentence `Φ_S := ⋁_{M ∈ S} Φ_M`, where `Φ_M` is the finite conjunction of atomic claims (kernel memberships at the initial state, evaluated by Thm2) together with their negations, pinning `M` up to the language's distinguishing power — has satisfying set exactly `S` *provided the language separates the models* (which it does on the declared class: two instantiations differing in any successor-table entry, safe-set cell, or disturbance element are separated by an atomic claim evaluated at a distinguishing state, since the table is part of the data and the atomic kernel claims read it). `Φ_S` is in the language (Boolean closure), and by construction non-monotone whenever `S` is neither an up-set nor a down-set. Hence arbitrary satisfying sets occur, and no extremal-evaluation shortcut exists for them. ∎
