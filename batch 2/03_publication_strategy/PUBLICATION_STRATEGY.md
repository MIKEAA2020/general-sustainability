...
| 5 | Sampled Governance, Closed-Loop Certification, Empirical Falsification | How can the theory be tested? | + sampled-data erosion theorem (B1, repaired two-depth form) closes R02.Cor6's bridge at the depth bookkeeping `V_max T_s ≤ R − r` (a sample-time certificate at depth `R` converts to continuous-time safety at depth `r`); + decidability theorem (C-a) guarantees computability at fixed data on the finite class (TCS-1.0 language); + the E5 admission template for case screening — **template only: the linear toy's numbers support no real-system claim (R04 forbids transfer)** |
...
| **Edwards well J-17** | San Antonio Pool index well (TWDB 6837203). Scored \(\Omega_{\mathrm{SA}}\) | [`wave_e_edwards/`](../../wave_e_edwards/) | Real system — groundwater | two-pool **not** constructed | H0 forecast-map `APPROXIMATION` in `wave_e_edwards/admission/R04_Cor2_edwards_H0.md`; **kernel-level Cor2 row** `R04_Cor2_edwards_kernel.md` (2026-08-26: governed one-pool object, erosion conversion invoked, `APPROXIMATION` defect-bound to T ≤ 3 yr; the §15 intervention-selection leg executed on it — S1/cpm retained at the drought-floor/physical reading, institutional-threshold negative certificate) | **NONE** (intervention artifacts first-run) |
...
**No gate is treated as closed for Wave E without spec matching and independent verification.** Every Wave E support row is NOT CONFIRMED until the frozen specification (S, Ω, y_t, T, scoring rule) is matched against the computation artifacts. **[Update 2026-08-26: the two scored trees are now spec-matched at the artifact level — `batch 4/WAVE_E_SPEC_MATCH.md`, 36 machine checks exit 0 (Edwards with a dated pre-score protocol; cod manuscript-declared with recorded freeze-discipline and coverage caveats). The Part III paper-support rows remain NOT CONFIRMED — they concern paper claims (continuum lift, NAIM persistence, governance-template instantiation), not the scored trees.]** See PROOF_MANIFEST.md Part III and its "Reproducibility status" section (which carries the consolidated disclosure content).
