...
2. **It is not the real system.** The programme's real-system targets are of a different class: the northern-cod program (NAFO **2J3KL**; A014/A016) and the scored delay-model series of the A021 joint docket (the **J-series**, incl. J17), whose C4 gated DDE is the object of the validated computations. **No number in this file or in `E5_NUMBERS.json` is a statement about 2J3KL, about any J-series model, or about any calibrated real system.**
3. **R04 forbids transfer.** By R04.Thm1's converse (batch-2 record, Field 8), no judgment transfers from a module to any other model without the five-map admission certificate (type/unit, phase-space, dynamics, safe-set, policy/information correspondences); `ANALOGY_ONLY` is excluded from transfer *by the theorem*. **No five-map certificate exists from the linear (S,K) toy to 2J3KL or to any A021 J-series model.** The only other route is R04.Cor2 approximate admission (dynamics defect `ε` → Grönwall deviation `(ε/L)(e^{LT}−1)` → kernel erosion) — likewise not constructed for any real system.
...
- **Transfer obligations (from the prohibition section):** any use of this module's numbers toward 2J3KL/J-17-class systems requires first constructing the R04 five-map certificate or the Cor2 approximate admission — neither exists; the attempt is a registered Wave-0/G1 gating item.
