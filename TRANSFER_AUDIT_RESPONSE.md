...
2. **Edwards well J-17** — Edwards Aquifer San Antonio Pool index well (TWDB 6837203): a real groundwater system (G1b). A scored forecast ladder \(\Omega_{\mathrm{SA}}\) exists locally (`wave_e_edwards/`). There is **no** case-search manuscript in this repository that rejects Edwards on a “confound gate”; that clause is withdrawn. A005 two-pool stays conditionally admissible. Local R04.Cor2 covers the H0 **forecast map only**, not a kernel certificate.
